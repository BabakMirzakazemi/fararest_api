# Graph Report - main  (2026-05-17)

## Corpus Check
- 1716 files · ~226,442 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 12083 nodes · 10953 edges · 1650 communities (1541 shown, 109 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 2 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f6935755`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- [[_COMMUNITY_Community 0|Community 0]]
- [[_COMMUNITY_Community 1|Community 1]]
- [[_COMMUNITY_Community 2|Community 2]]
- [[_COMMUNITY_Community 3|Community 3]]
- [[_COMMUNITY_Community 4|Community 4]]
- [[_COMMUNITY_Community 5|Community 5]]
- [[_COMMUNITY_Community 6|Community 6]]
- [[_COMMUNITY_Community 7|Community 7]]
- [[_COMMUNITY_Community 8|Community 8]]
- [[_COMMUNITY_Community 9|Community 9]]
- [[_COMMUNITY_Community 10|Community 10]]
- [[_COMMUNITY_Community 11|Community 11]]
- [[_COMMUNITY_Community 12|Community 12]]
- [[_COMMUNITY_Community 13|Community 13]]
- [[_COMMUNITY_Community 14|Community 14]]
- [[_COMMUNITY_Community 15|Community 15]]
- [[_COMMUNITY_Community 16|Community 16]]
- [[_COMMUNITY_Community 17|Community 17]]
- [[_COMMUNITY_Community 18|Community 18]]
- [[_COMMUNITY_Community 19|Community 19]]
- [[_COMMUNITY_Community 20|Community 20]]
- [[_COMMUNITY_Community 21|Community 21]]
- [[_COMMUNITY_Community 22|Community 22]]
- [[_COMMUNITY_Community 23|Community 23]]
- [[_COMMUNITY_Community 24|Community 24]]
- [[_COMMUNITY_Community 25|Community 25]]
- [[_COMMUNITY_Community 26|Community 26]]
- [[_COMMUNITY_Community 27|Community 27]]
- [[_COMMUNITY_Community 28|Community 28]]
- [[_COMMUNITY_Community 29|Community 29]]
- [[_COMMUNITY_Community 30|Community 30]]
- [[_COMMUNITY_Community 31|Community 31]]
- [[_COMMUNITY_Community 32|Community 32]]
- [[_COMMUNITY_Community 33|Community 33]]
- [[_COMMUNITY_Community 34|Community 34]]
- [[_COMMUNITY_Community 35|Community 35]]
- [[_COMMUNITY_Community 36|Community 36]]
- [[_COMMUNITY_Community 37|Community 37]]
- [[_COMMUNITY_Community 38|Community 38]]
- [[_COMMUNITY_Community 39|Community 39]]
- [[_COMMUNITY_Community 40|Community 40]]
- [[_COMMUNITY_Community 41|Community 41]]
- [[_COMMUNITY_Community 42|Community 42]]
- [[_COMMUNITY_Community 43|Community 43]]
- [[_COMMUNITY_Community 44|Community 44]]
- [[_COMMUNITY_Community 45|Community 45]]
- [[_COMMUNITY_Community 46|Community 46]]
- [[_COMMUNITY_Community 47|Community 47]]
- [[_COMMUNITY_Community 48|Community 48]]
- [[_COMMUNITY_Community 49|Community 49]]
- [[_COMMUNITY_Community 50|Community 50]]
- [[_COMMUNITY_Community 51|Community 51]]
- [[_COMMUNITY_Community 52|Community 52]]
- [[_COMMUNITY_Community 53|Community 53]]
- [[_COMMUNITY_Community 54|Community 54]]
- [[_COMMUNITY_Community 55|Community 55]]
- [[_COMMUNITY_Community 56|Community 56]]
- [[_COMMUNITY_Community 57|Community 57]]
- [[_COMMUNITY_Community 58|Community 58]]
- [[_COMMUNITY_Community 59|Community 59]]
- [[_COMMUNITY_Community 60|Community 60]]
- [[_COMMUNITY_Community 61|Community 61]]
- [[_COMMUNITY_Community 62|Community 62]]
- [[_COMMUNITY_Community 63|Community 63]]
- [[_COMMUNITY_Community 64|Community 64]]
- [[_COMMUNITY_Community 65|Community 65]]
- [[_COMMUNITY_Community 66|Community 66]]
- [[_COMMUNITY_Community 67|Community 67]]
- [[_COMMUNITY_Community 68|Community 68]]
- [[_COMMUNITY_Community 69|Community 69]]
- [[_COMMUNITY_Community 70|Community 70]]
- [[_COMMUNITY_Community 71|Community 71]]
- [[_COMMUNITY_Community 72|Community 72]]
- [[_COMMUNITY_Community 73|Community 73]]
- [[_COMMUNITY_Community 74|Community 74]]
- [[_COMMUNITY_Community 75|Community 75]]
- [[_COMMUNITY_Community 76|Community 76]]
- [[_COMMUNITY_Community 77|Community 77]]
- [[_COMMUNITY_Community 78|Community 78]]
- [[_COMMUNITY_Community 79|Community 79]]
- [[_COMMUNITY_Community 80|Community 80]]
- [[_COMMUNITY_Community 81|Community 81]]
- [[_COMMUNITY_Community 82|Community 82]]
- [[_COMMUNITY_Community 83|Community 83]]
- [[_COMMUNITY_Community 84|Community 84]]
- [[_COMMUNITY_Community 85|Community 85]]
- [[_COMMUNITY_Community 86|Community 86]]
- [[_COMMUNITY_Community 87|Community 87]]
- [[_COMMUNITY_Community 88|Community 88]]
- [[_COMMUNITY_Community 89|Community 89]]
- [[_COMMUNITY_Community 90|Community 90]]
- [[_COMMUNITY_Community 91|Community 91]]
- [[_COMMUNITY_Community 92|Community 92]]
- [[_COMMUNITY_Community 93|Community 93]]
- [[_COMMUNITY_Community 94|Community 94]]
- [[_COMMUNITY_Community 95|Community 95]]
- [[_COMMUNITY_Community 96|Community 96]]
- [[_COMMUNITY_Community 97|Community 97]]
- [[_COMMUNITY_Community 98|Community 98]]
- [[_COMMUNITY_Community 99|Community 99]]
- [[_COMMUNITY_Community 100|Community 100]]
- [[_COMMUNITY_Community 101|Community 101]]
- [[_COMMUNITY_Community 102|Community 102]]
- [[_COMMUNITY_Community 103|Community 103]]
- [[_COMMUNITY_Community 104|Community 104]]
- [[_COMMUNITY_Community 105|Community 105]]
- [[_COMMUNITY_Community 106|Community 106]]
- [[_COMMUNITY_Community 107|Community 107]]
- [[_COMMUNITY_Community 108|Community 108]]
- [[_COMMUNITY_Community 109|Community 109]]
- [[_COMMUNITY_Community 110|Community 110]]
- [[_COMMUNITY_Community 111|Community 111]]
- [[_COMMUNITY_Community 112|Community 112]]
- [[_COMMUNITY_Community 113|Community 113]]
- [[_COMMUNITY_Community 114|Community 114]]
- [[_COMMUNITY_Community 115|Community 115]]
- [[_COMMUNITY_Community 116|Community 116]]
- [[_COMMUNITY_Community 117|Community 117]]
- [[_COMMUNITY_Community 118|Community 118]]
- [[_COMMUNITY_Community 119|Community 119]]
- [[_COMMUNITY_Community 120|Community 120]]
- [[_COMMUNITY_Community 121|Community 121]]
- [[_COMMUNITY_Community 122|Community 122]]
- [[_COMMUNITY_Community 123|Community 123]]
- [[_COMMUNITY_Community 124|Community 124]]
- [[_COMMUNITY_Community 125|Community 125]]
- [[_COMMUNITY_Community 126|Community 126]]
- [[_COMMUNITY_Community 127|Community 127]]
- [[_COMMUNITY_Community 128|Community 128]]
- [[_COMMUNITY_Community 129|Community 129]]
- [[_COMMUNITY_Community 130|Community 130]]
- [[_COMMUNITY_Community 131|Community 131]]
- [[_COMMUNITY_Community 132|Community 132]]
- [[_COMMUNITY_Community 133|Community 133]]
- [[_COMMUNITY_Community 134|Community 134]]
- [[_COMMUNITY_Community 135|Community 135]]
- [[_COMMUNITY_Community 136|Community 136]]
- [[_COMMUNITY_Community 137|Community 137]]
- [[_COMMUNITY_Community 138|Community 138]]
- [[_COMMUNITY_Community 139|Community 139]]
- [[_COMMUNITY_Community 140|Community 140]]
- [[_COMMUNITY_Community 141|Community 141]]
- [[_COMMUNITY_Community 142|Community 142]]
- [[_COMMUNITY_Community 143|Community 143]]
- [[_COMMUNITY_Community 144|Community 144]]
- [[_COMMUNITY_Community 145|Community 145]]
- [[_COMMUNITY_Community 146|Community 146]]
- [[_COMMUNITY_Community 147|Community 147]]
- [[_COMMUNITY_Community 148|Community 148]]
- [[_COMMUNITY_Community 149|Community 149]]
- [[_COMMUNITY_Community 150|Community 150]]
- [[_COMMUNITY_Community 151|Community 151]]
- [[_COMMUNITY_Community 152|Community 152]]
- [[_COMMUNITY_Community 153|Community 153]]
- [[_COMMUNITY_Community 154|Community 154]]
- [[_COMMUNITY_Community 155|Community 155]]
- [[_COMMUNITY_Community 156|Community 156]]
- [[_COMMUNITY_Community 157|Community 157]]
- [[_COMMUNITY_Community 158|Community 158]]
- [[_COMMUNITY_Community 159|Community 159]]
- [[_COMMUNITY_Community 160|Community 160]]
- [[_COMMUNITY_Community 161|Community 161]]
- [[_COMMUNITY_Community 162|Community 162]]
- [[_COMMUNITY_Community 163|Community 163]]
- [[_COMMUNITY_Community 164|Community 164]]
- [[_COMMUNITY_Community 165|Community 165]]
- [[_COMMUNITY_Community 166|Community 166]]
- [[_COMMUNITY_Community 167|Community 167]]
- [[_COMMUNITY_Community 168|Community 168]]
- [[_COMMUNITY_Community 169|Community 169]]
- [[_COMMUNITY_Community 170|Community 170]]
- [[_COMMUNITY_Community 171|Community 171]]
- [[_COMMUNITY_Community 172|Community 172]]
- [[_COMMUNITY_Community 173|Community 173]]
- [[_COMMUNITY_Community 174|Community 174]]
- [[_COMMUNITY_Community 175|Community 175]]
- [[_COMMUNITY_Community 176|Community 176]]
- [[_COMMUNITY_Community 177|Community 177]]
- [[_COMMUNITY_Community 178|Community 178]]
- [[_COMMUNITY_Community 179|Community 179]]
- [[_COMMUNITY_Community 180|Community 180]]
- [[_COMMUNITY_Community 181|Community 181]]
- [[_COMMUNITY_Community 182|Community 182]]
- [[_COMMUNITY_Community 183|Community 183]]
- [[_COMMUNITY_Community 184|Community 184]]
- [[_COMMUNITY_Community 185|Community 185]]
- [[_COMMUNITY_Community 186|Community 186]]
- [[_COMMUNITY_Community 187|Community 187]]
- [[_COMMUNITY_Community 188|Community 188]]
- [[_COMMUNITY_Community 189|Community 189]]
- [[_COMMUNITY_Community 190|Community 190]]
- [[_COMMUNITY_Community 191|Community 191]]
- [[_COMMUNITY_Community 192|Community 192]]
- [[_COMMUNITY_Community 193|Community 193]]
- [[_COMMUNITY_Community 194|Community 194]]
- [[_COMMUNITY_Community 195|Community 195]]
- [[_COMMUNITY_Community 196|Community 196]]
- [[_COMMUNITY_Community 197|Community 197]]
- [[_COMMUNITY_Community 198|Community 198]]
- [[_COMMUNITY_Community 199|Community 199]]
- [[_COMMUNITY_Community 200|Community 200]]
- [[_COMMUNITY_Community 201|Community 201]]
- [[_COMMUNITY_Community 202|Community 202]]
- [[_COMMUNITY_Community 203|Community 203]]
- [[_COMMUNITY_Community 204|Community 204]]
- [[_COMMUNITY_Community 205|Community 205]]
- [[_COMMUNITY_Community 206|Community 206]]
- [[_COMMUNITY_Community 207|Community 207]]
- [[_COMMUNITY_Community 208|Community 208]]
- [[_COMMUNITY_Community 209|Community 209]]
- [[_COMMUNITY_Community 210|Community 210]]
- [[_COMMUNITY_Community 211|Community 211]]
- [[_COMMUNITY_Community 212|Community 212]]
- [[_COMMUNITY_Community 213|Community 213]]
- [[_COMMUNITY_Community 214|Community 214]]
- [[_COMMUNITY_Community 215|Community 215]]
- [[_COMMUNITY_Community 216|Community 216]]
- [[_COMMUNITY_Community 217|Community 217]]
- [[_COMMUNITY_Community 218|Community 218]]
- [[_COMMUNITY_Community 219|Community 219]]
- [[_COMMUNITY_Community 220|Community 220]]
- [[_COMMUNITY_Community 221|Community 221]]
- [[_COMMUNITY_Community 222|Community 222]]
- [[_COMMUNITY_Community 223|Community 223]]
- [[_COMMUNITY_Community 224|Community 224]]
- [[_COMMUNITY_Community 225|Community 225]]
- [[_COMMUNITY_Community 226|Community 226]]
- [[_COMMUNITY_Community 227|Community 227]]
- [[_COMMUNITY_Community 228|Community 228]]
- [[_COMMUNITY_Community 229|Community 229]]
- [[_COMMUNITY_Community 230|Community 230]]
- [[_COMMUNITY_Community 231|Community 231]]
- [[_COMMUNITY_Community 232|Community 232]]
- [[_COMMUNITY_Community 233|Community 233]]
- [[_COMMUNITY_Community 234|Community 234]]
- [[_COMMUNITY_Community 235|Community 235]]
- [[_COMMUNITY_Community 236|Community 236]]
- [[_COMMUNITY_Community 237|Community 237]]
- [[_COMMUNITY_Community 238|Community 238]]
- [[_COMMUNITY_Community 239|Community 239]]
- [[_COMMUNITY_Community 240|Community 240]]
- [[_COMMUNITY_Community 241|Community 241]]
- [[_COMMUNITY_Community 242|Community 242]]
- [[_COMMUNITY_Community 243|Community 243]]
- [[_COMMUNITY_Community 244|Community 244]]
- [[_COMMUNITY_Community 245|Community 245]]
- [[_COMMUNITY_Community 246|Community 246]]
- [[_COMMUNITY_Community 247|Community 247]]
- [[_COMMUNITY_Community 248|Community 248]]
- [[_COMMUNITY_Community 249|Community 249]]
- [[_COMMUNITY_Community 250|Community 250]]
- [[_COMMUNITY_Community 251|Community 251]]
- [[_COMMUNITY_Community 252|Community 252]]
- [[_COMMUNITY_Community 253|Community 253]]
- [[_COMMUNITY_Community 254|Community 254]]
- [[_COMMUNITY_Community 255|Community 255]]
- [[_COMMUNITY_Community 256|Community 256]]
- [[_COMMUNITY_Community 257|Community 257]]
- [[_COMMUNITY_Community 258|Community 258]]
- [[_COMMUNITY_Community 259|Community 259]]
- [[_COMMUNITY_Community 260|Community 260]]
- [[_COMMUNITY_Community 261|Community 261]]
- [[_COMMUNITY_Community 262|Community 262]]
- [[_COMMUNITY_Community 263|Community 263]]
- [[_COMMUNITY_Community 264|Community 264]]
- [[_COMMUNITY_Community 265|Community 265]]
- [[_COMMUNITY_Community 266|Community 266]]
- [[_COMMUNITY_Community 267|Community 267]]
- [[_COMMUNITY_Community 268|Community 268]]
- [[_COMMUNITY_Community 269|Community 269]]
- [[_COMMUNITY_Community 270|Community 270]]
- [[_COMMUNITY_Community 271|Community 271]]
- [[_COMMUNITY_Community 272|Community 272]]
- [[_COMMUNITY_Community 273|Community 273]]
- [[_COMMUNITY_Community 274|Community 274]]
- [[_COMMUNITY_Community 275|Community 275]]
- [[_COMMUNITY_Community 276|Community 276]]
- [[_COMMUNITY_Community 277|Community 277]]
- [[_COMMUNITY_Community 278|Community 278]]
- [[_COMMUNITY_Community 279|Community 279]]
- [[_COMMUNITY_Community 280|Community 280]]
- [[_COMMUNITY_Community 281|Community 281]]
- [[_COMMUNITY_Community 282|Community 282]]
- [[_COMMUNITY_Community 283|Community 283]]
- [[_COMMUNITY_Community 284|Community 284]]
- [[_COMMUNITY_Community 285|Community 285]]
- [[_COMMUNITY_Community 286|Community 286]]
- [[_COMMUNITY_Community 287|Community 287]]
- [[_COMMUNITY_Community 288|Community 288]]
- [[_COMMUNITY_Community 289|Community 289]]
- [[_COMMUNITY_Community 290|Community 290]]
- [[_COMMUNITY_Community 291|Community 291]]
- [[_COMMUNITY_Community 292|Community 292]]
- [[_COMMUNITY_Community 293|Community 293]]
- [[_COMMUNITY_Community 294|Community 294]]
- [[_COMMUNITY_Community 295|Community 295]]
- [[_COMMUNITY_Community 296|Community 296]]
- [[_COMMUNITY_Community 297|Community 297]]
- [[_COMMUNITY_Community 298|Community 298]]
- [[_COMMUNITY_Community 299|Community 299]]
- [[_COMMUNITY_Community 300|Community 300]]
- [[_COMMUNITY_Community 301|Community 301]]
- [[_COMMUNITY_Community 302|Community 302]]
- [[_COMMUNITY_Community 303|Community 303]]
- [[_COMMUNITY_Community 304|Community 304]]
- [[_COMMUNITY_Community 305|Community 305]]
- [[_COMMUNITY_Community 306|Community 306]]
- [[_COMMUNITY_Community 307|Community 307]]
- [[_COMMUNITY_Community 308|Community 308]]
- [[_COMMUNITY_Community 309|Community 309]]
- [[_COMMUNITY_Community 310|Community 310]]
- [[_COMMUNITY_Community 311|Community 311]]
- [[_COMMUNITY_Community 312|Community 312]]
- [[_COMMUNITY_Community 313|Community 313]]
- [[_COMMUNITY_Community 314|Community 314]]
- [[_COMMUNITY_Community 315|Community 315]]
- [[_COMMUNITY_Community 316|Community 316]]
- [[_COMMUNITY_Community 317|Community 317]]
- [[_COMMUNITY_Community 318|Community 318]]
- [[_COMMUNITY_Community 319|Community 319]]
- [[_COMMUNITY_Community 320|Community 320]]
- [[_COMMUNITY_Community 321|Community 321]]
- [[_COMMUNITY_Community 322|Community 322]]
- [[_COMMUNITY_Community 323|Community 323]]
- [[_COMMUNITY_Community 324|Community 324]]
- [[_COMMUNITY_Community 325|Community 325]]
- [[_COMMUNITY_Community 326|Community 326]]
- [[_COMMUNITY_Community 327|Community 327]]
- [[_COMMUNITY_Community 328|Community 328]]
- [[_COMMUNITY_Community 329|Community 329]]
- [[_COMMUNITY_Community 330|Community 330]]
- [[_COMMUNITY_Community 331|Community 331]]
- [[_COMMUNITY_Community 332|Community 332]]
- [[_COMMUNITY_Community 333|Community 333]]
- [[_COMMUNITY_Community 334|Community 334]]
- [[_COMMUNITY_Community 335|Community 335]]
- [[_COMMUNITY_Community 336|Community 336]]
- [[_COMMUNITY_Community 337|Community 337]]
- [[_COMMUNITY_Community 338|Community 338]]
- [[_COMMUNITY_Community 339|Community 339]]
- [[_COMMUNITY_Community 340|Community 340]]
- [[_COMMUNITY_Community 341|Community 341]]
- [[_COMMUNITY_Community 342|Community 342]]
- [[_COMMUNITY_Community 343|Community 343]]
- [[_COMMUNITY_Community 344|Community 344]]
- [[_COMMUNITY_Community 345|Community 345]]
- [[_COMMUNITY_Community 346|Community 346]]
- [[_COMMUNITY_Community 347|Community 347]]
- [[_COMMUNITY_Community 348|Community 348]]
- [[_COMMUNITY_Community 349|Community 349]]
- [[_COMMUNITY_Community 350|Community 350]]
- [[_COMMUNITY_Community 351|Community 351]]
- [[_COMMUNITY_Community 352|Community 352]]
- [[_COMMUNITY_Community 353|Community 353]]
- [[_COMMUNITY_Community 354|Community 354]]
- [[_COMMUNITY_Community 355|Community 355]]
- [[_COMMUNITY_Community 356|Community 356]]
- [[_COMMUNITY_Community 357|Community 357]]
- [[_COMMUNITY_Community 358|Community 358]]
- [[_COMMUNITY_Community 359|Community 359]]
- [[_COMMUNITY_Community 360|Community 360]]
- [[_COMMUNITY_Community 361|Community 361]]
- [[_COMMUNITY_Community 362|Community 362]]
- [[_COMMUNITY_Community 363|Community 363]]
- [[_COMMUNITY_Community 364|Community 364]]
- [[_COMMUNITY_Community 365|Community 365]]
- [[_COMMUNITY_Community 366|Community 366]]
- [[_COMMUNITY_Community 367|Community 367]]
- [[_COMMUNITY_Community 368|Community 368]]
- [[_COMMUNITY_Community 369|Community 369]]
- [[_COMMUNITY_Community 370|Community 370]]
- [[_COMMUNITY_Community 371|Community 371]]
- [[_COMMUNITY_Community 372|Community 372]]
- [[_COMMUNITY_Community 373|Community 373]]
- [[_COMMUNITY_Community 374|Community 374]]
- [[_COMMUNITY_Community 375|Community 375]]
- [[_COMMUNITY_Community 376|Community 376]]
- [[_COMMUNITY_Community 377|Community 377]]
- [[_COMMUNITY_Community 378|Community 378]]
- [[_COMMUNITY_Community 379|Community 379]]
- [[_COMMUNITY_Community 380|Community 380]]
- [[_COMMUNITY_Community 381|Community 381]]
- [[_COMMUNITY_Community 382|Community 382]]
- [[_COMMUNITY_Community 383|Community 383]]
- [[_COMMUNITY_Community 384|Community 384]]
- [[_COMMUNITY_Community 385|Community 385]]
- [[_COMMUNITY_Community 386|Community 386]]
- [[_COMMUNITY_Community 387|Community 387]]
- [[_COMMUNITY_Community 388|Community 388]]
- [[_COMMUNITY_Community 389|Community 389]]
- [[_COMMUNITY_Community 390|Community 390]]
- [[_COMMUNITY_Community 391|Community 391]]
- [[_COMMUNITY_Community 392|Community 392]]
- [[_COMMUNITY_Community 393|Community 393]]
- [[_COMMUNITY_Community 394|Community 394]]
- [[_COMMUNITY_Community 395|Community 395]]
- [[_COMMUNITY_Community 396|Community 396]]
- [[_COMMUNITY_Community 397|Community 397]]
- [[_COMMUNITY_Community 398|Community 398]]
- [[_COMMUNITY_Community 399|Community 399]]
- [[_COMMUNITY_Community 400|Community 400]]
- [[_COMMUNITY_Community 401|Community 401]]
- [[_COMMUNITY_Community 402|Community 402]]
- [[_COMMUNITY_Community 403|Community 403]]
- [[_COMMUNITY_Community 404|Community 404]]
- [[_COMMUNITY_Community 405|Community 405]]
- [[_COMMUNITY_Community 406|Community 406]]
- [[_COMMUNITY_Community 407|Community 407]]
- [[_COMMUNITY_Community 408|Community 408]]
- [[_COMMUNITY_Community 409|Community 409]]
- [[_COMMUNITY_Community 410|Community 410]]
- [[_COMMUNITY_Community 411|Community 411]]
- [[_COMMUNITY_Community 412|Community 412]]
- [[_COMMUNITY_Community 413|Community 413]]
- [[_COMMUNITY_Community 414|Community 414]]
- [[_COMMUNITY_Community 415|Community 415]]
- [[_COMMUNITY_Community 416|Community 416]]
- [[_COMMUNITY_Community 417|Community 417]]
- [[_COMMUNITY_Community 418|Community 418]]
- [[_COMMUNITY_Community 419|Community 419]]
- [[_COMMUNITY_Community 420|Community 420]]
- [[_COMMUNITY_Community 421|Community 421]]
- [[_COMMUNITY_Community 422|Community 422]]
- [[_COMMUNITY_Community 423|Community 423]]
- [[_COMMUNITY_Community 424|Community 424]]
- [[_COMMUNITY_Community 425|Community 425]]
- [[_COMMUNITY_Community 426|Community 426]]
- [[_COMMUNITY_Community 427|Community 427]]
- [[_COMMUNITY_Community 428|Community 428]]
- [[_COMMUNITY_Community 429|Community 429]]
- [[_COMMUNITY_Community 430|Community 430]]
- [[_COMMUNITY_Community 431|Community 431]]
- [[_COMMUNITY_Community 432|Community 432]]
- [[_COMMUNITY_Community 433|Community 433]]
- [[_COMMUNITY_Community 434|Community 434]]
- [[_COMMUNITY_Community 435|Community 435]]
- [[_COMMUNITY_Community 436|Community 436]]
- [[_COMMUNITY_Community 437|Community 437]]
- [[_COMMUNITY_Community 438|Community 438]]
- [[_COMMUNITY_Community 439|Community 439]]
- [[_COMMUNITY_Community 440|Community 440]]
- [[_COMMUNITY_Community 441|Community 441]]
- [[_COMMUNITY_Community 442|Community 442]]
- [[_COMMUNITY_Community 443|Community 443]]
- [[_COMMUNITY_Community 444|Community 444]]
- [[_COMMUNITY_Community 445|Community 445]]
- [[_COMMUNITY_Community 446|Community 446]]
- [[_COMMUNITY_Community 447|Community 447]]
- [[_COMMUNITY_Community 448|Community 448]]
- [[_COMMUNITY_Community 449|Community 449]]
- [[_COMMUNITY_Community 450|Community 450]]
- [[_COMMUNITY_Community 451|Community 451]]
- [[_COMMUNITY_Community 452|Community 452]]
- [[_COMMUNITY_Community 453|Community 453]]
- [[_COMMUNITY_Community 454|Community 454]]
- [[_COMMUNITY_Community 455|Community 455]]
- [[_COMMUNITY_Community 456|Community 456]]
- [[_COMMUNITY_Community 457|Community 457]]
- [[_COMMUNITY_Community 458|Community 458]]
- [[_COMMUNITY_Community 459|Community 459]]
- [[_COMMUNITY_Community 460|Community 460]]
- [[_COMMUNITY_Community 461|Community 461]]
- [[_COMMUNITY_Community 462|Community 462]]
- [[_COMMUNITY_Community 463|Community 463]]
- [[_COMMUNITY_Community 464|Community 464]]
- [[_COMMUNITY_Community 465|Community 465]]
- [[_COMMUNITY_Community 466|Community 466]]
- [[_COMMUNITY_Community 467|Community 467]]
- [[_COMMUNITY_Community 468|Community 468]]
- [[_COMMUNITY_Community 469|Community 469]]
- [[_COMMUNITY_Community 470|Community 470]]
- [[_COMMUNITY_Community 471|Community 471]]
- [[_COMMUNITY_Community 472|Community 472]]
- [[_COMMUNITY_Community 473|Community 473]]
- [[_COMMUNITY_Community 474|Community 474]]
- [[_COMMUNITY_Community 475|Community 475]]
- [[_COMMUNITY_Community 476|Community 476]]
- [[_COMMUNITY_Community 477|Community 477]]
- [[_COMMUNITY_Community 478|Community 478]]
- [[_COMMUNITY_Community 479|Community 479]]
- [[_COMMUNITY_Community 480|Community 480]]
- [[_COMMUNITY_Community 481|Community 481]]
- [[_COMMUNITY_Community 482|Community 482]]
- [[_COMMUNITY_Community 483|Community 483]]
- [[_COMMUNITY_Community 484|Community 484]]
- [[_COMMUNITY_Community 485|Community 485]]
- [[_COMMUNITY_Community 486|Community 486]]
- [[_COMMUNITY_Community 487|Community 487]]
- [[_COMMUNITY_Community 488|Community 488]]
- [[_COMMUNITY_Community 489|Community 489]]
- [[_COMMUNITY_Community 490|Community 490]]
- [[_COMMUNITY_Community 491|Community 491]]
- [[_COMMUNITY_Community 492|Community 492]]
- [[_COMMUNITY_Community 493|Community 493]]
- [[_COMMUNITY_Community 494|Community 494]]
- [[_COMMUNITY_Community 495|Community 495]]
- [[_COMMUNITY_Community 496|Community 496]]
- [[_COMMUNITY_Community 497|Community 497]]
- [[_COMMUNITY_Community 498|Community 498]]
- [[_COMMUNITY_Community 499|Community 499]]
- [[_COMMUNITY_Community 500|Community 500]]
- [[_COMMUNITY_Community 501|Community 501]]
- [[_COMMUNITY_Community 502|Community 502]]
- [[_COMMUNITY_Community 503|Community 503]]
- [[_COMMUNITY_Community 504|Community 504]]
- [[_COMMUNITY_Community 505|Community 505]]
- [[_COMMUNITY_Community 506|Community 506]]
- [[_COMMUNITY_Community 507|Community 507]]
- [[_COMMUNITY_Community 508|Community 508]]
- [[_COMMUNITY_Community 509|Community 509]]
- [[_COMMUNITY_Community 510|Community 510]]
- [[_COMMUNITY_Community 511|Community 511]]
- [[_COMMUNITY_Community 512|Community 512]]
- [[_COMMUNITY_Community 513|Community 513]]
- [[_COMMUNITY_Community 514|Community 514]]
- [[_COMMUNITY_Community 515|Community 515]]
- [[_COMMUNITY_Community 516|Community 516]]
- [[_COMMUNITY_Community 517|Community 517]]
- [[_COMMUNITY_Community 518|Community 518]]
- [[_COMMUNITY_Community 519|Community 519]]
- [[_COMMUNITY_Community 520|Community 520]]
- [[_COMMUNITY_Community 521|Community 521]]
- [[_COMMUNITY_Community 522|Community 522]]
- [[_COMMUNITY_Community 523|Community 523]]
- [[_COMMUNITY_Community 524|Community 524]]
- [[_COMMUNITY_Community 525|Community 525]]
- [[_COMMUNITY_Community 526|Community 526]]
- [[_COMMUNITY_Community 527|Community 527]]
- [[_COMMUNITY_Community 528|Community 528]]
- [[_COMMUNITY_Community 529|Community 529]]
- [[_COMMUNITY_Community 530|Community 530]]
- [[_COMMUNITY_Community 531|Community 531]]
- [[_COMMUNITY_Community 532|Community 532]]
- [[_COMMUNITY_Community 533|Community 533]]
- [[_COMMUNITY_Community 534|Community 534]]
- [[_COMMUNITY_Community 535|Community 535]]
- [[_COMMUNITY_Community 536|Community 536]]
- [[_COMMUNITY_Community 537|Community 537]]
- [[_COMMUNITY_Community 538|Community 538]]
- [[_COMMUNITY_Community 539|Community 539]]
- [[_COMMUNITY_Community 540|Community 540]]
- [[_COMMUNITY_Community 541|Community 541]]
- [[_COMMUNITY_Community 542|Community 542]]
- [[_COMMUNITY_Community 543|Community 543]]
- [[_COMMUNITY_Community 544|Community 544]]
- [[_COMMUNITY_Community 545|Community 545]]
- [[_COMMUNITY_Community 546|Community 546]]
- [[_COMMUNITY_Community 547|Community 547]]
- [[_COMMUNITY_Community 548|Community 548]]
- [[_COMMUNITY_Community 549|Community 549]]
- [[_COMMUNITY_Community 550|Community 550]]
- [[_COMMUNITY_Community 551|Community 551]]
- [[_COMMUNITY_Community 552|Community 552]]
- [[_COMMUNITY_Community 553|Community 553]]
- [[_COMMUNITY_Community 554|Community 554]]
- [[_COMMUNITY_Community 555|Community 555]]
- [[_COMMUNITY_Community 556|Community 556]]
- [[_COMMUNITY_Community 557|Community 557]]
- [[_COMMUNITY_Community 558|Community 558]]
- [[_COMMUNITY_Community 559|Community 559]]
- [[_COMMUNITY_Community 560|Community 560]]
- [[_COMMUNITY_Community 561|Community 561]]
- [[_COMMUNITY_Community 562|Community 562]]
- [[_COMMUNITY_Community 563|Community 563]]
- [[_COMMUNITY_Community 564|Community 564]]
- [[_COMMUNITY_Community 565|Community 565]]
- [[_COMMUNITY_Community 566|Community 566]]
- [[_COMMUNITY_Community 567|Community 567]]
- [[_COMMUNITY_Community 568|Community 568]]
- [[_COMMUNITY_Community 569|Community 569]]
- [[_COMMUNITY_Community 570|Community 570]]
- [[_COMMUNITY_Community 571|Community 571]]
- [[_COMMUNITY_Community 572|Community 572]]
- [[_COMMUNITY_Community 573|Community 573]]
- [[_COMMUNITY_Community 574|Community 574]]
- [[_COMMUNITY_Community 575|Community 575]]
- [[_COMMUNITY_Community 576|Community 576]]
- [[_COMMUNITY_Community 577|Community 577]]
- [[_COMMUNITY_Community 578|Community 578]]
- [[_COMMUNITY_Community 579|Community 579]]
- [[_COMMUNITY_Community 580|Community 580]]
- [[_COMMUNITY_Community 581|Community 581]]
- [[_COMMUNITY_Community 582|Community 582]]
- [[_COMMUNITY_Community 583|Community 583]]
- [[_COMMUNITY_Community 584|Community 584]]
- [[_COMMUNITY_Community 585|Community 585]]
- [[_COMMUNITY_Community 586|Community 586]]
- [[_COMMUNITY_Community 587|Community 587]]
- [[_COMMUNITY_Community 588|Community 588]]
- [[_COMMUNITY_Community 589|Community 589]]
- [[_COMMUNITY_Community 590|Community 590]]
- [[_COMMUNITY_Community 591|Community 591]]
- [[_COMMUNITY_Community 592|Community 592]]
- [[_COMMUNITY_Community 593|Community 593]]
- [[_COMMUNITY_Community 594|Community 594]]
- [[_COMMUNITY_Community 595|Community 595]]
- [[_COMMUNITY_Community 596|Community 596]]
- [[_COMMUNITY_Community 597|Community 597]]
- [[_COMMUNITY_Community 598|Community 598]]
- [[_COMMUNITY_Community 599|Community 599]]
- [[_COMMUNITY_Community 600|Community 600]]
- [[_COMMUNITY_Community 601|Community 601]]
- [[_COMMUNITY_Community 602|Community 602]]
- [[_COMMUNITY_Community 603|Community 603]]
- [[_COMMUNITY_Community 604|Community 604]]
- [[_COMMUNITY_Community 605|Community 605]]
- [[_COMMUNITY_Community 606|Community 606]]
- [[_COMMUNITY_Community 607|Community 607]]
- [[_COMMUNITY_Community 608|Community 608]]
- [[_COMMUNITY_Community 609|Community 609]]
- [[_COMMUNITY_Community 610|Community 610]]
- [[_COMMUNITY_Community 611|Community 611]]
- [[_COMMUNITY_Community 612|Community 612]]
- [[_COMMUNITY_Community 613|Community 613]]
- [[_COMMUNITY_Community 614|Community 614]]
- [[_COMMUNITY_Community 615|Community 615]]
- [[_COMMUNITY_Community 616|Community 616]]
- [[_COMMUNITY_Community 617|Community 617]]
- [[_COMMUNITY_Community 618|Community 618]]
- [[_COMMUNITY_Community 619|Community 619]]
- [[_COMMUNITY_Community 620|Community 620]]
- [[_COMMUNITY_Community 621|Community 621]]
- [[_COMMUNITY_Community 622|Community 622]]
- [[_COMMUNITY_Community 623|Community 623]]
- [[_COMMUNITY_Community 624|Community 624]]
- [[_COMMUNITY_Community 625|Community 625]]
- [[_COMMUNITY_Community 626|Community 626]]
- [[_COMMUNITY_Community 627|Community 627]]
- [[_COMMUNITY_Community 628|Community 628]]
- [[_COMMUNITY_Community 629|Community 629]]
- [[_COMMUNITY_Community 630|Community 630]]
- [[_COMMUNITY_Community 631|Community 631]]
- [[_COMMUNITY_Community 632|Community 632]]
- [[_COMMUNITY_Community 633|Community 633]]
- [[_COMMUNITY_Community 634|Community 634]]
- [[_COMMUNITY_Community 635|Community 635]]
- [[_COMMUNITY_Community 636|Community 636]]
- [[_COMMUNITY_Community 637|Community 637]]
- [[_COMMUNITY_Community 638|Community 638]]
- [[_COMMUNITY_Community 639|Community 639]]
- [[_COMMUNITY_Community 640|Community 640]]
- [[_COMMUNITY_Community 641|Community 641]]
- [[_COMMUNITY_Community 642|Community 642]]
- [[_COMMUNITY_Community 643|Community 643]]
- [[_COMMUNITY_Community 644|Community 644]]
- [[_COMMUNITY_Community 645|Community 645]]
- [[_COMMUNITY_Community 646|Community 646]]
- [[_COMMUNITY_Community 647|Community 647]]
- [[_COMMUNITY_Community 648|Community 648]]
- [[_COMMUNITY_Community 649|Community 649]]
- [[_COMMUNITY_Community 650|Community 650]]
- [[_COMMUNITY_Community 651|Community 651]]
- [[_COMMUNITY_Community 652|Community 652]]
- [[_COMMUNITY_Community 653|Community 653]]
- [[_COMMUNITY_Community 654|Community 654]]
- [[_COMMUNITY_Community 655|Community 655]]
- [[_COMMUNITY_Community 656|Community 656]]
- [[_COMMUNITY_Community 657|Community 657]]
- [[_COMMUNITY_Community 658|Community 658]]
- [[_COMMUNITY_Community 659|Community 659]]
- [[_COMMUNITY_Community 660|Community 660]]
- [[_COMMUNITY_Community 661|Community 661]]
- [[_COMMUNITY_Community 662|Community 662]]
- [[_COMMUNITY_Community 663|Community 663]]
- [[_COMMUNITY_Community 664|Community 664]]
- [[_COMMUNITY_Community 665|Community 665]]
- [[_COMMUNITY_Community 666|Community 666]]
- [[_COMMUNITY_Community 667|Community 667]]
- [[_COMMUNITY_Community 668|Community 668]]
- [[_COMMUNITY_Community 669|Community 669]]
- [[_COMMUNITY_Community 670|Community 670]]
- [[_COMMUNITY_Community 671|Community 671]]
- [[_COMMUNITY_Community 672|Community 672]]
- [[_COMMUNITY_Community 673|Community 673]]
- [[_COMMUNITY_Community 674|Community 674]]
- [[_COMMUNITY_Community 675|Community 675]]
- [[_COMMUNITY_Community 676|Community 676]]
- [[_COMMUNITY_Community 677|Community 677]]
- [[_COMMUNITY_Community 678|Community 678]]
- [[_COMMUNITY_Community 679|Community 679]]
- [[_COMMUNITY_Community 680|Community 680]]
- [[_COMMUNITY_Community 681|Community 681]]
- [[_COMMUNITY_Community 682|Community 682]]
- [[_COMMUNITY_Community 683|Community 683]]
- [[_COMMUNITY_Community 684|Community 684]]
- [[_COMMUNITY_Community 685|Community 685]]
- [[_COMMUNITY_Community 686|Community 686]]
- [[_COMMUNITY_Community 687|Community 687]]
- [[_COMMUNITY_Community 688|Community 688]]
- [[_COMMUNITY_Community 689|Community 689]]
- [[_COMMUNITY_Community 690|Community 690]]
- [[_COMMUNITY_Community 691|Community 691]]
- [[_COMMUNITY_Community 692|Community 692]]
- [[_COMMUNITY_Community 693|Community 693]]
- [[_COMMUNITY_Community 694|Community 694]]
- [[_COMMUNITY_Community 695|Community 695]]
- [[_COMMUNITY_Community 696|Community 696]]
- [[_COMMUNITY_Community 697|Community 697]]
- [[_COMMUNITY_Community 698|Community 698]]
- [[_COMMUNITY_Community 699|Community 699]]
- [[_COMMUNITY_Community 700|Community 700]]
- [[_COMMUNITY_Community 701|Community 701]]
- [[_COMMUNITY_Community 702|Community 702]]
- [[_COMMUNITY_Community 703|Community 703]]
- [[_COMMUNITY_Community 704|Community 704]]
- [[_COMMUNITY_Community 705|Community 705]]
- [[_COMMUNITY_Community 706|Community 706]]
- [[_COMMUNITY_Community 707|Community 707]]
- [[_COMMUNITY_Community 708|Community 708]]
- [[_COMMUNITY_Community 709|Community 709]]
- [[_COMMUNITY_Community 710|Community 710]]
- [[_COMMUNITY_Community 711|Community 711]]
- [[_COMMUNITY_Community 712|Community 712]]
- [[_COMMUNITY_Community 713|Community 713]]
- [[_COMMUNITY_Community 714|Community 714]]
- [[_COMMUNITY_Community 715|Community 715]]
- [[_COMMUNITY_Community 716|Community 716]]
- [[_COMMUNITY_Community 717|Community 717]]
- [[_COMMUNITY_Community 718|Community 718]]
- [[_COMMUNITY_Community 719|Community 719]]
- [[_COMMUNITY_Community 720|Community 720]]
- [[_COMMUNITY_Community 721|Community 721]]
- [[_COMMUNITY_Community 722|Community 722]]
- [[_COMMUNITY_Community 723|Community 723]]
- [[_COMMUNITY_Community 724|Community 724]]
- [[_COMMUNITY_Community 725|Community 725]]
- [[_COMMUNITY_Community 726|Community 726]]
- [[_COMMUNITY_Community 727|Community 727]]
- [[_COMMUNITY_Community 728|Community 728]]
- [[_COMMUNITY_Community 729|Community 729]]
- [[_COMMUNITY_Community 730|Community 730]]
- [[_COMMUNITY_Community 731|Community 731]]
- [[_COMMUNITY_Community 732|Community 732]]
- [[_COMMUNITY_Community 733|Community 733]]
- [[_COMMUNITY_Community 734|Community 734]]
- [[_COMMUNITY_Community 735|Community 735]]
- [[_COMMUNITY_Community 736|Community 736]]
- [[_COMMUNITY_Community 737|Community 737]]
- [[_COMMUNITY_Community 738|Community 738]]
- [[_COMMUNITY_Community 739|Community 739]]
- [[_COMMUNITY_Community 740|Community 740]]
- [[_COMMUNITY_Community 741|Community 741]]
- [[_COMMUNITY_Community 742|Community 742]]
- [[_COMMUNITY_Community 743|Community 743]]
- [[_COMMUNITY_Community 744|Community 744]]
- [[_COMMUNITY_Community 745|Community 745]]
- [[_COMMUNITY_Community 746|Community 746]]
- [[_COMMUNITY_Community 747|Community 747]]
- [[_COMMUNITY_Community 748|Community 748]]
- [[_COMMUNITY_Community 749|Community 749]]
- [[_COMMUNITY_Community 750|Community 750]]
- [[_COMMUNITY_Community 751|Community 751]]
- [[_COMMUNITY_Community 752|Community 752]]
- [[_COMMUNITY_Community 753|Community 753]]
- [[_COMMUNITY_Community 754|Community 754]]
- [[_COMMUNITY_Community 755|Community 755]]
- [[_COMMUNITY_Community 756|Community 756]]
- [[_COMMUNITY_Community 757|Community 757]]
- [[_COMMUNITY_Community 758|Community 758]]
- [[_COMMUNITY_Community 759|Community 759]]
- [[_COMMUNITY_Community 760|Community 760]]
- [[_COMMUNITY_Community 761|Community 761]]
- [[_COMMUNITY_Community 762|Community 762]]
- [[_COMMUNITY_Community 763|Community 763]]
- [[_COMMUNITY_Community 764|Community 764]]
- [[_COMMUNITY_Community 765|Community 765]]
- [[_COMMUNITY_Community 766|Community 766]]
- [[_COMMUNITY_Community 767|Community 767]]
- [[_COMMUNITY_Community 768|Community 768]]
- [[_COMMUNITY_Community 769|Community 769]]
- [[_COMMUNITY_Community 770|Community 770]]
- [[_COMMUNITY_Community 771|Community 771]]
- [[_COMMUNITY_Community 772|Community 772]]
- [[_COMMUNITY_Community 773|Community 773]]
- [[_COMMUNITY_Community 774|Community 774]]
- [[_COMMUNITY_Community 775|Community 775]]
- [[_COMMUNITY_Community 776|Community 776]]
- [[_COMMUNITY_Community 777|Community 777]]
- [[_COMMUNITY_Community 778|Community 778]]
- [[_COMMUNITY_Community 779|Community 779]]
- [[_COMMUNITY_Community 780|Community 780]]
- [[_COMMUNITY_Community 781|Community 781]]
- [[_COMMUNITY_Community 782|Community 782]]
- [[_COMMUNITY_Community 783|Community 783]]
- [[_COMMUNITY_Community 784|Community 784]]
- [[_COMMUNITY_Community 785|Community 785]]
- [[_COMMUNITY_Community 786|Community 786]]
- [[_COMMUNITY_Community 787|Community 787]]
- [[_COMMUNITY_Community 788|Community 788]]
- [[_COMMUNITY_Community 789|Community 789]]
- [[_COMMUNITY_Community 790|Community 790]]
- [[_COMMUNITY_Community 791|Community 791]]
- [[_COMMUNITY_Community 792|Community 792]]
- [[_COMMUNITY_Community 793|Community 793]]
- [[_COMMUNITY_Community 794|Community 794]]
- [[_COMMUNITY_Community 795|Community 795]]
- [[_COMMUNITY_Community 796|Community 796]]
- [[_COMMUNITY_Community 797|Community 797]]
- [[_COMMUNITY_Community 798|Community 798]]
- [[_COMMUNITY_Community 799|Community 799]]
- [[_COMMUNITY_Community 800|Community 800]]
- [[_COMMUNITY_Community 801|Community 801]]
- [[_COMMUNITY_Community 802|Community 802]]
- [[_COMMUNITY_Community 803|Community 803]]
- [[_COMMUNITY_Community 804|Community 804]]
- [[_COMMUNITY_Community 805|Community 805]]
- [[_COMMUNITY_Community 806|Community 806]]
- [[_COMMUNITY_Community 807|Community 807]]
- [[_COMMUNITY_Community 808|Community 808]]
- [[_COMMUNITY_Community 809|Community 809]]
- [[_COMMUNITY_Community 810|Community 810]]
- [[_COMMUNITY_Community 811|Community 811]]
- [[_COMMUNITY_Community 812|Community 812]]
- [[_COMMUNITY_Community 813|Community 813]]
- [[_COMMUNITY_Community 814|Community 814]]
- [[_COMMUNITY_Community 815|Community 815]]
- [[_COMMUNITY_Community 816|Community 816]]
- [[_COMMUNITY_Community 817|Community 817]]
- [[_COMMUNITY_Community 818|Community 818]]
- [[_COMMUNITY_Community 819|Community 819]]
- [[_COMMUNITY_Community 820|Community 820]]
- [[_COMMUNITY_Community 821|Community 821]]
- [[_COMMUNITY_Community 822|Community 822]]
- [[_COMMUNITY_Community 823|Community 823]]
- [[_COMMUNITY_Community 824|Community 824]]
- [[_COMMUNITY_Community 825|Community 825]]
- [[_COMMUNITY_Community 826|Community 826]]
- [[_COMMUNITY_Community 827|Community 827]]
- [[_COMMUNITY_Community 828|Community 828]]
- [[_COMMUNITY_Community 829|Community 829]]
- [[_COMMUNITY_Community 830|Community 830]]
- [[_COMMUNITY_Community 831|Community 831]]
- [[_COMMUNITY_Community 832|Community 832]]
- [[_COMMUNITY_Community 833|Community 833]]
- [[_COMMUNITY_Community 834|Community 834]]
- [[_COMMUNITY_Community 835|Community 835]]
- [[_COMMUNITY_Community 836|Community 836]]
- [[_COMMUNITY_Community 837|Community 837]]
- [[_COMMUNITY_Community 838|Community 838]]
- [[_COMMUNITY_Community 839|Community 839]]
- [[_COMMUNITY_Community 840|Community 840]]
- [[_COMMUNITY_Community 841|Community 841]]
- [[_COMMUNITY_Community 842|Community 842]]
- [[_COMMUNITY_Community 843|Community 843]]
- [[_COMMUNITY_Community 844|Community 844]]
- [[_COMMUNITY_Community 845|Community 845]]
- [[_COMMUNITY_Community 846|Community 846]]
- [[_COMMUNITY_Community 847|Community 847]]
- [[_COMMUNITY_Community 848|Community 848]]
- [[_COMMUNITY_Community 849|Community 849]]
- [[_COMMUNITY_Community 850|Community 850]]
- [[_COMMUNITY_Community 851|Community 851]]
- [[_COMMUNITY_Community 852|Community 852]]
- [[_COMMUNITY_Community 853|Community 853]]
- [[_COMMUNITY_Community 854|Community 854]]
- [[_COMMUNITY_Community 855|Community 855]]
- [[_COMMUNITY_Community 856|Community 856]]
- [[_COMMUNITY_Community 857|Community 857]]
- [[_COMMUNITY_Community 858|Community 858]]
- [[_COMMUNITY_Community 859|Community 859]]
- [[_COMMUNITY_Community 860|Community 860]]
- [[_COMMUNITY_Community 861|Community 861]]
- [[_COMMUNITY_Community 862|Community 862]]
- [[_COMMUNITY_Community 863|Community 863]]
- [[_COMMUNITY_Community 864|Community 864]]
- [[_COMMUNITY_Community 865|Community 865]]
- [[_COMMUNITY_Community 866|Community 866]]
- [[_COMMUNITY_Community 867|Community 867]]
- [[_COMMUNITY_Community 868|Community 868]]
- [[_COMMUNITY_Community 869|Community 869]]
- [[_COMMUNITY_Community 870|Community 870]]
- [[_COMMUNITY_Community 871|Community 871]]
- [[_COMMUNITY_Community 872|Community 872]]
- [[_COMMUNITY_Community 873|Community 873]]
- [[_COMMUNITY_Community 874|Community 874]]
- [[_COMMUNITY_Community 875|Community 875]]
- [[_COMMUNITY_Community 876|Community 876]]
- [[_COMMUNITY_Community 877|Community 877]]
- [[_COMMUNITY_Community 878|Community 878]]
- [[_COMMUNITY_Community 879|Community 879]]
- [[_COMMUNITY_Community 880|Community 880]]
- [[_COMMUNITY_Community 881|Community 881]]
- [[_COMMUNITY_Community 882|Community 882]]
- [[_COMMUNITY_Community 883|Community 883]]
- [[_COMMUNITY_Community 884|Community 884]]
- [[_COMMUNITY_Community 885|Community 885]]
- [[_COMMUNITY_Community 886|Community 886]]
- [[_COMMUNITY_Community 887|Community 887]]
- [[_COMMUNITY_Community 888|Community 888]]
- [[_COMMUNITY_Community 889|Community 889]]
- [[_COMMUNITY_Community 890|Community 890]]
- [[_COMMUNITY_Community 891|Community 891]]
- [[_COMMUNITY_Community 892|Community 892]]
- [[_COMMUNITY_Community 893|Community 893]]
- [[_COMMUNITY_Community 894|Community 894]]
- [[_COMMUNITY_Community 895|Community 895]]
- [[_COMMUNITY_Community 896|Community 896]]
- [[_COMMUNITY_Community 897|Community 897]]
- [[_COMMUNITY_Community 898|Community 898]]
- [[_COMMUNITY_Community 899|Community 899]]
- [[_COMMUNITY_Community 900|Community 900]]
- [[_COMMUNITY_Community 901|Community 901]]
- [[_COMMUNITY_Community 902|Community 902]]
- [[_COMMUNITY_Community 903|Community 903]]
- [[_COMMUNITY_Community 904|Community 904]]
- [[_COMMUNITY_Community 905|Community 905]]
- [[_COMMUNITY_Community 906|Community 906]]
- [[_COMMUNITY_Community 907|Community 907]]
- [[_COMMUNITY_Community 908|Community 908]]
- [[_COMMUNITY_Community 909|Community 909]]
- [[_COMMUNITY_Community 910|Community 910]]
- [[_COMMUNITY_Community 911|Community 911]]
- [[_COMMUNITY_Community 912|Community 912]]
- [[_COMMUNITY_Community 913|Community 913]]
- [[_COMMUNITY_Community 914|Community 914]]
- [[_COMMUNITY_Community 915|Community 915]]
- [[_COMMUNITY_Community 916|Community 916]]
- [[_COMMUNITY_Community 917|Community 917]]
- [[_COMMUNITY_Community 918|Community 918]]
- [[_COMMUNITY_Community 919|Community 919]]
- [[_COMMUNITY_Community 920|Community 920]]
- [[_COMMUNITY_Community 921|Community 921]]
- [[_COMMUNITY_Community 922|Community 922]]
- [[_COMMUNITY_Community 923|Community 923]]
- [[_COMMUNITY_Community 924|Community 924]]
- [[_COMMUNITY_Community 925|Community 925]]
- [[_COMMUNITY_Community 926|Community 926]]
- [[_COMMUNITY_Community 927|Community 927]]
- [[_COMMUNITY_Community 928|Community 928]]
- [[_COMMUNITY_Community 929|Community 929]]
- [[_COMMUNITY_Community 930|Community 930]]
- [[_COMMUNITY_Community 931|Community 931]]
- [[_COMMUNITY_Community 932|Community 932]]
- [[_COMMUNITY_Community 933|Community 933]]
- [[_COMMUNITY_Community 934|Community 934]]
- [[_COMMUNITY_Community 935|Community 935]]
- [[_COMMUNITY_Community 936|Community 936]]
- [[_COMMUNITY_Community 937|Community 937]]
- [[_COMMUNITY_Community 938|Community 938]]
- [[_COMMUNITY_Community 939|Community 939]]
- [[_COMMUNITY_Community 940|Community 940]]
- [[_COMMUNITY_Community 941|Community 941]]
- [[_COMMUNITY_Community 942|Community 942]]
- [[_COMMUNITY_Community 943|Community 943]]
- [[_COMMUNITY_Community 944|Community 944]]
- [[_COMMUNITY_Community 945|Community 945]]
- [[_COMMUNITY_Community 946|Community 946]]
- [[_COMMUNITY_Community 947|Community 947]]
- [[_COMMUNITY_Community 948|Community 948]]
- [[_COMMUNITY_Community 949|Community 949]]
- [[_COMMUNITY_Community 950|Community 950]]
- [[_COMMUNITY_Community 951|Community 951]]
- [[_COMMUNITY_Community 952|Community 952]]
- [[_COMMUNITY_Community 953|Community 953]]
- [[_COMMUNITY_Community 954|Community 954]]
- [[_COMMUNITY_Community 955|Community 955]]
- [[_COMMUNITY_Community 956|Community 956]]
- [[_COMMUNITY_Community 957|Community 957]]
- [[_COMMUNITY_Community 958|Community 958]]
- [[_COMMUNITY_Community 959|Community 959]]
- [[_COMMUNITY_Community 960|Community 960]]
- [[_COMMUNITY_Community 961|Community 961]]
- [[_COMMUNITY_Community 962|Community 962]]
- [[_COMMUNITY_Community 963|Community 963]]
- [[_COMMUNITY_Community 964|Community 964]]
- [[_COMMUNITY_Community 965|Community 965]]
- [[_COMMUNITY_Community 966|Community 966]]
- [[_COMMUNITY_Community 967|Community 967]]
- [[_COMMUNITY_Community 968|Community 968]]
- [[_COMMUNITY_Community 969|Community 969]]
- [[_COMMUNITY_Community 970|Community 970]]
- [[_COMMUNITY_Community 971|Community 971]]
- [[_COMMUNITY_Community 972|Community 972]]
- [[_COMMUNITY_Community 973|Community 973]]
- [[_COMMUNITY_Community 974|Community 974]]
- [[_COMMUNITY_Community 975|Community 975]]
- [[_COMMUNITY_Community 976|Community 976]]
- [[_COMMUNITY_Community 977|Community 977]]
- [[_COMMUNITY_Community 978|Community 978]]
- [[_COMMUNITY_Community 979|Community 979]]
- [[_COMMUNITY_Community 980|Community 980]]
- [[_COMMUNITY_Community 981|Community 981]]
- [[_COMMUNITY_Community 982|Community 982]]
- [[_COMMUNITY_Community 983|Community 983]]
- [[_COMMUNITY_Community 984|Community 984]]
- [[_COMMUNITY_Community 985|Community 985]]
- [[_COMMUNITY_Community 986|Community 986]]
- [[_COMMUNITY_Community 987|Community 987]]
- [[_COMMUNITY_Community 988|Community 988]]
- [[_COMMUNITY_Community 989|Community 989]]
- [[_COMMUNITY_Community 990|Community 990]]
- [[_COMMUNITY_Community 991|Community 991]]
- [[_COMMUNITY_Community 992|Community 992]]
- [[_COMMUNITY_Community 993|Community 993]]
- [[_COMMUNITY_Community 994|Community 994]]
- [[_COMMUNITY_Community 995|Community 995]]
- [[_COMMUNITY_Community 996|Community 996]]
- [[_COMMUNITY_Community 997|Community 997]]
- [[_COMMUNITY_Community 998|Community 998]]
- [[_COMMUNITY_Community 999|Community 999]]
- [[_COMMUNITY_Community 1000|Community 1000]]
- [[_COMMUNITY_Community 1001|Community 1001]]
- [[_COMMUNITY_Community 1002|Community 1002]]
- [[_COMMUNITY_Community 1003|Community 1003]]
- [[_COMMUNITY_Community 1004|Community 1004]]
- [[_COMMUNITY_Community 1005|Community 1005]]
- [[_COMMUNITY_Community 1006|Community 1006]]
- [[_COMMUNITY_Community 1007|Community 1007]]
- [[_COMMUNITY_Community 1008|Community 1008]]
- [[_COMMUNITY_Community 1009|Community 1009]]
- [[_COMMUNITY_Community 1010|Community 1010]]
- [[_COMMUNITY_Community 1011|Community 1011]]
- [[_COMMUNITY_Community 1012|Community 1012]]
- [[_COMMUNITY_Community 1013|Community 1013]]
- [[_COMMUNITY_Community 1014|Community 1014]]
- [[_COMMUNITY_Community 1015|Community 1015]]
- [[_COMMUNITY_Community 1016|Community 1016]]
- [[_COMMUNITY_Community 1017|Community 1017]]
- [[_COMMUNITY_Community 1018|Community 1018]]
- [[_COMMUNITY_Community 1019|Community 1019]]
- [[_COMMUNITY_Community 1020|Community 1020]]
- [[_COMMUNITY_Community 1021|Community 1021]]
- [[_COMMUNITY_Community 1022|Community 1022]]
- [[_COMMUNITY_Community 1023|Community 1023]]
- [[_COMMUNITY_Community 1024|Community 1024]]
- [[_COMMUNITY_Community 1025|Community 1025]]
- [[_COMMUNITY_Community 1026|Community 1026]]
- [[_COMMUNITY_Community 1027|Community 1027]]
- [[_COMMUNITY_Community 1028|Community 1028]]
- [[_COMMUNITY_Community 1029|Community 1029]]
- [[_COMMUNITY_Community 1030|Community 1030]]
- [[_COMMUNITY_Community 1031|Community 1031]]
- [[_COMMUNITY_Community 1032|Community 1032]]
- [[_COMMUNITY_Community 1033|Community 1033]]
- [[_COMMUNITY_Community 1034|Community 1034]]
- [[_COMMUNITY_Community 1035|Community 1035]]
- [[_COMMUNITY_Community 1036|Community 1036]]
- [[_COMMUNITY_Community 1037|Community 1037]]
- [[_COMMUNITY_Community 1038|Community 1038]]
- [[_COMMUNITY_Community 1039|Community 1039]]
- [[_COMMUNITY_Community 1040|Community 1040]]
- [[_COMMUNITY_Community 1041|Community 1041]]
- [[_COMMUNITY_Community 1042|Community 1042]]
- [[_COMMUNITY_Community 1043|Community 1043]]
- [[_COMMUNITY_Community 1044|Community 1044]]
- [[_COMMUNITY_Community 1045|Community 1045]]
- [[_COMMUNITY_Community 1046|Community 1046]]
- [[_COMMUNITY_Community 1047|Community 1047]]
- [[_COMMUNITY_Community 1048|Community 1048]]
- [[_COMMUNITY_Community 1049|Community 1049]]
- [[_COMMUNITY_Community 1050|Community 1050]]
- [[_COMMUNITY_Community 1051|Community 1051]]
- [[_COMMUNITY_Community 1052|Community 1052]]
- [[_COMMUNITY_Community 1053|Community 1053]]
- [[_COMMUNITY_Community 1054|Community 1054]]
- [[_COMMUNITY_Community 1055|Community 1055]]
- [[_COMMUNITY_Community 1056|Community 1056]]
- [[_COMMUNITY_Community 1057|Community 1057]]
- [[_COMMUNITY_Community 1058|Community 1058]]
- [[_COMMUNITY_Community 1059|Community 1059]]
- [[_COMMUNITY_Community 1060|Community 1060]]
- [[_COMMUNITY_Community 1061|Community 1061]]
- [[_COMMUNITY_Community 1062|Community 1062]]
- [[_COMMUNITY_Community 1063|Community 1063]]
- [[_COMMUNITY_Community 1064|Community 1064]]
- [[_COMMUNITY_Community 1065|Community 1065]]
- [[_COMMUNITY_Community 1066|Community 1066]]
- [[_COMMUNITY_Community 1067|Community 1067]]
- [[_COMMUNITY_Community 1068|Community 1068]]
- [[_COMMUNITY_Community 1069|Community 1069]]
- [[_COMMUNITY_Community 1070|Community 1070]]
- [[_COMMUNITY_Community 1071|Community 1071]]
- [[_COMMUNITY_Community 1072|Community 1072]]
- [[_COMMUNITY_Community 1073|Community 1073]]
- [[_COMMUNITY_Community 1074|Community 1074]]
- [[_COMMUNITY_Community 1075|Community 1075]]
- [[_COMMUNITY_Community 1076|Community 1076]]
- [[_COMMUNITY_Community 1077|Community 1077]]
- [[_COMMUNITY_Community 1078|Community 1078]]
- [[_COMMUNITY_Community 1079|Community 1079]]
- [[_COMMUNITY_Community 1080|Community 1080]]
- [[_COMMUNITY_Community 1081|Community 1081]]
- [[_COMMUNITY_Community 1082|Community 1082]]
- [[_COMMUNITY_Community 1083|Community 1083]]
- [[_COMMUNITY_Community 1084|Community 1084]]
- [[_COMMUNITY_Community 1085|Community 1085]]
- [[_COMMUNITY_Community 1086|Community 1086]]
- [[_COMMUNITY_Community 1087|Community 1087]]
- [[_COMMUNITY_Community 1088|Community 1088]]
- [[_COMMUNITY_Community 1089|Community 1089]]
- [[_COMMUNITY_Community 1090|Community 1090]]
- [[_COMMUNITY_Community 1091|Community 1091]]
- [[_COMMUNITY_Community 1092|Community 1092]]
- [[_COMMUNITY_Community 1093|Community 1093]]
- [[_COMMUNITY_Community 1094|Community 1094]]
- [[_COMMUNITY_Community 1095|Community 1095]]
- [[_COMMUNITY_Community 1096|Community 1096]]
- [[_COMMUNITY_Community 1097|Community 1097]]
- [[_COMMUNITY_Community 1098|Community 1098]]
- [[_COMMUNITY_Community 1099|Community 1099]]
- [[_COMMUNITY_Community 1100|Community 1100]]
- [[_COMMUNITY_Community 1101|Community 1101]]
- [[_COMMUNITY_Community 1102|Community 1102]]
- [[_COMMUNITY_Community 1103|Community 1103]]
- [[_COMMUNITY_Community 1104|Community 1104]]
- [[_COMMUNITY_Community 1105|Community 1105]]
- [[_COMMUNITY_Community 1106|Community 1106]]
- [[_COMMUNITY_Community 1107|Community 1107]]
- [[_COMMUNITY_Community 1108|Community 1108]]
- [[_COMMUNITY_Community 1109|Community 1109]]
- [[_COMMUNITY_Community 1110|Community 1110]]
- [[_COMMUNITY_Community 1111|Community 1111]]
- [[_COMMUNITY_Community 1112|Community 1112]]
- [[_COMMUNITY_Community 1113|Community 1113]]
- [[_COMMUNITY_Community 1114|Community 1114]]
- [[_COMMUNITY_Community 1115|Community 1115]]
- [[_COMMUNITY_Community 1116|Community 1116]]
- [[_COMMUNITY_Community 1117|Community 1117]]
- [[_COMMUNITY_Community 1118|Community 1118]]
- [[_COMMUNITY_Community 1119|Community 1119]]
- [[_COMMUNITY_Community 1120|Community 1120]]
- [[_COMMUNITY_Community 1121|Community 1121]]
- [[_COMMUNITY_Community 1122|Community 1122]]
- [[_COMMUNITY_Community 1123|Community 1123]]
- [[_COMMUNITY_Community 1124|Community 1124]]
- [[_COMMUNITY_Community 1125|Community 1125]]
- [[_COMMUNITY_Community 1126|Community 1126]]
- [[_COMMUNITY_Community 1127|Community 1127]]
- [[_COMMUNITY_Community 1128|Community 1128]]
- [[_COMMUNITY_Community 1129|Community 1129]]
- [[_COMMUNITY_Community 1130|Community 1130]]
- [[_COMMUNITY_Community 1131|Community 1131]]
- [[_COMMUNITY_Community 1132|Community 1132]]
- [[_COMMUNITY_Community 1133|Community 1133]]
- [[_COMMUNITY_Community 1134|Community 1134]]
- [[_COMMUNITY_Community 1135|Community 1135]]
- [[_COMMUNITY_Community 1136|Community 1136]]
- [[_COMMUNITY_Community 1137|Community 1137]]
- [[_COMMUNITY_Community 1138|Community 1138]]
- [[_COMMUNITY_Community 1139|Community 1139]]
- [[_COMMUNITY_Community 1140|Community 1140]]
- [[_COMMUNITY_Community 1141|Community 1141]]
- [[_COMMUNITY_Community 1142|Community 1142]]
- [[_COMMUNITY_Community 1143|Community 1143]]
- [[_COMMUNITY_Community 1144|Community 1144]]
- [[_COMMUNITY_Community 1145|Community 1145]]
- [[_COMMUNITY_Community 1146|Community 1146]]
- [[_COMMUNITY_Community 1147|Community 1147]]
- [[_COMMUNITY_Community 1148|Community 1148]]
- [[_COMMUNITY_Community 1149|Community 1149]]
- [[_COMMUNITY_Community 1150|Community 1150]]
- [[_COMMUNITY_Community 1151|Community 1151]]
- [[_COMMUNITY_Community 1152|Community 1152]]
- [[_COMMUNITY_Community 1153|Community 1153]]
- [[_COMMUNITY_Community 1154|Community 1154]]
- [[_COMMUNITY_Community 1155|Community 1155]]
- [[_COMMUNITY_Community 1156|Community 1156]]
- [[_COMMUNITY_Community 1157|Community 1157]]
- [[_COMMUNITY_Community 1158|Community 1158]]
- [[_COMMUNITY_Community 1159|Community 1159]]
- [[_COMMUNITY_Community 1160|Community 1160]]
- [[_COMMUNITY_Community 1161|Community 1161]]
- [[_COMMUNITY_Community 1162|Community 1162]]
- [[_COMMUNITY_Community 1163|Community 1163]]
- [[_COMMUNITY_Community 1164|Community 1164]]
- [[_COMMUNITY_Community 1165|Community 1165]]
- [[_COMMUNITY_Community 1166|Community 1166]]
- [[_COMMUNITY_Community 1167|Community 1167]]
- [[_COMMUNITY_Community 1168|Community 1168]]
- [[_COMMUNITY_Community 1169|Community 1169]]
- [[_COMMUNITY_Community 1170|Community 1170]]
- [[_COMMUNITY_Community 1171|Community 1171]]
- [[_COMMUNITY_Community 1172|Community 1172]]
- [[_COMMUNITY_Community 1173|Community 1173]]
- [[_COMMUNITY_Community 1174|Community 1174]]
- [[_COMMUNITY_Community 1175|Community 1175]]
- [[_COMMUNITY_Community 1176|Community 1176]]
- [[_COMMUNITY_Community 1177|Community 1177]]
- [[_COMMUNITY_Community 1178|Community 1178]]
- [[_COMMUNITY_Community 1179|Community 1179]]
- [[_COMMUNITY_Community 1180|Community 1180]]
- [[_COMMUNITY_Community 1181|Community 1181]]
- [[_COMMUNITY_Community 1182|Community 1182]]
- [[_COMMUNITY_Community 1183|Community 1183]]
- [[_COMMUNITY_Community 1184|Community 1184]]
- [[_COMMUNITY_Community 1185|Community 1185]]
- [[_COMMUNITY_Community 1186|Community 1186]]
- [[_COMMUNITY_Community 1187|Community 1187]]
- [[_COMMUNITY_Community 1188|Community 1188]]
- [[_COMMUNITY_Community 1189|Community 1189]]
- [[_COMMUNITY_Community 1190|Community 1190]]
- [[_COMMUNITY_Community 1191|Community 1191]]
- [[_COMMUNITY_Community 1192|Community 1192]]
- [[_COMMUNITY_Community 1193|Community 1193]]
- [[_COMMUNITY_Community 1194|Community 1194]]
- [[_COMMUNITY_Community 1195|Community 1195]]
- [[_COMMUNITY_Community 1196|Community 1196]]
- [[_COMMUNITY_Community 1197|Community 1197]]
- [[_COMMUNITY_Community 1198|Community 1198]]
- [[_COMMUNITY_Community 1199|Community 1199]]
- [[_COMMUNITY_Community 1200|Community 1200]]
- [[_COMMUNITY_Community 1201|Community 1201]]
- [[_COMMUNITY_Community 1202|Community 1202]]
- [[_COMMUNITY_Community 1203|Community 1203]]
- [[_COMMUNITY_Community 1204|Community 1204]]
- [[_COMMUNITY_Community 1205|Community 1205]]
- [[_COMMUNITY_Community 1206|Community 1206]]
- [[_COMMUNITY_Community 1207|Community 1207]]
- [[_COMMUNITY_Community 1208|Community 1208]]
- [[_COMMUNITY_Community 1209|Community 1209]]
- [[_COMMUNITY_Community 1210|Community 1210]]
- [[_COMMUNITY_Community 1211|Community 1211]]
- [[_COMMUNITY_Community 1212|Community 1212]]
- [[_COMMUNITY_Community 1213|Community 1213]]
- [[_COMMUNITY_Community 1214|Community 1214]]
- [[_COMMUNITY_Community 1215|Community 1215]]
- [[_COMMUNITY_Community 1216|Community 1216]]
- [[_COMMUNITY_Community 1217|Community 1217]]
- [[_COMMUNITY_Community 1218|Community 1218]]
- [[_COMMUNITY_Community 1219|Community 1219]]
- [[_COMMUNITY_Community 1220|Community 1220]]
- [[_COMMUNITY_Community 1221|Community 1221]]
- [[_COMMUNITY_Community 1222|Community 1222]]
- [[_COMMUNITY_Community 1223|Community 1223]]
- [[_COMMUNITY_Community 1224|Community 1224]]
- [[_COMMUNITY_Community 1225|Community 1225]]
- [[_COMMUNITY_Community 1226|Community 1226]]
- [[_COMMUNITY_Community 1227|Community 1227]]
- [[_COMMUNITY_Community 1228|Community 1228]]
- [[_COMMUNITY_Community 1229|Community 1229]]
- [[_COMMUNITY_Community 1230|Community 1230]]
- [[_COMMUNITY_Community 1231|Community 1231]]
- [[_COMMUNITY_Community 1232|Community 1232]]
- [[_COMMUNITY_Community 1233|Community 1233]]
- [[_COMMUNITY_Community 1234|Community 1234]]
- [[_COMMUNITY_Community 1235|Community 1235]]
- [[_COMMUNITY_Community 1236|Community 1236]]
- [[_COMMUNITY_Community 1237|Community 1237]]
- [[_COMMUNITY_Community 1238|Community 1238]]
- [[_COMMUNITY_Community 1239|Community 1239]]
- [[_COMMUNITY_Community 1240|Community 1240]]
- [[_COMMUNITY_Community 1241|Community 1241]]
- [[_COMMUNITY_Community 1242|Community 1242]]
- [[_COMMUNITY_Community 1243|Community 1243]]
- [[_COMMUNITY_Community 1244|Community 1244]]
- [[_COMMUNITY_Community 1245|Community 1245]]
- [[_COMMUNITY_Community 1246|Community 1246]]
- [[_COMMUNITY_Community 1247|Community 1247]]
- [[_COMMUNITY_Community 1248|Community 1248]]
- [[_COMMUNITY_Community 1249|Community 1249]]
- [[_COMMUNITY_Community 1250|Community 1250]]
- [[_COMMUNITY_Community 1251|Community 1251]]
- [[_COMMUNITY_Community 1252|Community 1252]]
- [[_COMMUNITY_Community 1253|Community 1253]]
- [[_COMMUNITY_Community 1254|Community 1254]]
- [[_COMMUNITY_Community 1255|Community 1255]]
- [[_COMMUNITY_Community 1256|Community 1256]]
- [[_COMMUNITY_Community 1257|Community 1257]]
- [[_COMMUNITY_Community 1258|Community 1258]]
- [[_COMMUNITY_Community 1259|Community 1259]]
- [[_COMMUNITY_Community 1260|Community 1260]]
- [[_COMMUNITY_Community 1261|Community 1261]]
- [[_COMMUNITY_Community 1262|Community 1262]]
- [[_COMMUNITY_Community 1263|Community 1263]]
- [[_COMMUNITY_Community 1264|Community 1264]]
- [[_COMMUNITY_Community 1265|Community 1265]]
- [[_COMMUNITY_Community 1266|Community 1266]]
- [[_COMMUNITY_Community 1267|Community 1267]]
- [[_COMMUNITY_Community 1268|Community 1268]]
- [[_COMMUNITY_Community 1269|Community 1269]]
- [[_COMMUNITY_Community 1270|Community 1270]]
- [[_COMMUNITY_Community 1271|Community 1271]]
- [[_COMMUNITY_Community 1272|Community 1272]]
- [[_COMMUNITY_Community 1273|Community 1273]]
- [[_COMMUNITY_Community 1274|Community 1274]]
- [[_COMMUNITY_Community 1275|Community 1275]]
- [[_COMMUNITY_Community 1276|Community 1276]]
- [[_COMMUNITY_Community 1277|Community 1277]]
- [[_COMMUNITY_Community 1278|Community 1278]]
- [[_COMMUNITY_Community 1279|Community 1279]]
- [[_COMMUNITY_Community 1280|Community 1280]]
- [[_COMMUNITY_Community 1281|Community 1281]]
- [[_COMMUNITY_Community 1282|Community 1282]]
- [[_COMMUNITY_Community 1283|Community 1283]]
- [[_COMMUNITY_Community 1284|Community 1284]]
- [[_COMMUNITY_Community 1285|Community 1285]]
- [[_COMMUNITY_Community 1286|Community 1286]]
- [[_COMMUNITY_Community 1287|Community 1287]]
- [[_COMMUNITY_Community 1288|Community 1288]]
- [[_COMMUNITY_Community 1289|Community 1289]]
- [[_COMMUNITY_Community 1290|Community 1290]]
- [[_COMMUNITY_Community 1291|Community 1291]]
- [[_COMMUNITY_Community 1292|Community 1292]]
- [[_COMMUNITY_Community 1293|Community 1293]]
- [[_COMMUNITY_Community 1294|Community 1294]]
- [[_COMMUNITY_Community 1295|Community 1295]]
- [[_COMMUNITY_Community 1296|Community 1296]]
- [[_COMMUNITY_Community 1297|Community 1297]]
- [[_COMMUNITY_Community 1298|Community 1298]]
- [[_COMMUNITY_Community 1299|Community 1299]]
- [[_COMMUNITY_Community 1300|Community 1300]]
- [[_COMMUNITY_Community 1301|Community 1301]]
- [[_COMMUNITY_Community 1302|Community 1302]]
- [[_COMMUNITY_Community 1303|Community 1303]]
- [[_COMMUNITY_Community 1304|Community 1304]]
- [[_COMMUNITY_Community 1305|Community 1305]]
- [[_COMMUNITY_Community 1306|Community 1306]]
- [[_COMMUNITY_Community 1307|Community 1307]]
- [[_COMMUNITY_Community 1308|Community 1308]]
- [[_COMMUNITY_Community 1309|Community 1309]]
- [[_COMMUNITY_Community 1310|Community 1310]]
- [[_COMMUNITY_Community 1311|Community 1311]]
- [[_COMMUNITY_Community 1312|Community 1312]]
- [[_COMMUNITY_Community 1313|Community 1313]]
- [[_COMMUNITY_Community 1314|Community 1314]]
- [[_COMMUNITY_Community 1315|Community 1315]]
- [[_COMMUNITY_Community 1316|Community 1316]]
- [[_COMMUNITY_Community 1317|Community 1317]]
- [[_COMMUNITY_Community 1318|Community 1318]]
- [[_COMMUNITY_Community 1319|Community 1319]]
- [[_COMMUNITY_Community 1320|Community 1320]]
- [[_COMMUNITY_Community 1321|Community 1321]]
- [[_COMMUNITY_Community 1322|Community 1322]]
- [[_COMMUNITY_Community 1323|Community 1323]]
- [[_COMMUNITY_Community 1324|Community 1324]]
- [[_COMMUNITY_Community 1325|Community 1325]]
- [[_COMMUNITY_Community 1326|Community 1326]]
- [[_COMMUNITY_Community 1327|Community 1327]]
- [[_COMMUNITY_Community 1328|Community 1328]]
- [[_COMMUNITY_Community 1329|Community 1329]]
- [[_COMMUNITY_Community 1330|Community 1330]]
- [[_COMMUNITY_Community 1331|Community 1331]]
- [[_COMMUNITY_Community 1332|Community 1332]]
- [[_COMMUNITY_Community 1333|Community 1333]]
- [[_COMMUNITY_Community 1334|Community 1334]]
- [[_COMMUNITY_Community 1335|Community 1335]]
- [[_COMMUNITY_Community 1336|Community 1336]]
- [[_COMMUNITY_Community 1337|Community 1337]]
- [[_COMMUNITY_Community 1338|Community 1338]]
- [[_COMMUNITY_Community 1339|Community 1339]]
- [[_COMMUNITY_Community 1340|Community 1340]]
- [[_COMMUNITY_Community 1341|Community 1341]]
- [[_COMMUNITY_Community 1342|Community 1342]]
- [[_COMMUNITY_Community 1343|Community 1343]]
- [[_COMMUNITY_Community 1344|Community 1344]]
- [[_COMMUNITY_Community 1345|Community 1345]]
- [[_COMMUNITY_Community 1346|Community 1346]]
- [[_COMMUNITY_Community 1347|Community 1347]]
- [[_COMMUNITY_Community 1348|Community 1348]]
- [[_COMMUNITY_Community 1349|Community 1349]]
- [[_COMMUNITY_Community 1350|Community 1350]]
- [[_COMMUNITY_Community 1351|Community 1351]]
- [[_COMMUNITY_Community 1352|Community 1352]]
- [[_COMMUNITY_Community 1353|Community 1353]]
- [[_COMMUNITY_Community 1354|Community 1354]]
- [[_COMMUNITY_Community 1355|Community 1355]]
- [[_COMMUNITY_Community 1356|Community 1356]]
- [[_COMMUNITY_Community 1357|Community 1357]]
- [[_COMMUNITY_Community 1358|Community 1358]]
- [[_COMMUNITY_Community 1359|Community 1359]]
- [[_COMMUNITY_Community 1360|Community 1360]]
- [[_COMMUNITY_Community 1361|Community 1361]]
- [[_COMMUNITY_Community 1362|Community 1362]]
- [[_COMMUNITY_Community 1363|Community 1363]]
- [[_COMMUNITY_Community 1364|Community 1364]]
- [[_COMMUNITY_Community 1365|Community 1365]]
- [[_COMMUNITY_Community 1366|Community 1366]]
- [[_COMMUNITY_Community 1367|Community 1367]]
- [[_COMMUNITY_Community 1368|Community 1368]]
- [[_COMMUNITY_Community 1369|Community 1369]]
- [[_COMMUNITY_Community 1370|Community 1370]]
- [[_COMMUNITY_Community 1371|Community 1371]]
- [[_COMMUNITY_Community 1372|Community 1372]]
- [[_COMMUNITY_Community 1373|Community 1373]]
- [[_COMMUNITY_Community 1374|Community 1374]]
- [[_COMMUNITY_Community 1375|Community 1375]]
- [[_COMMUNITY_Community 1376|Community 1376]]
- [[_COMMUNITY_Community 1377|Community 1377]]
- [[_COMMUNITY_Community 1378|Community 1378]]
- [[_COMMUNITY_Community 1379|Community 1379]]
- [[_COMMUNITY_Community 1380|Community 1380]]
- [[_COMMUNITY_Community 1381|Community 1381]]
- [[_COMMUNITY_Community 1382|Community 1382]]
- [[_COMMUNITY_Community 1383|Community 1383]]
- [[_COMMUNITY_Community 1384|Community 1384]]
- [[_COMMUNITY_Community 1385|Community 1385]]
- [[_COMMUNITY_Community 1386|Community 1386]]
- [[_COMMUNITY_Community 1387|Community 1387]]
- [[_COMMUNITY_Community 1388|Community 1388]]
- [[_COMMUNITY_Community 1389|Community 1389]]
- [[_COMMUNITY_Community 1390|Community 1390]]
- [[_COMMUNITY_Community 1391|Community 1391]]
- [[_COMMUNITY_Community 1392|Community 1392]]
- [[_COMMUNITY_Community 1393|Community 1393]]
- [[_COMMUNITY_Community 1394|Community 1394]]
- [[_COMMUNITY_Community 1395|Community 1395]]
- [[_COMMUNITY_Community 1396|Community 1396]]
- [[_COMMUNITY_Community 1397|Community 1397]]
- [[_COMMUNITY_Community 1398|Community 1398]]
- [[_COMMUNITY_Community 1399|Community 1399]]
- [[_COMMUNITY_Community 1400|Community 1400]]
- [[_COMMUNITY_Community 1401|Community 1401]]
- [[_COMMUNITY_Community 1402|Community 1402]]
- [[_COMMUNITY_Community 1403|Community 1403]]
- [[_COMMUNITY_Community 1404|Community 1404]]
- [[_COMMUNITY_Community 1405|Community 1405]]
- [[_COMMUNITY_Community 1406|Community 1406]]
- [[_COMMUNITY_Community 1407|Community 1407]]
- [[_COMMUNITY_Community 1408|Community 1408]]
- [[_COMMUNITY_Community 1409|Community 1409]]
- [[_COMMUNITY_Community 1410|Community 1410]]
- [[_COMMUNITY_Community 1411|Community 1411]]
- [[_COMMUNITY_Community 1412|Community 1412]]
- [[_COMMUNITY_Community 1413|Community 1413]]
- [[_COMMUNITY_Community 1414|Community 1414]]
- [[_COMMUNITY_Community 1415|Community 1415]]
- [[_COMMUNITY_Community 1416|Community 1416]]
- [[_COMMUNITY_Community 1417|Community 1417]]
- [[_COMMUNITY_Community 1418|Community 1418]]
- [[_COMMUNITY_Community 1419|Community 1419]]
- [[_COMMUNITY_Community 1420|Community 1420]]
- [[_COMMUNITY_Community 1421|Community 1421]]
- [[_COMMUNITY_Community 1422|Community 1422]]
- [[_COMMUNITY_Community 1423|Community 1423]]
- [[_COMMUNITY_Community 1424|Community 1424]]
- [[_COMMUNITY_Community 1425|Community 1425]]
- [[_COMMUNITY_Community 1426|Community 1426]]
- [[_COMMUNITY_Community 1427|Community 1427]]
- [[_COMMUNITY_Community 1428|Community 1428]]
- [[_COMMUNITY_Community 1429|Community 1429]]
- [[_COMMUNITY_Community 1430|Community 1430]]
- [[_COMMUNITY_Community 1431|Community 1431]]
- [[_COMMUNITY_Community 1432|Community 1432]]
- [[_COMMUNITY_Community 1433|Community 1433]]
- [[_COMMUNITY_Community 1434|Community 1434]]
- [[_COMMUNITY_Community 1435|Community 1435]]
- [[_COMMUNITY_Community 1436|Community 1436]]
- [[_COMMUNITY_Community 1437|Community 1437]]
- [[_COMMUNITY_Community 1438|Community 1438]]
- [[_COMMUNITY_Community 1439|Community 1439]]
- [[_COMMUNITY_Community 1440|Community 1440]]
- [[_COMMUNITY_Community 1441|Community 1441]]
- [[_COMMUNITY_Community 1442|Community 1442]]
- [[_COMMUNITY_Community 1443|Community 1443]]
- [[_COMMUNITY_Community 1444|Community 1444]]
- [[_COMMUNITY_Community 1445|Community 1445]]
- [[_COMMUNITY_Community 1446|Community 1446]]
- [[_COMMUNITY_Community 1447|Community 1447]]
- [[_COMMUNITY_Community 1448|Community 1448]]
- [[_COMMUNITY_Community 1449|Community 1449]]
- [[_COMMUNITY_Community 1450|Community 1450]]
- [[_COMMUNITY_Community 1451|Community 1451]]
- [[_COMMUNITY_Community 1452|Community 1452]]
- [[_COMMUNITY_Community 1453|Community 1453]]
- [[_COMMUNITY_Community 1454|Community 1454]]
- [[_COMMUNITY_Community 1455|Community 1455]]
- [[_COMMUNITY_Community 1456|Community 1456]]
- [[_COMMUNITY_Community 1457|Community 1457]]
- [[_COMMUNITY_Community 1458|Community 1458]]
- [[_COMMUNITY_Community 1459|Community 1459]]
- [[_COMMUNITY_Community 1460|Community 1460]]
- [[_COMMUNITY_Community 1461|Community 1461]]
- [[_COMMUNITY_Community 1462|Community 1462]]
- [[_COMMUNITY_Community 1463|Community 1463]]
- [[_COMMUNITY_Community 1464|Community 1464]]
- [[_COMMUNITY_Community 1465|Community 1465]]
- [[_COMMUNITY_Community 1466|Community 1466]]
- [[_COMMUNITY_Community 1467|Community 1467]]
- [[_COMMUNITY_Community 1468|Community 1468]]
- [[_COMMUNITY_Community 1469|Community 1469]]
- [[_COMMUNITY_Community 1470|Community 1470]]
- [[_COMMUNITY_Community 1471|Community 1471]]
- [[_COMMUNITY_Community 1472|Community 1472]]
- [[_COMMUNITY_Community 1473|Community 1473]]
- [[_COMMUNITY_Community 1474|Community 1474]]
- [[_COMMUNITY_Community 1475|Community 1475]]
- [[_COMMUNITY_Community 1476|Community 1476]]
- [[_COMMUNITY_Community 1477|Community 1477]]
- [[_COMMUNITY_Community 1478|Community 1478]]
- [[_COMMUNITY_Community 1479|Community 1479]]
- [[_COMMUNITY_Community 1480|Community 1480]]
- [[_COMMUNITY_Community 1481|Community 1481]]
- [[_COMMUNITY_Community 1482|Community 1482]]
- [[_COMMUNITY_Community 1483|Community 1483]]
- [[_COMMUNITY_Community 1484|Community 1484]]
- [[_COMMUNITY_Community 1485|Community 1485]]
- [[_COMMUNITY_Community 1486|Community 1486]]
- [[_COMMUNITY_Community 1487|Community 1487]]
- [[_COMMUNITY_Community 1488|Community 1488]]
- [[_COMMUNITY_Community 1489|Community 1489]]
- [[_COMMUNITY_Community 1490|Community 1490]]
- [[_COMMUNITY_Community 1491|Community 1491]]
- [[_COMMUNITY_Community 1492|Community 1492]]
- [[_COMMUNITY_Community 1493|Community 1493]]
- [[_COMMUNITY_Community 1494|Community 1494]]
- [[_COMMUNITY_Community 1495|Community 1495]]
- [[_COMMUNITY_Community 1496|Community 1496]]
- [[_COMMUNITY_Community 1497|Community 1497]]
- [[_COMMUNITY_Community 1498|Community 1498]]
- [[_COMMUNITY_Community 1499|Community 1499]]
- [[_COMMUNITY_Community 1500|Community 1500]]
- [[_COMMUNITY_Community 1501|Community 1501]]
- [[_COMMUNITY_Community 1502|Community 1502]]
- [[_COMMUNITY_Community 1503|Community 1503]]
- [[_COMMUNITY_Community 1504|Community 1504]]
- [[_COMMUNITY_Community 1505|Community 1505]]
- [[_COMMUNITY_Community 1506|Community 1506]]
- [[_COMMUNITY_Community 1507|Community 1507]]
- [[_COMMUNITY_Community 1508|Community 1508]]
- [[_COMMUNITY_Community 1509|Community 1509]]
- [[_COMMUNITY_Community 1510|Community 1510]]
- [[_COMMUNITY_Community 1511|Community 1511]]
- [[_COMMUNITY_Community 1512|Community 1512]]
- [[_COMMUNITY_Community 1513|Community 1513]]
- [[_COMMUNITY_Community 1514|Community 1514]]
- [[_COMMUNITY_Community 1515|Community 1515]]
- [[_COMMUNITY_Community 1516|Community 1516]]
- [[_COMMUNITY_Community 1517|Community 1517]]
- [[_COMMUNITY_Community 1518|Community 1518]]
- [[_COMMUNITY_Community 1519|Community 1519]]
- [[_COMMUNITY_Community 1520|Community 1520]]
- [[_COMMUNITY_Community 1521|Community 1521]]
- [[_COMMUNITY_Community 1522|Community 1522]]
- [[_COMMUNITY_Community 1523|Community 1523]]
- [[_COMMUNITY_Community 1524|Community 1524]]
- [[_COMMUNITY_Community 1525|Community 1525]]
- [[_COMMUNITY_Community 1526|Community 1526]]
- [[_COMMUNITY_Community 1527|Community 1527]]
- [[_COMMUNITY_Community 1528|Community 1528]]
- [[_COMMUNITY_Community 1529|Community 1529]]
- [[_COMMUNITY_Community 1530|Community 1530]]
- [[_COMMUNITY_Community 1531|Community 1531]]
- [[_COMMUNITY_Community 1532|Community 1532]]
- [[_COMMUNITY_Community 1533|Community 1533]]
- [[_COMMUNITY_Community 1534|Community 1534]]
- [[_COMMUNITY_Community 1535|Community 1535]]
- [[_COMMUNITY_Community 1536|Community 1536]]
- [[_COMMUNITY_Community 1537|Community 1537]]
- [[_COMMUNITY_Community 1538|Community 1538]]
- [[_COMMUNITY_Community 1539|Community 1539]]
- [[_COMMUNITY_Community 1540|Community 1540]]
- [[_COMMUNITY_Community 1541|Community 1541]]
- [[_COMMUNITY_Community 1542|Community 1542]]
- [[_COMMUNITY_Community 1543|Community 1543]]
- [[_COMMUNITY_Community 1544|Community 1544]]
- [[_COMMUNITY_Community 1545|Community 1545]]
- [[_COMMUNITY_Community 1546|Community 1546]]
- [[_COMMUNITY_Community 1547|Community 1547]]
- [[_COMMUNITY_Community 1548|Community 1548]]
- [[_COMMUNITY_Community 1549|Community 1549]]
- [[_COMMUNITY_Community 1550|Community 1550]]
- [[_COMMUNITY_Community 1551|Community 1551]]
- [[_COMMUNITY_Community 1552|Community 1552]]
- [[_COMMUNITY_Community 1553|Community 1553]]
- [[_COMMUNITY_Community 1554|Community 1554]]
- [[_COMMUNITY_Community 1555|Community 1555]]
- [[_COMMUNITY_Community 1556|Community 1556]]
- [[_COMMUNITY_Community 1557|Community 1557]]
- [[_COMMUNITY_Community 1558|Community 1558]]
- [[_COMMUNITY_Community 1559|Community 1559]]
- [[_COMMUNITY_Community 1560|Community 1560]]
- [[_COMMUNITY_Community 1561|Community 1561]]
- [[_COMMUNITY_Community 1562|Community 1562]]
- [[_COMMUNITY_Community 1563|Community 1563]]
- [[_COMMUNITY_Community 1564|Community 1564]]
- [[_COMMUNITY_Community 1565|Community 1565]]
- [[_COMMUNITY_Community 1566|Community 1566]]
- [[_COMMUNITY_Community 1567|Community 1567]]
- [[_COMMUNITY_Community 1568|Community 1568]]
- [[_COMMUNITY_Community 1569|Community 1569]]
- [[_COMMUNITY_Community 1570|Community 1570]]
- [[_COMMUNITY_Community 1571|Community 1571]]
- [[_COMMUNITY_Community 1572|Community 1572]]
- [[_COMMUNITY_Community 1573|Community 1573]]
- [[_COMMUNITY_Community 1574|Community 1574]]
- [[_COMMUNITY_Community 1575|Community 1575]]
- [[_COMMUNITY_Community 1576|Community 1576]]
- [[_COMMUNITY_Community 1577|Community 1577]]
- [[_COMMUNITY_Community 1578|Community 1578]]
- [[_COMMUNITY_Community 1579|Community 1579]]
- [[_COMMUNITY_Community 1580|Community 1580]]
- [[_COMMUNITY_Community 1581|Community 1581]]
- [[_COMMUNITY_Community 1582|Community 1582]]
- [[_COMMUNITY_Community 1583|Community 1583]]
- [[_COMMUNITY_Community 1584|Community 1584]]
- [[_COMMUNITY_Community 1585|Community 1585]]
- [[_COMMUNITY_Community 1586|Community 1586]]
- [[_COMMUNITY_Community 1587|Community 1587]]
- [[_COMMUNITY_Community 1588|Community 1588]]
- [[_COMMUNITY_Community 1589|Community 1589]]

## God Nodes (most connected - your core abstractions)
1. `AuthenticationService` - 56 edges
2. `Repository` - 54 edges
3. `IRepository` - 38 edges
4. `EntityAdminService` - 30 edges
5. `AuthController` - 24 edges
6. `IAuthenticationService` - 23 edges
7. `EpisodicMemoryService` - 21 edges
8. `string` - 20 edges
9. `UserService` - 20 edges
10. `OperationCatalogService` - 17 edges

## Surprising Connections (you probably didn't know these)
- `AdminPanelSecuritySettings` --references--> `string`  [EXTRACTED]
  obsidian-graph/Infrastructure/AdminPanelSecuritySettings.md → WebFramwork/Swagger/UnauthorizedResponsesOperationFilter.cs
- `Repository` --references--> `int`  [EXTRACTED]
  obsidian-graph/Repositories/Repository.md → WebFramwork/Infrastructure/Caching/DistributedAppCacheService.cs
- `Repository` --inherits--> `IRepository`  [EXTRACTED]
  obsidian-graph/Repositories/Repository.md → Services/Services/Repositories/ServiceRepository.cs
- `Repository` --references--> `DbContext`  [EXTRACTED]
  obsidian-graph/Repositories/Repository.md → Services/Services/Repositories/Repository.cs
- `RegularExpression` --references--> `string`  [EXTRACTED]
  Common/Utilities/Helpers/Assert.cs → WebFramwork/Swagger/UnauthorizedResponsesOperationFilter.cs

## Communities (1650 total, 109 thin omitted)

### Community 0 - "Community 0"
Cohesion: 0.06
Nodes (7): DbContext, Dependencies, Downstream Relationships, Inherits, Note, Repository, Upstream Relationships

### Community 1 - "Community 1"
Cohesion: 0.11
Nodes (3): AuthenticationService, IAuthenticationService, IHttpContextAccessor

### Community 3 - "Community 3"
Cohesion: 0.06
Nodes (15): AccountUserPermissionRevokeConfiguration, ConfirmationCodeConfiguration, EmailConfiguration, EmailDocumentConfiguration, EmailSharedInformationConfiguration, RoleConfiguration, UserConfiguration, UserRoleConfiguration (+7 more)

### Community 4 - "Community 4"
Cohesion: 0.14
Nodes (33): BaseEntity, CrmCustomer, CrmCustomerFinancialSummary, CrmCustomerInterest, CrmCustomerLedger, CrmCustomerLoyalty, CrmCustomerNote, CrmDiscountCampaign (+25 more)

### Community 5 - "Community 5"
Cohesion: 0.07
Nodes (5): BaseUserApiController, API.Controllers.Auth.v1, AuthController, API.Controllers.Users.v1, UsersController

### Community 6 - "Community 6"
Cohesion: 0.07
Nodes (29): Agent Output Format (for PRs/Task reports), Business Requirements Source, Change Safety Rules, code:bash (python -m graphify update .), Done Criteria (Definition of Done), Example A: Add a new use-case (feature), Example B: Add a new entity field persisted in DB, Example C: Authentication/Authorization changes (+21 more)

### Community 7 - "Community 7"
Cohesion: 0.15
Nodes (4): ApplicationDbContext, EntityAdminService, MethodInfo, Type

### Community 8 - "Community 8"
Cohesion: 0.13
Nodes (21): AbstractValidator, CompletePhoneRegistrationRequestValidator, ConfirmEmailRegistrationRequestValidator, EmailPasswordLoginRequestValidator, ForgotPasswordRequestValidator, PhoneOtpLoginRequestValidator, RegisterWithEmailRequestValidator, ResendEmailActivationLinkRequestValidator (+13 more)

### Community 10 - "Community 10"
Cohesion: 0.1
Nodes (9): RoleViewDTO, BaseDTO, BoolResultDto, IBaseDTO, IHaveCustomMapping, EmailReceiverDTO, SentEmailDTO, Services.DTOs.Users (+1 more)

### Community 11 - "Community 11"
Cohesion: 0.1
Nodes (4): BaseAdminApiController, AgentToolsController, AuthorizationAdminController, EpisodesController

### Community 12 - "Community 12"
Cohesion: 0.1
Nodes (9): Migration, Data.Migrations, Init, Data.Migrations, Init, AddEpisodicMemory, Data.Migrations, Data.Migrations (+1 more)

### Community 14 - "Community 14"
Cohesion: 0.11
Nodes (7): Func, AssemblyHelper, List, PropertyValidator, NationalCodeValidator, PasswordValidator, PhoneNumberValidator

### Community 15 - "Community 15"
Cohesion: 0.14
Nodes (6): Error(), IOperationCatalogService, OperationCatalogService, IReadOnlyList, IServiceProvider, OperationCatalogSecuritySettings

### Community 16 - "Community 16"
Cohesion: 0.12
Nodes (15): EpisodeDto, EpisodeListItemDto, EpisodeReferenceDto, EpisodeReferenceInput, EpisodeSearchEvaluationDto, EpisodeSearchEvaluationResultDto, EvaluateEpisodeSearchRequest, GetImportantEpisodesRequest (+7 more)

### Community 17 - "Community 17"
Cohesion: 0.12
Nodes (8): AgentToolingService, UserContext, IDataInitializer, EpisodicMemoryAutomationService, IAgentToolingService, IEpisodicMemoryAutomationService, IScopedDependency, IUserContext

### Community 18 - "Community 18"
Cohesion: 0.16
Nodes (5): ConcurrentDictionary, DbCommandInterceptor, SimpleRateLimitMiddleware, SlowQueryLoggingInterceptor, QueryTuningSettings

### Community 20 - "Community 20"
Cohesion: 0.12
Nodes (11): AccountPermission, AccountPermissionConfiguration, AccountPlanPermission, AccountPlanPermissionConfiguration, AccountRolePermission, AccountRolePermissionConfiguration, AccountUserPermissionGrant, AccountUserPermissionGrantConfiguration (+3 more)

### Community 21 - "Community 21"
Cohesion: 0.15
Nodes (6): ILogger, ISenderService, IWebHostEnvironment, CustomExceptionHandlerMiddleware, SenderService, RequestDelegate

### Community 22 - "Community 22"
Cohesion: 0.12
Nodes (7): IRoleService, IUserService, RoleService, IMapper, IRepository, IRoleService, Repository

### Community 23 - "Community 23"
Cohesion: 0.12
Nodes (9): AuthorizationCatalog, ApplicationMessages, ApplicationPropertyPersianName, DataInitializerDefaultValues, RegexHelper, RequestContextEnrichmentMiddleware, string, Entities.Users (+1 more)

### Community 25 - "Community 25"
Cohesion: 0.12
Nodes (10): AccountOrganizationSecuritySetting, AccountOrganizationSecuritySettingConfiguration, AccountSecurityFeature, AccountSecurityFeatureConfiguration, AccountUserOauthIdentity, AccountUserOauthIdentityConfiguration, AccountUserSecuritySetting, AccountUserSecuritySettingConfiguration (+2 more)

### Community 26 - "Community 26"
Cohesion: 0.16
Nodes (6): AuthorizationDataInitializer, RoleDataInitializer, UserDataInitializer, IDataInitializer, RoleManager, UserManager

### Community 27 - "Community 27"
Cohesion: 0.23
Nodes (3): UserService, CachingSettings, IUserService

### Community 28 - "Community 28"
Cohesion: 0.15
Nodes (5): DeleteModel, IndexModel, IEntityAdminService, AdminPanel.Pages, IndexModel

### Community 31 - "Community 31"
Cohesion: 0.15
Nodes (9): AdminPanelAuthenticationSettings, AdminPanelSecuritySettings, Contains, Dependencies, Downstream Relationships, Note, OperationCatalogSecuritySettings, References (+1 more)

### Community 32 - "Community 32"
Cohesion: 0.18
Nodes (7): AdminPanelServiceCollectionExtensions, Contains, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 33 - "Community 33"
Cohesion: 0.17
Nodes (6): LogoutModel, PageModel, AdminPanel.Pages, ErrorModel, AdminPanel.Pages, PrivacyModel

### Community 36 - "Community 36"
Cohesion: 0.18
Nodes (3): CookieManager, SessionManager, Dictionary

### Community 37 - "Community 37"
Cohesion: 0.2
Nodes (4): ActionFilterAttribute, ApiCustomAuthorize, ApiResultFilterAttribute, FluentValidationFilterAttribute

### Community 39 - "Community 39"
Cohesion: 0.17
Nodes (11): Definition of Done, Skillهای ریپویی, Workflow انجام تسک, دستورالعمل‌های سلسله‌مراتبی, سیاست کیفیت و Warning, فرمان‌های پیشنهادی برای بررسی محلی, قوانین وابستگی لایه‌ها (غیرقابل مذاکره), قوانین پیاده‌سازی (+3 more)

### Community 40 - "Community 40"
Cohesion: 0.17
Nodes (11): Actors and Permissions, AUTH Business Spec, Business Rules, Core Use Cases, Data Contracts, Events and Side Effects, Non-Functional Requirements, Open Questions (+3 more)

### Community 42 - "Community 42"
Cohesion: 0.22
Nodes (3): CrudController, Ok(), IServiceRepository

### Community 43 - "Community 43"
Cohesion: 0.27
Nodes (6): Add-UniqueName(), Escape-YamlValue(), Get-NodeFolder(), New-NoteContent(), Normalize-PathValue(), To-WikiCsv()

### Community 45 - "Community 45"
Cohesion: 0.18
Nodes (4): Assert, CodeGenerator, RegularExpression, Random

### Community 46 - "Community 46"
Cohesion: 0.18
Nodes (5): bool, IOperationFilter, ApplySummariesOperationFilter, RemoveVersionParameters, UnauthorizedResponsesOperationFilter

### Community 47 - "Community 47"
Cohesion: 0.18
Nodes (10): Actors and Permissions, Business Rules, Business Spec, Core Use Cases, Data Contracts, Events and Side Effects, Non-Functional Requirements, Open Questions (+2 more)

### Community 48 - "Community 48"
Cohesion: 0.18
Nodes (10): Actors and Permissions, Business Rules, Business Spec, Core Use Cases, Data Contracts, Events and Side Effects, Non-Functional Requirements, Open Questions (+2 more)

### Community 49 - "Community 49"
Cohesion: 0.18
Nodes (10): Actors and Permissions, Business Rules, Business Spec, Core Use Cases, Data Contracts, Events and Side Effects, Non-Functional Requirements, Open Questions (+2 more)

### Community 50 - "Community 50"
Cohesion: 0.18
Nodes (10): Actors and Permissions, Business Rules, Business Spec, Core Use Cases, Data Contracts, Events and Side Effects, Non-Functional Requirements, Open Questions (+2 more)

### Community 51 - "Community 51"
Cohesion: 0.18
Nodes (10): Actors and Permissions, Business Rules, Business Spec, Core Use Cases, Data Contracts, Events and Side Effects, Non-Functional Requirements, Open Questions (+2 more)

### Community 52 - "Community 52"
Cohesion: 0.18
Nodes (10): Actors and Permissions, Business Rules, Business Spec, Core Use Cases, Data Contracts, Events and Side Effects, Non-Functional Requirements, Open Questions (+2 more)

### Community 53 - "Community 53"
Cohesion: 0.18
Nodes (10): Actors and Permissions, Business Rules, Business Spec, Core Use Cases, Data Contracts, Events and Side Effects, Non-Functional Requirements, Open Questions (+2 more)

### Community 54 - "Community 54"
Cohesion: 0.18
Nodes (10): Episodic Memory Playbook, Hybrid Search سبک, workflow پیشنهادی agent, اتوماسیون فعلی, الگوهای پیشنهادی tag, جستجو قبل از اقدام, قواعد کیفیت episode, مراحل ثبت episode (+2 more)

### Community 57 - "Community 57"
Cohesion: 0.2
Nodes (9): Contains, CrudController, Dependencies, Downstream Relationships, Inherits, Method, Note, References (+1 more)

### Community 58 - "Community 58"
Cohesion: 0.2
Nodes (9): Contains, DeleteModel, Dependencies, Downstream Relationships, Inherits, Method, Note, References (+1 more)

### Community 59 - "Community 59"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, EditModel, Inherits, Method, Note, References (+1 more)

### Community 60 - "Community 60"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, IndexModel (2), Inherits, Method, Note, References (+1 more)

### Community 61 - "Community 61"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, Inherits, ListModel, Method, Note, References (+1 more)

### Community 62 - "Community 62"
Cohesion: 0.2
Nodes (9): ApiCustomAuthorize, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, References (+1 more)

### Community 63 - "Community 63"
Cohesion: 0.2
Nodes (9): ApiPermissionAuthorizeAttribute, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, References (+1 more)

### Community 64 - "Community 64"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, IndexModel (3), Inherits, Method, Note, References (+1 more)

### Community 65 - "Community 65"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, IndexModel, Inherits, Method, Note, References (+1 more)

### Community 66 - "Community 66"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, Inherits, InvokeModel, Method, Note, References (+1 more)

### Community 67 - "Community 67"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, PasswordValidator, References (+1 more)

### Community 68 - "Community 68"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, References, RoleDataInitializer (+1 more)

### Community 69 - "Community 69"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, References, SlowQueryLoggingInterceptor (+1 more)

### Community 70 - "Community 70"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, References, UnauthorizedResponsesOperationFilter (+1 more)

### Community 71 - "Community 71"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, References, Upstream Relationships (+1 more)

### Community 72 - "Community 72"
Cohesion: 0.2
Nodes (9): AdminPanelAuthService, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, References (+1 more)

### Community 73 - "Community 73"
Cohesion: 0.2
Nodes (9): AuthenticationService, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, References (+1 more)

### Community 74 - "Community 74"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, DistributedAppCacheService, Downstream Relationships, Inherits, Method, Note, References (+1 more)

### Community 75 - "Community 75"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, EffectiveAuthorizationService, Inherits, Method, Note, References (+1 more)

### Community 76 - "Community 76"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, EntityAdminService, Inherits, Method, Note, References (+1 more)

### Community 77 - "Community 77"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, Inherits, JwtService, Method, Note, References (+1 more)

### Community 78 - "Community 78"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, OperationCatalogService, References (+1 more)

### Community 79 - "Community 79"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, References, Repository (2) (+1 more)

### Community 80 - "Community 80"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, References, RoleService (+1 more)

### Community 81 - "Community 81"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, References, SenderService (+1 more)

### Community 82 - "Community 82"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, References, ServiceRepository (+1 more)

### Community 83 - "Community 83"
Cohesion: 0.2
Nodes (9): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, References, Upstream Relationships (+1 more)

### Community 84 - "Community 84"
Cohesion: 0.28
Nodes (3): AdminPanelAuthenticationSettings, AdminPanelAuthService, IAdminPanelAuthService

### Community 85 - "Community 85"
Cohesion: 0.25
Nodes (3): ApplicationDefaultSettings, ListModel, int

### Community 86 - "Community 86"
Cohesion: 0.22
Nodes (3): IOperationCatalogService, IndexModel, InvokeModel

### Community 88 - "Community 88"
Cohesion: 0.22
Nodes (5): BaseAdminApiController, BaseCommonController, BaseThirdPartyController, BaseUserApiController, ControllerBase

### Community 89 - "Community 89"
Cohesion: 0.42
Nodes (3): DistributedAppCacheService, JsonSerializerOptions, long

### Community 90 - "Community 90"
Cohesion: 0.22
Nodes (8): AuthController, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 91 - "Community 91"
Cohesion: 0.22
Nodes (8): AuthorizationAdminController, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 92 - "Community 92"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships, UsersController

### Community 93 - "Community 93"
Cohesion: 0.22
Nodes (8): AccountMembershipConfiguration, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 94 - "Community 94"
Cohesion: 0.22
Nodes (8): AccountMembershipGroupConfiguration, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 95 - "Community 95"
Cohesion: 0.22
Nodes (8): AccountMembershipPermissionConfiguration, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 96 - "Community 96"
Cohesion: 0.22
Nodes (8): AccountOrganizationConfiguration, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 97 - "Community 97"
Cohesion: 0.22
Nodes (8): AccountOrganizationSecuritySettingConfiguration, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 98 - "Community 98"
Cohesion: 0.22
Nodes (8): AccountPermissionConfiguration, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 99 - "Community 99"
Cohesion: 0.22
Nodes (8): AccountPlanPermissionConfiguration, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 100 - "Community 100"
Cohesion: 0.22
Nodes (8): AccountRolePermissionConfiguration, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 101 - "Community 101"
Cohesion: 0.22
Nodes (8): AccountSecurityFeatureConfiguration, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 102 - "Community 102"
Cohesion: 0.22
Nodes (8): AccountUserOauthIdentityConfiguration, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 103 - "Community 103"
Cohesion: 0.22
Nodes (8): AccountUserPermissionGrantConfiguration, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 104 - "Community 104"
Cohesion: 0.22
Nodes (8): AccountUserPermissionRevokeConfiguration, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 105 - "Community 105"
Cohesion: 0.22
Nodes (8): AccountUserPlanSubscriptionConfiguration, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 106 - "Community 106"
Cohesion: 0.22
Nodes (8): AccountUserSecuritySettingConfiguration, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 107 - "Community 107"
Cohesion: 0.22
Nodes (8): AccountUserSessionConfiguration, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 108 - "Community 108"
Cohesion: 0.22
Nodes (8): AccountUserTotpFactorConfiguration, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 109 - "Community 109"
Cohesion: 0.22
Nodes (8): AccountUserVerificationConfiguration, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 110 - "Community 110"
Cohesion: 0.22
Nodes (8): CategoryConfiguration, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 111 - "Community 111"
Cohesion: 0.22
Nodes (8): ConfirmationCodeConfiguration, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 112 - "Community 112"
Cohesion: 0.22
Nodes (8): Contains, CoreCountyConfiguration, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 113 - "Community 113"
Cohesion: 0.22
Nodes (8): Contains, CoreProvinceConfiguration, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 114 - "Community 114"
Cohesion: 0.22
Nodes (8): Contains, CrmCommonConfiguration, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 115 - "Community 115"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, EmailConfiguration, Inherits, Method, Note, Upstream Relationships

### Community 116 - "Community 116"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, EmailDocumentConfiguration, Inherits, Method, Note, Upstream Relationships

### Community 117 - "Community 117"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, EmailSharedInformationConfiguration, Inherits, Method, Note, Upstream Relationships

### Community 118 - "Community 118"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, LicenseBillingCycleConfiguration, Method, Note, Upstream Relationships

### Community 119 - "Community 119"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, LicenseModuleConfiguration, Method, Note, Upstream Relationships

### Community 120 - "Community 120"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, LicenseModulePermissionConfiguration, Method, Note, Upstream Relationships

### Community 121 - "Community 121"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, LicensePlanConfiguration, Method, Note, Upstream Relationships

### Community 122 - "Community 122"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, LicensePlanModuleConfiguration, Method, Note, Upstream Relationships

### Community 123 - "Community 123"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, LicensePlanPriceConfiguration, Method, Note, Upstream Relationships

### Community 124 - "Community 124"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, LicenseSubscriptionConfiguration, Method, Note, Upstream Relationships

### Community 125 - "Community 125"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, MenuExtraConfiguration, Method, Note, Upstream Relationships

### Community 126 - "Community 126"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, MenuUnitConfiguration, Method, Note, Upstream Relationships

### Community 127 - "Community 127"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, PaymentOperationConfiguration, Upstream Relationships

### Community 128 - "Community 128"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, PaymentWalletConfiguration, Upstream Relationships

### Community 129 - "Community 129"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, PaymentWalletEntryConfiguration, Upstream Relationships

### Community 130 - "Community 130"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, PostConfiguration, Upstream Relationships

### Community 131 - "Community 131"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, RoleConfiguration, Upstream Relationships

### Community 132 - "Community 132"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, SupportConfiguration, Upstream Relationships

### Community 133 - "Community 133"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships, UserConfiguration

### Community 134 - "Community 134"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships, UserRoleConfiguration

### Community 135 - "Community 135"
Cohesion: 0.22
Nodes (8): ApiResultFilterAttribute, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 136 - "Community 136"
Cohesion: 0.22
Nodes (8): ApplicationDbContext (2), Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 137 - "Community 137"
Cohesion: 0.22
Nodes (8): ApplicationDbContextModelSnapshot, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 138 - "Community 138"
Cohesion: 0.22
Nodes (8): ApplySummariesOperationFilter, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 139 - "Community 139"
Cohesion: 0.22
Nodes (8): AppTelemetry, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 140 - "Community 140"
Cohesion: 0.22
Nodes (8): AssemblyHelper.FindMyAssemblies(), Calls, Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 141 - "Community 141"
Cohesion: 0.22
Nodes (8): AssemblyHelper, Contains, Dependencies, Downstream Relationships, Method, Note, References, Upstream Relationships

### Community 142 - "Community 142"
Cohesion: 0.22
Nodes (8): AuthorizationDataInitializer, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 143 - "Community 143"
Cohesion: 0.22
Nodes (8): AutofacModule, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 144 - "Community 144"
Cohesion: 0.22
Nodes (8): BaseDTO, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 145 - "Community 145"
Cohesion: 0.22
Nodes (8): CodeGenerator, Contains, Dependencies, Downstream Relationships, Method, Note, References, Upstream Relationships

### Community 146 - "Community 146"
Cohesion: 0.22
Nodes (8): Contains, CookieManager, Dependencies, Downstream Relationships, Method, Note, References, Upstream Relationships

### Community 147 - "Community 147"
Cohesion: 0.22
Nodes (8): Contains, CustomExceptionHandlerMiddleware, Dependencies, Downstream Relationships, Method, Note, References, Upstream Relationships

### Community 148 - "Community 148"
Cohesion: 0.22
Nodes (8): Calls, Calls, DatabaseSqlObjectsInstaller.EnsureLicenseSellableTablesAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 149 - "Community 149"
Cohesion: 0.22
Nodes (8): Contains, DatabaseSqlObjectsInstaller, Dependencies, Downstream Relationships, Method, Note, References, Upstream Relationships

### Community 150 - "Community 150"
Cohesion: 0.22
Nodes (8): Contains, DateTimeHelper, Dependencies, Downstream Relationships, Method, Note, References, Upstream Relationships

### Community 151 - "Community 151"
Cohesion: 0.22
Nodes (8): Calls, Calls, Dependencies, DistributedAppCacheService.RemoveAsync(), Downstream Relationships, Method, Note, Upstream Relationships

### Community 152 - "Community 152"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, DistributedCacheHealthCheck, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 153 - "Community 153"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, EmailReceiverDTO, Inherits, Method, Note, Upstream Relationships

### Community 154 - "Community 154"
Cohesion: 0.22
Nodes (8): Calls, Calls, Dependencies, Downstream Relationships, EntityAdminService.BuildLookupLabel(), Method, Note, Upstream Relationships

### Community 155 - "Community 155"
Cohesion: 0.22
Nodes (8): Calls, Calls, Dependencies, Downstream Relationships, EntityAdminService.ParseKeyToken(), Method, Note, Upstream Relationships

### Community 156 - "Community 156"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, ErrorModel, Inherits, Method, Note, Upstream Relationships

### Community 157 - "Community 157"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, FluentValidationFilterAttribute, Inherits, Method, Note, Upstream Relationships

### Community 158 - "Community 158"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, IDataInitializer (2), Inherits, Method, Note, Upstream Relationships

### Community 159 - "Community 159"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, IdentityExtensions, Method, Note, References, Upstream Relationships

### Community 160 - "Community 160"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, Init, Method, Note, Upstream Relationships

### Community 161 - "Community 161"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, LoginModel, Method, Note, Upstream Relationships

### Community 162 - "Community 162"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, LogoutModel, Method, Note, Upstream Relationships

### Community 163 - "Community 163"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, LowercaseContractResolver, Method, Note, Upstream Relationships

### Community 164 - "Community 164"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, MessageBuilder, Method, Note, References, Upstream Relationships

### Community 165 - "Community 165"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, Method, NationalCodeValidator, Note, Upstream Relationships

### Community 166 - "Community 166"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, PhoneNumberValidator, Upstream Relationships

### Community 167 - "Community 167"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, PrivacyModel, Upstream Relationships

### Community 168 - "Community 168"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, RemoveVersionParameters, Upstream Relationships

### Community 169 - "Community 169"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, RoleViewDTO, Upstream Relationships

### Community 170 - "Community 170"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Method, Note, References, SecurityHelper, Upstream Relationships

### Community 171 - "Community 171"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, SentEmailDTO, Upstream Relationships

### Community 172 - "Community 172"
Cohesion: 0.22
Nodes (8): Calls, Calls, Dependencies, Downstream Relationships, Method, Note, ServiceCollectionExtensions.AddOtlpExporterIfConfigured(), Upstream Relationships

### Community 173 - "Community 173"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Method, Note, References, ServiceCollectionExtensions, Upstream Relationships

### Community 174 - "Community 174"
Cohesion: 0.22
Nodes (8): Calls, Calls, Dependencies, Downstream Relationships, Method, Note, ServiceCollectionExtensions.RegisterOpenTelemetry(), Upstream Relationships

### Community 175 - "Community 175"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Method, Note, References, SessionManager, Upstream Relationships

### Community 176 - "Community 176"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, SetVersionInPaths, Upstream Relationships

### Community 177 - "Community 177"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Method, Note, References, SimpleRateLimitMiddleware, Upstream Relationships

### Community 178 - "Community 178"
Cohesion: 0.22
Nodes (8): Calls, Calls, Dependencies, Downstream Relationships, Method, Note, TypeExtensions.FindInterfacesThatClosesCore(), Upstream Relationships

### Community 179 - "Community 179"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships, UserListDTO

### Community 180 - "Community 180"
Cohesion: 0.22
Nodes (8): AuthenticationService.CreateOrUpdateSessionAsync(), Calls, Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 181 - "Community 181"
Cohesion: 0.22
Nodes (8): AuthenticationService.GetCurrentBearerHash(), Calls, Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 182 - "Community 182"
Cohesion: 0.22
Nodes (8): AuthenticationService.GetOrCreateConfirmationCodeAsync(), Calls, Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 183 - "Community 183"
Cohesion: 0.22
Nodes (8): AuthenticationService.RevokeAllUserSessionsAsync(), Calls, Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 184 - "Community 184"
Cohesion: 0.22
Nodes (8): AuthenticationService.SendActivationEmailAsync(), Calls, Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 185 - "Community 185"
Cohesion: 0.22
Nodes (8): AuthenticationService.SendRecoveryOtpAsync(), Calls, Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 186 - "Community 186"
Cohesion: 0.22
Nodes (8): AuthorizationManagementService, Contains, Dependencies, Downstream Relationships, Inherits, Method, Note, Upstream Relationships

### Community 187 - "Community 187"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, IAdminPanelAuthService, Inherits, Method, Note, Upstream Relationships

### Community 188 - "Community 188"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, IEntityAdminService, Inherits, Method, Note, Upstream Relationships

### Community 189 - "Community 189"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, IOperationCatalogService, Method, Note, Upstream Relationships

### Community 190 - "Community 190"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, IRoleService, Method, Note, Upstream Relationships

### Community 191 - "Community 191"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, IUserService, Method, Note, Upstream Relationships

### Community 192 - "Community 192"
Cohesion: 0.22
Nodes (8): Contains, Dependencies, Downstream Relationships, Inherits, Method, NoOpAppCacheService, Note, Upstream Relationships

### Community 193 - "Community 193"
Cohesion: 0.22
Nodes (8): Calls, Calls, Dependencies, Downstream Relationships, Method, Note, Repository.Delete(), Upstream Relationships

### Community 194 - "Community 194"
Cohesion: 0.22
Nodes (8): Calls, Calls, Dependencies, Downstream Relationships, Method, Note, Repository.DeleteAsync(), Upstream Relationships

### Community 195 - "Community 195"
Cohesion: 0.22
Nodes (8): Calls, Calls, Dependencies, Downstream Relationships, Method, Note, Repository.TryGetPropertyValueAsGuid(), Upstream Relationships

### Community 196 - "Community 196"
Cohesion: 0.22
Nodes (8): Calls, Calls, Dependencies, Downstream Relationships, Method, Note, Repository.Update(), Upstream Relationships

### Community 197 - "Community 197"
Cohesion: 0.22
Nodes (8): Calls, Calls, Dependencies, Downstream Relationships, Method, Note, Repository.UpdateRange(), Upstream Relationships

### Community 198 - "Community 198"
Cohesion: 0.22
Nodes (8): Calls, Calls, Dependencies, Downstream Relationships, Method, Note, SenderService.SendEmailAsync(), Upstream Relationships

### Community 202 - "Community 202"
Cohesion: 0.32
Nodes (3): ApplicationDbContext, Data, IdentityDbContext

### Community 204 - "Community 204"
Cohesion: 0.29
Nodes (4): JwtService, IJwtService, SignInManager, SiteSettings

### Community 205 - "Community 205"
Cohesion: 0.39
Nodes (3): EffectiveAuthorizationService, IEffectiveAuthorizationService, TimeSpan

### Community 206 - "Community 206"
Cohesion: 0.25
Nodes (7): Agent Memory Rules, ابزارهای مناسب برای agent, ثبت خودکار فعلی, دسترسی مستقیم به دیتابیس, قوانین کیفیت, چه زمانی episode ثبت شود, چه زمانی episode جستجو شود

### Community 207 - "Community 207"
Cohesion: 0.25
Nodes (7): Agent-Safe Tool APIs, Build / Verify, EF Core, Episodic Memory APIs, Graphify, MCP Guidance, Tooling Notes

### Community 208 - "Community 208"
Cohesion: 0.25
Nodes (7): AuthController.ConfirmEmailRegistrationAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 209 - "Community 209"
Cohesion: 0.25
Nodes (7): AuthController.ConfirmEmailRegistrationByLinkAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 210 - "Community 210"
Cohesion: 0.25
Nodes (7): BaseAdminApiController (2), Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 211 - "Community 211"
Cohesion: 0.25
Nodes (7): BaseCommonController, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 212 - "Community 212"
Cohesion: 0.25
Nodes (7): BaseThirdPartyController, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 213 - "Community 213"
Cohesion: 0.25
Nodes (7): BaseUserApiController (2), Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 214 - "Community 214"
Cohesion: 0.25
Nodes (7): AccountMembership, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 215 - "Community 215"
Cohesion: 0.25
Nodes (7): AccountMembershipGroup, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 216 - "Community 216"
Cohesion: 0.25
Nodes (7): AccountMembershipPermission, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 217 - "Community 217"
Cohesion: 0.25
Nodes (7): AccountOrganization, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 218 - "Community 218"
Cohesion: 0.25
Nodes (7): AccountOrganizationSecuritySetting, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 219 - "Community 219"
Cohesion: 0.25
Nodes (7): AccountPermission, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 220 - "Community 220"
Cohesion: 0.25
Nodes (7): AccountPlanPermission, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 221 - "Community 221"
Cohesion: 0.25
Nodes (7): AccountRolePermission, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 222 - "Community 222"
Cohesion: 0.25
Nodes (7): AccountSecurityFeature, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 223 - "Community 223"
Cohesion: 0.25
Nodes (7): AccountUserOauthIdentity, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 224 - "Community 224"
Cohesion: 0.25
Nodes (7): AccountUserPermissionGrant, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 225 - "Community 225"
Cohesion: 0.25
Nodes (7): AccountUserPermissionRevoke, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 226 - "Community 226"
Cohesion: 0.25
Nodes (7): AccountUserPlanSubscription, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 227 - "Community 227"
Cohesion: 0.25
Nodes (7): AccountUserSecuritySetting, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 228 - "Community 228"
Cohesion: 0.25
Nodes (7): AccountUserSession, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 229 - "Community 229"
Cohesion: 0.25
Nodes (7): AccountUserTotpFactor, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 230 - "Community 230"
Cohesion: 0.25
Nodes (7): AccountUserVerification, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 231 - "Community 231"
Cohesion: 0.25
Nodes (7): BaseDocumentEntity, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 232 - "Community 232"
Cohesion: 0.25
Nodes (7): BaseEntity (2), Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 233 - "Community 233"
Cohesion: 0.25
Nodes (7): Category, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 234 - "Community 234"
Cohesion: 0.25
Nodes (7): ConfirmationCode, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 235 - "Community 235"
Cohesion: 0.25
Nodes (7): Contains, CoreCounty, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 236 - "Community 236"
Cohesion: 0.25
Nodes (7): Contains, CoreProvince, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 237 - "Community 237"
Cohesion: 0.25
Nodes (7): Contains, CrmCustomer, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 238 - "Community 238"
Cohesion: 0.25
Nodes (7): Contains, CrmCustomerFinancialSummary, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 239 - "Community 239"
Cohesion: 0.25
Nodes (7): Contains, CrmCustomerInterest, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 240 - "Community 240"
Cohesion: 0.25
Nodes (7): Contains, CrmCustomerLedger, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 241 - "Community 241"
Cohesion: 0.25
Nodes (7): Contains, CrmCustomerLoyalty, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 242 - "Community 242"
Cohesion: 0.25
Nodes (7): Contains, CrmCustomerNote, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 243 - "Community 243"
Cohesion: 0.25
Nodes (7): Contains, CrmDiscountCampaign, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 244 - "Community 244"
Cohesion: 0.25
Nodes (7): Contains, CrmDiscountCampaignItemRule, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 245 - "Community 245"
Cohesion: 0.25
Nodes (7): Contains, CrmDiscountCampaignTargetCustomer, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 246 - "Community 246"
Cohesion: 0.25
Nodes (7): Contains, CrmDiscountCampaignUsage, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 247 - "Community 247"
Cohesion: 0.25
Nodes (7): Contains, CrmDiscountCoupon, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 248 - "Community 248"
Cohesion: 0.25
Nodes (7): Contains, CrmInterestTag, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 249 - "Community 249"
Cohesion: 0.25
Nodes (7): Contains, CrmLoyaltyTier, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 250 - "Community 250"
Cohesion: 0.25
Nodes (7): Contains, CrmOrganizationPhone, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 251 - "Community 251"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EditModel.GetValue(), Method, Note, Upstream Relationships

### Community 252 - "Community 252"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EditModel.OnGetAsync(), Method, Note, Upstream Relationships

### Community 253 - "Community 253"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Email, Inherits, Note, Upstream Relationships

### Community 254 - "Community 254"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, EmailDocument, Inherits, Note, Upstream Relationships

### Community 255 - "Community 255"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, EmailSharedInformation, Inherits, Note, Upstream Relationships

### Community 256 - "Community 256"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, IEntity (2), Inherits, Note, Upstream Relationships

### Community 257 - "Community 257"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Item, Note, Upstream Relationships

### Community 258 - "Community 258"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, LicenseBillingCycle, Note, Upstream Relationships

### Community 259 - "Community 259"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, LicenseModule, Note, Upstream Relationships

### Community 260 - "Community 260"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, LicenseModulePermission, Note, Upstream Relationships

### Community 261 - "Community 261"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, LicensePlan, Note, Upstream Relationships

### Community 262 - "Community 262"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, LicensePlanModule, Note, Upstream Relationships

### Community 263 - "Community 263"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, LicensePlanPrice, Note, Upstream Relationships

### Community 264 - "Community 264"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, LicenseSubscription, Note, Upstream Relationships

### Community 265 - "Community 265"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, ListModel.BuildColumns(), Method, Note, Upstream Relationships

### Community 266 - "Community 266"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, ListModel.OnGet(), Method, Note, Upstream Relationships

### Community 267 - "Community 267"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, MenuDigitalMenu, Note, Upstream Relationships

### Community 268 - "Community 268"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, MenuDigitalMenuMetrics, Note, Upstream Relationships

### Community 269 - "Community 269"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, MenuDigitalMenuProfile, Note, Upstream Relationships

### Community 270 - "Community 270"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, MenuDigitalMenuScheduleException, Note, Upstream Relationships

### Community 271 - "Community 271"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, MenuDigitalMenuScheduleWeekly, Note, Upstream Relationships

### Community 272 - "Community 272"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, MenuDigitalMenuVisit, Note, Upstream Relationships

### Community 273 - "Community 273"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, MenuIngredient, Note, Upstream Relationships

### Community 274 - "Community 274"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, MenuIngredientComponent, Note, Upstream Relationships

### Community 275 - "Community 275"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, MenuItemIngredient, Note, Upstream Relationships

### Community 276 - "Community 276"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, MenuUnit, Note, Upstream Relationships

### Community 277 - "Community 277"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, PaymentOperation, Upstream Relationships

### Community 278 - "Community 278"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, PaymentWallet, Upstream Relationships

### Community 279 - "Community 279"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, PaymentWalletEntry, Upstream Relationships

### Community 280 - "Community 280"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, Role, Upstream Relationships

### Community 281 - "Community 281"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Note, References, RoleHelper, Upstream Relationships

### Community 282 - "Community 282"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, SupportTicket, Upstream Relationships

### Community 283 - "Community 283"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, SupportTicketAttachment, Upstream Relationships

### Community 284 - "Community 284"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, SupportTicketEvent, Upstream Relationships

### Community 285 - "Community 285"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, SupportTicketMessage, Upstream Relationships

### Community 286 - "Community 286"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, SupportTicketMetrics, Upstream Relationships

### Community 287 - "Community 287"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships, User

### Community 288 - "Community 288"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships, UserRole

### Community 289 - "Community 289"
Cohesion: 0.25
Nodes (7): AdminPanelAuthService.HasAdminAccess(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 290 - "Community 290"
Cohesion: 0.25
Nodes (7): AdminPanelAuthService.SignInWithEmailPasswordAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 291 - "Community 291"
Cohesion: 0.25
Nodes (7): AdminPanelServiceCollectionExtensions.AddAdminPanelInfrastructure(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 292 - "Community 292"
Cohesion: 0.25
Nodes (7): AdminPanelServiceCollectionExtensions.RegisterMarkedServices(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 293 - "Community 293"
Cohesion: 0.25
Nodes (7): AI Agent Rules (VS Code + GPT-5.3-Codex), Contains, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 294 - "Community 294"
Cohesion: 0.25
Nodes (7): ApiCustomAuthorize.OnActionExecuting(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 295 - "Community 295"
Cohesion: 0.25
Nodes (7): ApiCustomAuthorize.OnActionExecutionAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 296 - "Community 296"
Cohesion: 0.25
Nodes (7): ApiCustomAuthorize.UnAuthorize(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 297 - "Community 297"
Cohesion: 0.25
Nodes (7): ApiResult, Contains, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 298 - "Community 298"
Cohesion: 0.25
Nodes (7): AppException, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 299 - "Community 299"
Cohesion: 0.25
Nodes (7): ApplicationBuilderExtensions, Contains, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 300 - "Community 300"
Cohesion: 0.25
Nodes (7): ApplicationDbContext.SaveChanges(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 301 - "Community 301"
Cohesion: 0.25
Nodes (7): ApplicationDbContext.SaveChangesAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 302 - "Community 302"
Cohesion: 0.25
Nodes (7): ApplicationDbContext._cleanString(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 303 - "Community 303"
Cohesion: 0.25
Nodes (7): ApplicationDefaultSettings, Contains, Dependencies, Downstream Relationships, Note, References, Upstream Relationships

### Community 304 - "Community 304"
Cohesion: 0.25
Nodes (7): ApplicationMessages, Contains, Dependencies, Downstream Relationships, Note, References, Upstream Relationships

### Community 305 - "Community 305"
Cohesion: 0.25
Nodes (7): ApplicationPropertyPersianName, Contains, Dependencies, Downstream Relationships, Note, References, Upstream Relationships

### Community 306 - "Community 306"
Cohesion: 0.25
Nodes (7): AssemblyHelper.FindAllTypes(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 307 - "Community 307"
Cohesion: 0.25
Nodes (7): AssemblyHelper.LoadAllBinDirectoryAssemblies(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 308 - "Community 308"
Cohesion: 0.25
Nodes (7): Assert, Contains, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 309 - "Community 309"
Cohesion: 0.25
Nodes (7): AUTH Business Spec, Contains, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 310 - "Community 310"
Cohesion: 0.25
Nodes (7): AuthorizationDataInitializer.EnsurePlan(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 311 - "Community 311"
Cohesion: 0.25
Nodes (7): AuthorizationDataInitializer.EnsureSeedUserPlan(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 312 - "Community 312"
Cohesion: 0.25
Nodes (7): AuthorizationDataInitializer.InitializeData(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 313 - "Community 313"
Cohesion: 0.25
Nodes (7): AuthorizationDataInitializer.MapPlanPermissions(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 314 - "Community 314"
Cohesion: 0.25
Nodes (7): AuthorizationDataInitializer.MapRolePermissions(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 315 - "Community 315"
Cohesion: 0.25
Nodes (7): AutofacModule.cs, Contains, Dependencies, Downstream Relationships, Imports, Note, Upstream Relationships

### Community 316 - "Community 316"
Cohesion: 0.25
Nodes (7): AutoMapperConfiguration.AddCustomMappingProfile(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 317 - "Community 317"
Cohesion: 0.25
Nodes (7): AutoMapperConfiguration.InitializeAutoMapper(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 318 - "Community 318"
Cohesion: 0.25
Nodes (7): AutoMapperConfiguration, Contains, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 319 - "Community 319"
Cohesion: 0.25
Nodes (7): BadRequestException, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 320 - "Community 320"
Cohesion: 0.25
Nodes (7): BaseDTO.CastToDerivedClass(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 321 - "Community 321"
Cohesion: 0.25
Nodes (7): BaseDTO.CreateMappings(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 322 - "Community 322"
Cohesion: 0.25
Nodes (7): BaseDTO.CustomMappings(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 323 - "Community 323"
Cohesion: 0.25
Nodes (7): BaseDTO.ToEntity(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 324 - "Community 324"
Cohesion: 0.25
Nodes (7): Business Docs Index, Contains, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 325 - "Community 325"
Cohesion: 0.25
Nodes (7): Business Spec (2), Contains, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 326 - "Community 326"
Cohesion: 0.25
Nodes (7): Business Spec (3), Contains, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 327 - "Community 327"
Cohesion: 0.25
Nodes (7): Business Spec (4), Contains, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 328 - "Community 328"
Cohesion: 0.25
Nodes (7): Business Spec (5), Contains, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 329 - "Community 329"
Cohesion: 0.25
Nodes (7): Business Spec (6), Contains, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 330 - "Community 330"
Cohesion: 0.25
Nodes (7): Business Spec (7), Contains, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 331 - "Community 331"
Cohesion: 0.25
Nodes (7): Business Spec, Contains, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 332 - "Community 332"
Cohesion: 0.25
Nodes (7): Calculator.CalculateProjectRemainTime(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 333 - "Community 333"
Cohesion: 0.25
Nodes (7): Calculator.CalculateProjectRemainTimeLabel(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 334 - "Community 334"
Cohesion: 0.25
Nodes (7): Calculator.CalculateProjectTime(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 335 - "Community 335"
Cohesion: 0.25
Nodes (7): Calculator.CalculateRequiredPricePercent(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 336 - "Community 336"
Cohesion: 0.25
Nodes (7): Calculator.CalculateTotalPricePercent(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 337 - "Community 337"
Cohesion: 0.25
Nodes (7): Calculator, Contains, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 338 - "Community 338"
Cohesion: 0.25
Nodes (7): CaptchaHelper, Contains, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 339 - "Community 339"
Cohesion: 0.25
Nodes (7): CI recommendation, Contains, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 340 - "Community 340"
Cohesion: 0.25
Nodes (7): CompletePhoneRegistrationRequestValidator, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 341 - "Community 341"
Cohesion: 0.25
Nodes (7): ConfirmEmailRegistrationRequestValidator, Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 342 - "Community 342"
Cohesion: 0.25
Nodes (7): Calls, CookieManager.Remove(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 343 - "Community 343"
Cohesion: 0.25
Nodes (7): Calls, CookieManager.RemoveAllCookie(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 344 - "Community 344"
Cohesion: 0.25
Nodes (7): Calls, CrudController.Delete(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 345 - "Community 345"
Cohesion: 0.25
Nodes (7): Calls, CrudController.Get(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 346 - "Community 346"
Cohesion: 0.25
Nodes (7): Contains, CustomMappingProfile, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 347 - "Community 347"
Cohesion: 0.25
Nodes (7): Calls, DatabaseSqlObjectsInstaller.ApplyAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 348 - "Community 348"
Cohesion: 0.25
Nodes (7): Calls, DatabaseSqlObjectsInstaller.ExecuteSqlScriptAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 349 - "Community 349"
Cohesion: 0.25
Nodes (7): Calls, DatabaseSqlObjectsInstaller.NormalizeSql(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 350 - "Community 350"
Cohesion: 0.25
Nodes (7): Calls, DatabaseSqlObjectsInstaller.ReadEmbeddedSql(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 351 - "Community 351"
Cohesion: 0.25
Nodes (7): Contains, DataInitializerDefaultValues, Dependencies, Downstream Relationships, Note, References, Upstream Relationships

### Community 352 - "Community 352"
Cohesion: 0.25
Nodes (7): Contains, DateTimeExtensions, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 353 - "Community 353"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, DistributedAppCacheService.GetAsync(), Downstream Relationships, Method, Note, Upstream Relationships

### Community 354 - "Community 354"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, DistributedAppCacheService.IsInFailureCooldown(), Downstream Relationships, Method, Note, Upstream Relationships

### Community 355 - "Community 355"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, DistributedAppCacheService.MarkBackendUnavailable(), Downstream Relationships, Method, Note, Upstream Relationships

### Community 356 - "Community 356"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, DistributedAppCacheService.SetAsync(), Downstream Relationships, Method, Note, Upstream Relationships

### Community 357 - "Community 357"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, EmailPasswordLoginRequestValidator, Inherits, Note, Upstream Relationships

### Community 358 - "Community 358"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, EmailReceiverWithIdDTO, Inherits, Note, Upstream Relationships

### Community 359 - "Community 359"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.ApplyDefaultOrder(), Method, Note, Upstream Relationships

### Community 360 - "Community 360"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.ApplySearch(), Method, Note, Upstream Relationships

### Community 361 - "Community 361"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.AsNoTracking(), Method, Note, Upstream Relationships

### Community 362 - "Community 362"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.BuildEntityDescriptors(), Method, Note, Upstream Relationships

### Community 363 - "Community 363"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.BuildKeyToken(), Method, Note, Upstream Relationships

### Community 364 - "Community 364"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.Count(), Method, Note, Upstream Relationships

### Community 365 - "Community 365"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.CreatePropertyDescriptor(), Method, Note, Upstream Relationships

### Community 366 - "Community 366"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.DeleteAsync(), Method, Note, Upstream Relationships

### Community 367 - "Community 367"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.FindAsync(), Method, Note, Upstream Relationships

### Community 368 - "Community 368"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.FindByKey(), Method, Note, Upstream Relationships

### Community 369 - "Community 369"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.FormatValue(), Method, Note, Upstream Relationships

### Community 370 - "Community 370"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.GetForeignKeyOptionsAsync(), Method, Note, Upstream Relationships

### Community 371 - "Community 371"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.GetManagedEntity(), Method, Note, Upstream Relationships

### Community 372 - "Community 372"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.GetPage(), Method, Note, Upstream Relationships

### Community 373 - "Community 373"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.GetQueryable(), Method, Note, Upstream Relationships

### Community 374 - "Community 374"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.GuessDisplayProperty(), Method, Note, Upstream Relationships

### Community 375 - "Community 375"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.IsEditable(), Method, Note, Upstream Relationships

### Community 376 - "Community 376"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.IsSimpleType(), Method, Note, Upstream Relationships

### Community 377 - "Community 377"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.SaveAsync(), Method, Note, Upstream Relationships

### Community 378 - "Community 378"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EntityAdminService.TryConvert(), Method, Note, Upstream Relationships

### Community 379 - "Community 379"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, EnumExtensions, Method, Note, Upstream Relationships

### Community 380 - "Community 380"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EnumExtensions.ToDictionary(), Method, Note, Upstream Relationships

### Community 381 - "Community 381"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EnumExtensions.ToDisplay(), Method, Note, Upstream Relationships

### Community 382 - "Community 382"
Cohesion: 0.25
Nodes (7): Calls, Contains, Dependencies, Downstream Relationships, Error(), Note, Upstream Relationships

### Community 383 - "Community 383"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, FilterUserCursorRequest, Inherits, Note, Upstream Relationships

### Community 384 - "Community 384"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, FilterUserRequest, Inherits, Note, Upstream Relationships

### Community 385 - "Community 385"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, FluentErrorResponseBuilder, Method, Note, Upstream Relationships

### Community 386 - "Community 386"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, FluentValidationExtensions, Method, Note, Upstream Relationships

### Community 387 - "Community 387"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, ForgotPasswordRequestValidator, Inherits, Note, Upstream Relationships

### Community 388 - "Community 388"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, GetAdminsPaginatedRequest, Inherits, Note, Upstream Relationships

### Community 389 - "Community 389"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, HttpRequestExtensions, Method, Note, Upstream Relationships

### Community 390 - "Community 390"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, IdentityConfigurationExtensions, Method, Note, Upstream Relationships

### Community 391 - "Community 391"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, IdentityExtensions.FindFirstValue(), Method, Note, Upstream Relationships

### Community 392 - "Community 392"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, IdentityExtensions.GetUserId(), Method, Note, Upstream Relationships

### Community 393 - "Community 393"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, IdentityExtensions.GetUserName(), Method, Note, Upstream Relationships

### Community 394 - "Community 394"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, IHaveCustomMapping, Method, Note, Upstream Relationships

### Community 395 - "Community 395"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Init (2), Method, Note, Upstream Relationships

### Community 396 - "Community 396"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, LogicException, Note, Upstream Relationships

### Community 397 - "Community 397"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, ModelBuilderExtensions.AddDefaultValues(), Note, Upstream Relationships

### Community 398 - "Community 398"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, ModelBuilderExtensions.AddDefaultValueSqlConvention(), Note, Upstream Relationships

### Community 399 - "Community 399"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, ModelBuilderExtensions.AddSequentialGuidForIdConvention(), Note, Upstream Relationships

### Community 400 - "Community 400"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Method, ModelBuilderExtensions, Note, Upstream Relationships

### Community 401 - "Community 401"
Cohesion: 0.25
Nodes (7): Dependencies, Downstream Relationships, Imports, Inherits, Module, Note, Upstream Relationships

### Community 402 - "Community 402"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, NotFoundException, Upstream Relationships

### Community 403 - "Community 403"
Cohesion: 0.25
Nodes (7): Calls, Contains, Dependencies, Downstream Relationships, Note, Ok(), Upstream Relationships

### Community 404 - "Community 404"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, OperationCatalogService.DiscoverOperations(), Upstream Relationships

### Community 405 - "Community 405"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, OperationCatalogService.GetOperation(), Upstream Relationships

### Community 406 - "Community 406"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, OperationCatalogService.InvokeAsync(), Upstream Relationships

### Community 407 - "Community 407"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, OperationCatalogService.IsOperationSafeShape(), Upstream Relationships

### Community 408 - "Community 408"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, OperationCatalogService.IsRequired(), Upstream Relationships

### Community 409 - "Community 409"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, OperationCatalogService.SerializeResult(), Upstream Relationships

### Community 410 - "Community 410"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, OperationCatalogService.TryConvert(), Upstream Relationships

### Community 411 - "Community 411"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Method, Note, OperationResult, Upstream Relationships

### Community 412 - "Community 412"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, PagingDTO, Upstream Relationships

### Community 413 - "Community 413"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, PhoneOtpLoginRequestValidator, Upstream Relationships

### Community 414 - "Community 414"
Cohesion: 0.25
Nodes (7): Contains, Contains, Dependencies, Downstream Relationships, Note, Practical Examples, Upstream Relationships

### Community 415 - "Community 415"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, PublicEnumAttribute, Upstream Relationships

### Community 416 - "Community 416"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Method, Note, QueryableExtensions, Upstream Relationships

### Community 417 - "Community 417"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, ReflectionHelper.GetTypesAssignableFrom(), Upstream Relationships

### Community 418 - "Community 418"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, ReflectionHelper.GetTypesHasAttribute(), Upstream Relationships

### Community 419 - "Community 419"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, ReflectionHelper.HasAttribute(), Upstream Relationships

### Community 420 - "Community 420"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, ReflectionHelper.IsCustomReferenceType(), Upstream Relationships

### Community 421 - "Community 421"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, ReflectionHelper.IsCustomType(), Upstream Relationships

### Community 422 - "Community 422"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, ReflectionHelper.IsCustomValueType(), Upstream Relationships

### Community 423 - "Community 423"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, ReflectionHelper.IsInheritFrom(), Upstream Relationships

### Community 424 - "Community 424"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Method, Note, ReflectionHelper, Upstream Relationships

### Community 425 - "Community 425"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Note, References, RegexHelper, Upstream Relationships

### Community 426 - "Community 426"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, RegisterWithEmailRequestValidator, Upstream Relationships

### Community 427 - "Community 427"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Note, References, RegularExpression, Upstream Relationships

### Community 428 - "Community 428"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Method, Note, RequestContextEnrichmentMiddleware, Upstream Relationships

### Community 429 - "Community 429"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, RequestTelemetryMiddleware.InvokeAsync(), Upstream Relationships

### Community 430 - "Community 430"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, RequestTelemetryMiddleware.IsExcludedPath(), Upstream Relationships

### Community 431 - "Community 431"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Method, Note, RequestTelemetryMiddleware, Upstream Relationships

### Community 432 - "Community 432"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, ResendEmailActivationLinkRequestValidator, Upstream Relationships

### Community 433 - "Community 433"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, ResendPhoneRegistrationOtpRequestValidator, Upstream Relationships

### Community 434 - "Community 434"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, ResetPasswordWithOtpRequestValidator, Upstream Relationships

### Community 435 - "Community 435"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, RevokeSessionRequestValidator, Upstream Relationships

### Community 436 - "Community 436"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, SecurityHelper.ConfigureAesFromPassword(), Upstream Relationships

### Community 437 - "Community 437"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, SecurityHelper.Decrypt(), Upstream Relationships

### Community 438 - "Community 438"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, SecurityHelper.Encrypt(), Upstream Relationships

### Community 439 - "Community 439"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, SecurityHelper.ResolveDefaultEncryptionKey(), Upstream Relationships

### Community 440 - "Community 440"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, SendPhoneLoginOtpRequestValidator, Upstream Relationships

### Community 441 - "Community 441"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Method, Note, SerilogConfiguration, Upstream Relationships

### Community 442 - "Community 442"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, ServiceCollectionExtensions.AddPerformanceInfrastructure(), Upstream Relationships

### Community 443 - "Community 443"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, ServiceCollectionExtensions.IsExcludedTelemetryPath(), Upstream Relationships

### Community 444 - "Community 444"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, ServiceCollectionExtensions.ParseOtlpProtocol(), Upstream Relationships

### Community 445 - "Community 445"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, ServiceCollectionExtensions.RegisterDistributedCache(), Upstream Relationships

### Community 446 - "Community 446"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, SetMfaStatusRequestValidator, Upstream Relationships

### Community 447 - "Community 447"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, SimpleRateLimitMiddleware.BuildClientKey(), Upstream Relationships

### Community 448 - "Community 448"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, SimpleRateLimitMiddleware.InvokeAsync(), Upstream Relationships

### Community 449 - "Community 449"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, SlowQueryLoggingInterceptor.NonQueryExecuted(), Upstream Relationships

### Community 450 - "Community 450"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, SlowQueryLoggingInterceptor.NonQueryExecuting(), Upstream Relationships

### Community 451 - "Community 451"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, SlowQueryLoggingInterceptor.ReaderExecuted(), Upstream Relationships

### Community 452 - "Community 452"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, SlowQueryLoggingInterceptor.ReaderExecuting(), Upstream Relationships

### Community 453 - "Community 453"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, SlowQueryLoggingInterceptor.ScalarExecuted(), Upstream Relationships

### Community 454 - "Community 454"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, SlowQueryLoggingInterceptor.ScalarExecuting(), Upstream Relationships

### Community 455 - "Community 455"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, SlowQueryLoggingInterceptor.StartTiming(), Upstream Relationships

### Community 456 - "Community 456"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, SlowQueryLoggingInterceptor.StopAndLogIfSlow(), Upstream Relationships

### Community 457 - "Community 457"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, StartPhoneRegistrationRequestValidator, Upstream Relationships

### Community 458 - "Community 458"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, StringExtensions.CleanString(), Upstream Relationships

### Community 459 - "Community 459"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, StringExtensions.Fa2En(), Upstream Relationships

### Community 460 - "Community 460"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, StringExtensions.FixPersianChars(), Upstream Relationships

### Community 461 - "Community 461"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Method, Note, StringExtensions, Upstream Relationships

### Community 462 - "Community 462"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, StringExtensions.NullIfEmpty(), Upstream Relationships

### Community 463 - "Community 463"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Method, Note, SwaggerConfigurationExtensions, Upstream Relationships

### Community 464 - "Community 464"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, TypeExtensions.FindInterfacesThatClose(), Upstream Relationships

### Community 465 - "Community 465"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, TypeExtensions.IsConcrete(), Upstream Relationships

### Community 466 - "Community 466"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Method, Note, TypeExtensions, Upstream Relationships

### Community 467 - "Community 467"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships, UserIdDTOValidator

### Community 468 - "Community 468"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UtilConvertor.ContainProperty()

### Community 469 - "Community 469"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UtilConvertor.GetPropertiesName()

### Community 470 - "Community 470"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UtilConvertor

### Community 471 - "Community 471"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UtilConvertor.ToObject()

### Community 472 - "Community 472"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UtilConvertor.TryGetJObject()

### Community 473 - "Community 473"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, ValidationHelper

### Community 474 - "Community 474"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, WebApplicationBuilderExtensions

### Community 475 - "Community 475"
Cohesion: 0.25
Nodes (7): Contains, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, Workflow

### Community 476 - "Community 476"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, IRepository (2), Method, Note, Upstream Relationships

### Community 477 - "Community 477"
Cohesion: 0.25
Nodes (7): AuthenticationService.AddDefaultRoleToUserAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 478 - "Community 478"
Cohesion: 0.25
Nodes (7): AuthenticationService.BuildEmailActivationLink(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 479 - "Community 479"
Cohesion: 0.25
Nodes (7): AuthenticationService.BuildLoginResponse(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 480 - "Community 480"
Cohesion: 0.25
Nodes (7): AuthenticationService.CompletePhoneRegistrationAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 481 - "Community 481"
Cohesion: 0.25
Nodes (7): AuthenticationService.ComputeSha256(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 482 - "Community 482"
Cohesion: 0.25
Nodes (7): AuthenticationService.ConfirmEmailRegistrationAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 483 - "Community 483"
Cohesion: 0.25
Nodes (7): AuthenticationService.CreateUserAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 484 - "Community 484"
Cohesion: 0.25
Nodes (7): AuthenticationService.EmailPasswordLoginAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 485 - "Community 485"
Cohesion: 0.25
Nodes (7): AuthenticationService.EnsureMfaFeatureAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 486 - "Community 486"
Cohesion: 0.25
Nodes (7): AuthenticationService.FindUserByIdentifierAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 487 - "Community 487"
Cohesion: 0.25
Nodes (7): AuthenticationService.ForgotPasswordAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 488 - "Community 488"
Cohesion: 0.25
Nodes (7): AuthenticationService.GenerateOtp(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 489 - "Community 489"
Cohesion: 0.25
Nodes (7): AuthenticationService.GetMfaStatusAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 490 - "Community 490"
Cohesion: 0.25
Nodes (7): AuthenticationService.GetMySessionsAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 491 - "Community 491"
Cohesion: 0.25
Nodes (7): AuthenticationService.HasRecentActivationEmail(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 492 - "Community 492"
Cohesion: 0.25
Nodes (7): AuthenticationService.HasRecentPhoneRegistrationOtp(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 493 - "Community 493"
Cohesion: 0.25
Nodes (7): AuthenticationService.MapUserIdToLegacyInt(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 494 - "Community 494"
Cohesion: 0.25
Nodes (7): AuthenticationService.OtpLoginAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 495 - "Community 495"
Cohesion: 0.25
Nodes (7): AuthenticationService.ParseDeviceType(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 496 - "Community 496"
Cohesion: 0.25
Nodes (7): AuthenticationService.PasswordLoginAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 497 - "Community 497"
Cohesion: 0.25
Nodes (7): AuthenticationService.PhoneOtpLoginAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 498 - "Community 498"
Cohesion: 0.25
Nodes (7): AuthenticationService.RegisterWithEmailAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 499 - "Community 499"
Cohesion: 0.25
Nodes (7): AuthenticationService.ResendEmailActivationLinkAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 500 - "Community 500"
Cohesion: 0.25
Nodes (7): AuthenticationService.ResendPhoneRegistrationOtpAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 501 - "Community 501"
Cohesion: 0.25
Nodes (7): AuthenticationService.ResetPasswordWithOtpAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 502 - "Community 502"
Cohesion: 0.25
Nodes (7): AuthenticationService.RevokeOtherSessionsAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 503 - "Community 503"
Cohesion: 0.25
Nodes (7): AuthenticationService.RevokeSessionAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 504 - "Community 504"
Cohesion: 0.25
Nodes (7): AuthenticationService.SendOtpAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 505 - "Community 505"
Cohesion: 0.25
Nodes (7): AuthenticationService.SendPhoneLoginOtpAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 506 - "Community 506"
Cohesion: 0.25
Nodes (7): AuthenticationService.SetMfaStatusAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 507 - "Community 507"
Cohesion: 0.25
Nodes (7): AuthenticationService.StartPhoneRegistrationAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 508 - "Community 508"
Cohesion: 0.25
Nodes (7): AuthenticationService.UpdatePasswordAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 509 - "Community 509"
Cohesion: 0.25
Nodes (7): AuthenticationService.UpdateUserAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 510 - "Community 510"
Cohesion: 0.25
Nodes (7): AuthorizationCatalog, Contains, Dependencies, Downstream Relationships, Note, References, Upstream Relationships

### Community 511 - "Community 511"
Cohesion: 0.25
Nodes (7): AuthorizationManagementService.ResolvePermissionIdAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 512 - "Community 512"
Cohesion: 0.25
Nodes (7): AuthorizationManagementService.ResolvePermissionIdsAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 513 - "Community 513"
Cohesion: 0.25
Nodes (7): AuthorizationManagementService.UpsertPlanPermissionsAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 514 - "Community 514"
Cohesion: 0.25
Nodes (7): AuthorizationManagementService.UpsertRolePermissionsAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 515 - "Community 515"
Cohesion: 0.25
Nodes (7): AuthorizationManagementService.UpsertUserPermissionGrantAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 516 - "Community 516"
Cohesion: 0.25
Nodes (7): AuthorizationManagementService.UpsertUserPermissionRevokeAsync(), Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 517 - "Community 517"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EffectiveAuthorizationService.BuildAuthorizationCacheKey(), Method, Note, Upstream Relationships

### Community 518 - "Community 518"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EffectiveAuthorizationService.ComputePermissionVersionAsync(), Method, Note, Upstream Relationships

### Community 519 - "Community 519"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EffectiveAuthorizationService.GetEffectiveAuthorizationAsync(), Method, Note, Upstream Relationships

### Community 520 - "Community 520"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, EffectiveAuthorizationService.InvalidateUserAuthorizationAsync(), Method, Note, Upstream Relationships

### Community 521 - "Community 521"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, IAppCacheService (2), Method, Note, Upstream Relationships

### Community 522 - "Community 522"
Cohesion: 0.25
Nodes (7): Dependencies, Downstream Relationships, IAppCacheService, Inherits, Note, References, Upstream Relationships

### Community 523 - "Community 523"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, IAuthenticationService, Method, Note, Upstream Relationships

### Community 524 - "Community 524"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, IAuthorizationManagementService, Method, Note, Upstream Relationships

### Community 525 - "Community 525"
Cohesion: 0.25
Nodes (7): Dependencies, Downstream Relationships, IEffectiveAuthorizationService (2), Inherits, Note, References, Upstream Relationships

### Community 526 - "Community 526"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, IEffectiveAuthorizationService, Method, Note, Upstream Relationships

### Community 527 - "Community 527"
Cohesion: 0.25
Nodes (7): Dependencies, Downstream Relationships, IJwtService (2), Inherits, Note, References, Upstream Relationships

### Community 528 - "Community 528"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, IJwtService, Method, Note, Upstream Relationships

### Community 529 - "Community 529"
Cohesion: 0.25
Nodes (7): Dependencies, Downstream Relationships, Inherits, IRepository, Note, References, Upstream Relationships

### Community 530 - "Community 530"
Cohesion: 0.25
Nodes (7): Dependencies, Downstream Relationships, Inherits, ISenderService (2), Note, References, Upstream Relationships

### Community 531 - "Community 531"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, ISenderService, Method, Note, Upstream Relationships

### Community 532 - "Community 532"
Cohesion: 0.25
Nodes (7): Dependencies, Downstream Relationships, Inherits, IServiceRepository (2), Note, References, Upstream Relationships

### Community 533 - "Community 533"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, IServiceRepository, Method, Note, Upstream Relationships

### Community 534 - "Community 534"
Cohesion: 0.25
Nodes (7): Dependencies, Downstream Relationships, Inherits, IUserContext (2), Note, References, Upstream Relationships

### Community 535 - "Community 535"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, JwtService.GenerateAsync(), Method, Note, Upstream Relationships

### Community 536 - "Community 536"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, JwtService.GetClaimsAsync(), Method, Note, Upstream Relationships

### Community 537 - "Community 537"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.Add(), Upstream Relationships

### Community 538 - "Community 538"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.AddAsync(), Upstream Relationships

### Community 539 - "Community 539"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.AddRange(), Upstream Relationships

### Community 540 - "Community 540"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.AddRangeAsync(), Upstream Relationships

### Community 541 - "Community 541"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.Attach(), Upstream Relationships

### Community 542 - "Community 542"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.BuildSeekPredicate(), Upstream Relationships

### Community 543 - "Community 543"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.CreatePage(), Upstream Relationships

### Community 544 - "Community 544"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.DeleteById(), Upstream Relationships

### Community 545 - "Community 545"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.DeleteByIdAsync(), Upstream Relationships

### Community 546 - "Community 546"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.DeleteRange(), Upstream Relationships

### Community 547 - "Community 547"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.DeleteRangeAsync(), Upstream Relationships

### Community 548 - "Community 548"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.EncodeCursorValue(), Upstream Relationships

### Community 549 - "Community 549"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.GetById(), Upstream Relationships

### Community 550 - "Community 550"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.GetByIdAsync(), Upstream Relationships

### Community 551 - "Community 551"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.GetPrimaryKey(), Upstream Relationships

### Community 552 - "Community 552"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.LoadCollection(), Upstream Relationships

### Community 553 - "Community 553"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.LoadCollectionAsync(), Upstream Relationships

### Community 554 - "Community 554"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.LoadReference(), Upstream Relationships

### Community 555 - "Community 555"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.LoadReferenceAsync(), Upstream Relationships

### Community 556 - "Community 556"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.NormalizeCursorPageSize(), Upstream Relationships

### Community 557 - "Community 557"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.SaveChanges(), Upstream Relationships

### Community 558 - "Community 558"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.SaveChangesAsync(), Upstream Relationships

### Community 559 - "Community 559"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.SelectBy(), Upstream Relationships

### Community 560 - "Community 560"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.SelectByAsync(), Upstream Relationships

### Community 561 - "Community 561"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.SelectByCursorAsync(), Upstream Relationships

### Community 562 - "Community 562"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.TryDecodeCursorValue(), Upstream Relationships

### Community 563 - "Community 563"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.TryGetPropertyValue(), Upstream Relationships

### Community 564 - "Community 564"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.UpdateAsync(), Upstream Relationships

### Community 565 - "Community 565"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.UpdateCustomPropertiesAsync(), Upstream Relationships

### Community 566 - "Community 566"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Repository.UpdateRangeAsync(), Upstream Relationships

### Community 567 - "Community 567"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, SenderService.BuildMailMessage(), Upstream Relationships

### Community 568 - "Community 568"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, SenderService.SendEmailWithAttachmentAsync(), Upstream Relationships

### Community 569 - "Community 569"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, ServiceRepository.Create(), Upstream Relationships

### Community 570 - "Community 570"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, ServiceRepository.Get(), Upstream Relationships

### Community 571 - "Community 571"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, ServiceRepository.MapToSelectDto(), Upstream Relationships

### Community 572 - "Community 572"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, ServiceRepository.RequireKey(), Upstream Relationships

### Community 573 - "Community 573"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, ServiceRepository.Update(), Upstream Relationships

### Community 574 - "Community 574"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, ServiceRepository.UpdateCustomProperties(), Upstream Relationships

### Community 575 - "Community 575"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, ServiceRepository.UpdateProperty(), Upstream Relationships

### Community 576 - "Community 576"
Cohesion: 0.25
Nodes (7): Contains, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships, UserContext

### Community 577 - "Community 577"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UserService.AddAsync()

### Community 578 - "Community 578"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UserService.All()

### Community 579 - "Community 579"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UserService.AllByCursor()

### Community 580 - "Community 580"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UserService.BuildUsersCursorListCacheKey()

### Community 581 - "Community 581"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UserService.BuildUsersListCacheKey()

### Community 582 - "Community 582"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UserService.DeleteAsync()

### Community 583 - "Community 583"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UserService.DeleteByIdAsync()

### Community 584 - "Community 584"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UserService.GetUsersListCacheVersionAsync()

### Community 585 - "Community 585"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UserService.RefreshUsersListCacheVersionAsync()

### Community 586 - "Community 586"
Cohesion: 0.25
Nodes (7): Calls, Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UserService.UpdateAsync()

### Community 587 - "Community 587"
Cohesion: 0.25
Nodes (7): CI recommendation, code:powershell (powershell -ExecutionPolicy Bypass -File .\tools\Check-Warni), code:powershell (powershell -ExecutionPolicy Bypass -File .\tools\Check-Warni), code:powershell (powershell -ExecutionPolicy Bypass -File .\tools\Check-Warni), Suggested cleanup order, Why staged?, Workflow

### Community 588 - "Community 588"
Cohesion: 0.29
Nodes (6): CachingSettings, ObservabilitySettings, PerformanceSettings, QueryTuningSettings, RateLimitingSettings, TelemetrySettings

### Community 589 - "Community 589"
Cohesion: 0.29
Nodes (4): AppException, BadRequestException, LogicException, NotFoundException

### Community 590 - "Community 590"
Cohesion: 0.29
Nodes (4): CoreCounty, CoreCountyConfiguration, CoreProvince, CoreProvinceConfiguration

### Community 594 - "Community 594"
Cohesion: 0.29
Nodes (4): Attribute, PublicEnumAttribute, ApiPermissionAuthorizeAttribute, IAsyncActionFilter

### Community 595 - "Community 595"
Cohesion: 0.29
Nodes (6): API.Controllers.Auth.v1, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 596 - "Community 596"
Cohesion: 0.29
Nodes (6): API.Controllers.Users.v1, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 597 - "Community 597"
Cohesion: 0.29
Nodes (6): AuthController.CompletePhoneRegistrationAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 598 - "Community 598"
Cohesion: 0.29
Nodes (6): AuthController.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 599 - "Community 599"
Cohesion: 0.29
Nodes (6): AuthController.EmailPasswordLoginAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 600 - "Community 600"
Cohesion: 0.29
Nodes (6): AuthController.ForgotPasswordAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 601 - "Community 601"
Cohesion: 0.29
Nodes (6): AuthController.LoginWithPasswordAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 602 - "Community 602"
Cohesion: 0.29
Nodes (6): AuthController.MfaStatusAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 603 - "Community 603"
Cohesion: 0.29
Nodes (6): AuthController.MyAuthorizationAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 604 - "Community 604"
Cohesion: 0.29
Nodes (6): AuthController.MySessionsAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 605 - "Community 605"
Cohesion: 0.29
Nodes (6): AuthController.OtpLoginAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 606 - "Community 606"
Cohesion: 0.29
Nodes (6): AuthController.PhoneOtpLoginAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 607 - "Community 607"
Cohesion: 0.29
Nodes (6): AuthController.RegisterWithEmailAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 608 - "Community 608"
Cohesion: 0.29
Nodes (6): AuthController.ResendEmailActivationLinkAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 609 - "Community 609"
Cohesion: 0.29
Nodes (6): AuthController.ResendPhoneRegistrationOtpAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 610 - "Community 610"
Cohesion: 0.29
Nodes (6): AuthController.ResetPasswordWithOtpAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 611 - "Community 611"
Cohesion: 0.29
Nodes (6): AuthController.RevokeOtherSessionsAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 612 - "Community 612"
Cohesion: 0.29
Nodes (6): AuthController.RevokeSessionAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 613 - "Community 613"
Cohesion: 0.29
Nodes (6): AuthController.SendOtpAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 614 - "Community 614"
Cohesion: 0.29
Nodes (6): AuthController.SendPhoneLoginOtpAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 615 - "Community 615"
Cohesion: 0.29
Nodes (6): AuthController.SetMfaStatusAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 616 - "Community 616"
Cohesion: 0.29
Nodes (6): AuthController.StartPhoneRegistrationAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 617 - "Community 617"
Cohesion: 0.29
Nodes (6): AuthorizationAdminController.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 618 - "Community 618"
Cohesion: 0.29
Nodes (6): AuthorizationAdminController.UpsertPlanPermissionsAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 619 - "Community 619"
Cohesion: 0.29
Nodes (6): AuthorizationAdminController.UpsertRolePermissionsAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 620 - "Community 620"
Cohesion: 0.29
Nodes (6): AuthorizationAdminController.UpsertUserPermissionGrantAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 621 - "Community 621"
Cohesion: 0.29
Nodes (6): AuthorizationAdminController.UpsertUserPermissionRevokeAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 622 - "Community 622"
Cohesion: 0.29
Nodes (6): AuthorizationAdminController.UpsertUserPlanSubscriptionAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 623 - "Community 623"
Cohesion: 0.29
Nodes (6): BaseAdminApiController.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 624 - "Community 624"
Cohesion: 0.29
Nodes (6): BaseAdminApiController, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 625 - "Community 625"
Cohesion: 0.29
Nodes (6): BaseCommonController.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 626 - "Community 626"
Cohesion: 0.29
Nodes (6): BaseThirdPartyController.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 627 - "Community 627"
Cohesion: 0.29
Nodes (6): BaseUserApiController.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 628 - "Community 628"
Cohesion: 0.29
Nodes (6): BaseUserApiController, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 629 - "Community 629"
Cohesion: 0.29
Nodes (6): Contains, CrudController.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 630 - "Community 630"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UsersController.AllCursorAsync()

### Community 631 - "Community 631"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UsersController.AllPaginatedAsync()

### Community 632 - "Community 632"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, UsersController.cs

### Community 633 - "Community 633"
Cohesion: 0.29
Nodes (6): AccountAuthorizationModels.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 634 - "Community 634"
Cohesion: 0.29
Nodes (6): AccountMembership.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 635 - "Community 635"
Cohesion: 0.29
Nodes (6): AccountMembershipConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 636 - "Community 636"
Cohesion: 0.29
Nodes (6): AccountMembershipGroup.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 637 - "Community 637"
Cohesion: 0.29
Nodes (6): AccountMembershipGroupConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 638 - "Community 638"
Cohesion: 0.29
Nodes (6): AccountMembershipPermission.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 639 - "Community 639"
Cohesion: 0.29
Nodes (6): AccountMembershipPermissionConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 640 - "Community 640"
Cohesion: 0.29
Nodes (6): AccountOrganization.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 641 - "Community 641"
Cohesion: 0.29
Nodes (6): AccountOrganizationConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 642 - "Community 642"
Cohesion: 0.29
Nodes (6): AccountOrganizationSecuritySettingConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 643 - "Community 643"
Cohesion: 0.29
Nodes (6): AccountPermissionConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 644 - "Community 644"
Cohesion: 0.29
Nodes (6): AccountPlanPermissionConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 645 - "Community 645"
Cohesion: 0.29
Nodes (6): AccountRolePermissionConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 646 - "Community 646"
Cohesion: 0.29
Nodes (6): AccountSecurityFeatureConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 647 - "Community 647"
Cohesion: 0.29
Nodes (6): AccountSecurityModels.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 648 - "Community 648"
Cohesion: 0.29
Nodes (6): AccountUserOauthIdentityConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 649 - "Community 649"
Cohesion: 0.29
Nodes (6): AccountUserPermissionGrantConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 650 - "Community 650"
Cohesion: 0.29
Nodes (6): AccountUserPermissionRevokeConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 651 - "Community 651"
Cohesion: 0.29
Nodes (6): AccountUserPlanSubscriptionConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 652 - "Community 652"
Cohesion: 0.29
Nodes (6): AccountUserSecuritySettingConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 653 - "Community 653"
Cohesion: 0.29
Nodes (6): AccountUserSession.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 654 - "Community 654"
Cohesion: 0.29
Nodes (6): AccountUserSessionConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 655 - "Community 655"
Cohesion: 0.29
Nodes (6): AccountUserTotpFactorConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 656 - "Community 656"
Cohesion: 0.29
Nodes (6): AccountUserVerification.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 657 - "Community 657"
Cohesion: 0.29
Nodes (6): AccountUserVerificationConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 658 - "Community 658"
Cohesion: 0.29
Nodes (6): BaseDocumentEntity.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 659 - "Community 659"
Cohesion: 0.29
Nodes (6): BaseEntity.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 660 - "Community 660"
Cohesion: 0.29
Nodes (6): Category.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 661 - "Community 661"
Cohesion: 0.29
Nodes (6): CategoryConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 662 - "Community 662"
Cohesion: 0.29
Nodes (6): ConfirmationCode.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 663 - "Community 663"
Cohesion: 0.29
Nodes (6): ConfirmationCodeConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 664 - "Community 664"
Cohesion: 0.29
Nodes (6): ConfirmationCodeConfiguration.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 665 - "Community 665"
Cohesion: 0.29
Nodes (6): CoreCountyConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 666 - "Community 666"
Cohesion: 0.29
Nodes (6): Contains, CoreGeo.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 667 - "Community 667"
Cohesion: 0.29
Nodes (6): CoreProvinceConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 668 - "Community 668"
Cohesion: 0.29
Nodes (6): CrmCommonConfiguration.Configure(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 669 - "Community 669"
Cohesion: 0.29
Nodes (6): Contains, CrmModels.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 670 - "Community 670"
Cohesion: 0.29
Nodes (6): Contains, Delete.cshtml.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 671 - "Community 671"
Cohesion: 0.29
Nodes (6): DeleteModel.OnGet(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 672 - "Community 672"
Cohesion: 0.29
Nodes (6): DeleteModel.OnPostAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 673 - "Community 673"
Cohesion: 0.29
Nodes (6): DeleteModel.ShowValue(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 674 - "Community 674"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Edit.cshtml.cs, Note, Upstream Relationships

### Community 675 - "Community 675"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, EditModel.IsForeignKey(), Method, Note, Upstream Relationships

### Community 676 - "Community 676"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, EditModel.OnPostAsync(), Method, Note, Upstream Relationships

### Community 677 - "Community 677"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Email.cs, Note, Upstream Relationships

### Community 678 - "Community 678"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, EmailConfiguration.Configure(), Method, Note, Upstream Relationships

### Community 679 - "Community 679"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, EmailConfiguration.cs, Note, Upstream Relationships

### Community 680 - "Community 680"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, EmailDocument.cs, Note, Upstream Relationships

### Community 681 - "Community 681"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, EmailDocumentConfiguration.Configure(), Method, Note, Upstream Relationships

### Community 682 - "Community 682"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, EmailDocumentConfiguration.cs, Note, Upstream Relationships

### Community 683 - "Community 683"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, EmailSharedInformation.cs, Note, Upstream Relationships

### Community 684 - "Community 684"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, EmailSharedInformationConfiguration.Configure(), Method, Note, Upstream Relationships

### Community 685 - "Community 685"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, EmailSharedInformationConfiguration.cs, Note, Upstream Relationships

### Community 686 - "Community 686"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Entities.Categories, Note, Upstream Relationships

### Community 687 - "Community 687"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Entities.Common, Note, Upstream Relationships

### Community 688 - "Community 688"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Entities.Items, Note, Upstream Relationships

### Community 689 - "Community 689"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Entities.Users (2), Note, Upstream Relationships

### Community 690 - "Community 690"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Entities.Users (3), Note, Upstream Relationships

### Community 691 - "Community 691"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Entities.Users, Note, Upstream Relationships

### Community 692 - "Community 692"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Index.cshtml.cs (2), Note, Upstream Relationships

### Community 693 - "Community 693"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IndexModel.OnGet() (2), Method, Note, Upstream Relationships

### Community 694 - "Community 694"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Item.cs, Note, Upstream Relationships

### Community 695 - "Community 695"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, LicenseBillingCycle.cs, Note, Upstream Relationships

### Community 696 - "Community 696"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, LicenseBillingCycleConfiguration.Configure(), Method, Note, Upstream Relationships

### Community 697 - "Community 697"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, LicenseModule.cs, Note, Upstream Relationships

### Community 698 - "Community 698"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, LicenseModuleConfiguration.Configure(), Method, Note, Upstream Relationships

### Community 699 - "Community 699"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, LicenseModulePermission.cs, Note, Upstream Relationships

### Community 700 - "Community 700"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, LicenseModulePermissionConfiguration.Configure(), Method, Note, Upstream Relationships

### Community 701 - "Community 701"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, LicensePlan.cs, Note, Upstream Relationships

### Community 702 - "Community 702"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, LicensePlanConfiguration.Configure(), Method, Note, Upstream Relationships

### Community 703 - "Community 703"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, LicensePlanModule.cs, Note, Upstream Relationships

### Community 704 - "Community 704"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, LicensePlanModuleConfiguration.Configure(), Method, Note, Upstream Relationships

### Community 705 - "Community 705"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, LicensePlanPrice.cs, Note, Upstream Relationships

### Community 706 - "Community 706"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, LicensePlanPriceConfiguration.Configure(), Method, Note, Upstream Relationships

### Community 707 - "Community 707"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, LicenseSubscription.cs, Note, Upstream Relationships

### Community 708 - "Community 708"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, LicenseSubscriptionConfiguration.Configure(), Method, Note, Upstream Relationships

### Community 709 - "Community 709"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, List.cshtml.cs, Note, Upstream Relationships

### Community 710 - "Community 710"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, ListModel.BuildKey(), Method, Note, Upstream Relationships

### Community 711 - "Community 711"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, ListModel.GetCell(), Method, Note, Upstream Relationships

### Community 712 - "Community 712"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, MenuExtraConfiguration.Configure(), Method, Note, Upstream Relationships

### Community 713 - "Community 713"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, MenuExtraModels.cs, Note, Upstream Relationships

### Community 714 - "Community 714"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, MenuUnit.cs, Note, Upstream Relationships

### Community 715 - "Community 715"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, MenuUnitConfiguration.Configure(), Method, Note, Upstream Relationships

### Community 716 - "Community 716"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, PaymentOperation.cs, Upstream Relationships

### Community 717 - "Community 717"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, PaymentOperationConfiguration.Configure(), Upstream Relationships

### Community 718 - "Community 718"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, PaymentWallet.cs, Upstream Relationships

### Community 719 - "Community 719"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, PaymentWalletConfiguration.Configure(), Upstream Relationships

### Community 720 - "Community 720"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, PaymentWalletEntry.cs, Upstream Relationships

### Community 721 - "Community 721"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, PaymentWalletEntryConfiguration.Configure(), Upstream Relationships

### Community 722 - "Community 722"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, PostConfiguration.Configure(), Upstream Relationships

### Community 723 - "Community 723"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Role.cs, Upstream Relationships

### Community 724 - "Community 724"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, RoleConfiguration.Configure(), Upstream Relationships

### Community 725 - "Community 725"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, RoleConfiguration.cs, Upstream Relationships

### Community 726 - "Community 726"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, RoleHelper.cs, Upstream Relationships

### Community 727 - "Community 727"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, SupportConfiguration.Configure(), Upstream Relationships

### Community 728 - "Community 728"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, SupportModels.cs, Upstream Relationships

### Community 729 - "Community 729"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, User.cs

### Community 730 - "Community 730"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UserConfiguration.Configure()

### Community 731 - "Community 731"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, UserConfiguration.cs

### Community 732 - "Community 732"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, UserRole.cs

### Community 733 - "Community 733"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UserRoleConfiguration.Configure()

### Community 734 - "Community 734"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, UserRoleConfiguration.cs

### Community 735 - "Community 735"
Cohesion: 0.29
Nodes (6): 20260506162935_Init.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 736 - "Community 736"
Cohesion: 0.29
Nodes (6): 20260506162935_Init.Designer.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 737 - "Community 737"
Cohesion: 0.29
Nodes (6): AbstractValidator, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 738 - "Community 738"
Cohesion: 0.29
Nodes (6): accounts.md, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 739 - "Community 739"
Cohesion: 0.29
Nodes (6): AccountsRequestsValidators.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 740 - "Community 740"
Cohesion: 0.29
Nodes (6): ActionFilterAttribute, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 741 - "Community 741"
Cohesion: 0.29
Nodes (6): Actors and Permissions (2), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 742 - "Community 742"
Cohesion: 0.29
Nodes (6): Actors and Permissions (3), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 743 - "Community 743"
Cohesion: 0.29
Nodes (6): Actors and Permissions (4), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 744 - "Community 744"
Cohesion: 0.29
Nodes (6): Actors and Permissions (5), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 745 - "Community 745"
Cohesion: 0.29
Nodes (6): Actors and Permissions (6), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 746 - "Community 746"
Cohesion: 0.29
Nodes (6): Actors and Permissions (7), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 747 - "Community 747"
Cohesion: 0.29
Nodes (6): Actors and Permissions (8), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 748 - "Community 748"
Cohesion: 0.29
Nodes (6): Actors and Permissions, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 749 - "Community 749"
Cohesion: 0.29
Nodes (6): AdminDTO.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 750 - "Community 750"
Cohesion: 0.29
Nodes (6): AdminDTO, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 751 - "Community 751"
Cohesion: 0.29
Nodes (6): AdminPanel.Pages (2), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 752 - "Community 752"
Cohesion: 0.29
Nodes (6): AdminPanel.Pages (3), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 753 - "Community 753"
Cohesion: 0.29
Nodes (6): AdminPanel.Pages, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 754 - "Community 754"
Cohesion: 0.29
Nodes (6): AdminPanelAuthenticationSettings (2), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 755 - "Community 755"
Cohesion: 0.29
Nodes (6): AdminPanelAuthenticationSettings, Dependencies, Downstream Relationships, Note, References, Upstream Relationships

### Community 756 - "Community 756"
Cohesion: 0.29
Nodes (6): AdminPanelAuthService.SignOutAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 757 - "Community 757"
Cohesion: 0.29
Nodes (6): AdminPanelSecuritySettings.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 758 - "Community 758"
Cohesion: 0.29
Nodes (6): AdminPanelServiceCollectionExtensions.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 759 - "Community 759"
Cohesion: 0.29
Nodes (6): AdminPanelServiceCollectionExtensions.RegisterByMarker(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 760 - "Community 760"
Cohesion: 0.29
Nodes (6): Agent Output Format (for PRs-Task reports), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 761 - "Community 761"
Cohesion: 0.29
Nodes (6): AGENTS.fa.md, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 762 - "Community 762"
Cohesion: 0.29
Nodes (6): AGENTS.md, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 763 - "Community 763"
Cohesion: 0.29
Nodes (6): AGENTS.override.md (2), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 764 - "Community 764"
Cohesion: 0.29
Nodes (6): AGENTS.override.md, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 765 - "Community 765"
Cohesion: 0.29
Nodes (6): ApiCustomAuthorize.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 766 - "Community 766"
Cohesion: 0.29
Nodes (6): ApiPermissionAuthorizeAttribute.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 767 - "Community 767"
Cohesion: 0.29
Nodes (6): ApiPermissionAuthorizeAttribute.OnActionExecutionAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 768 - "Community 768"
Cohesion: 0.29
Nodes (6): ApiResult.CastToData(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 769 - "Community 769"
Cohesion: 0.29
Nodes (6): ApiResult.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 770 - "Community 770"
Cohesion: 0.29
Nodes (6): ApiResultFilterAttribute.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 771 - "Community 771"
Cohesion: 0.29
Nodes (6): ApiResultFilterAttribute.OnResultExecuting(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 772 - "Community 772"
Cohesion: 0.29
Nodes (6): AppException (2), Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 773 - "Community 773"
Cohesion: 0.29
Nodes (6): AppException.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 774 - "Community 774"
Cohesion: 0.29
Nodes (6): ApplicationBuilderExtensions.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 775 - "Community 775"
Cohesion: 0.29
Nodes (6): ApplicationBuilderExtensions.HandleRecurringJobs(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 776 - "Community 776"
Cohesion: 0.29
Nodes (6): ApplicationBuilderExtensions.InitializeDatabase(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 777 - "Community 777"
Cohesion: 0.29
Nodes (6): ApplicationBuilderExtensions.UseCustomExceptionHandler(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 778 - "Community 778"
Cohesion: 0.29
Nodes (6): ApplicationBuilderExtensions.UseCustomStaticFiles(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 779 - "Community 779"
Cohesion: 0.29
Nodes (6): ApplicationBuilderExtensions.UseHsts(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 780 - "Community 780"
Cohesion: 0.29
Nodes (6): ApplicationBuilderExtensions.UsePerformancePipeline(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 781 - "Community 781"
Cohesion: 0.29
Nodes (6): ApplicationBuilderExtensions.UseRequestContextEnrichment(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 782 - "Community 782"
Cohesion: 0.29
Nodes (6): ApplicationDbContext.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 783 - "Community 783"
Cohesion: 0.29
Nodes (6): ApplicationDbContext, Dependencies, Downstream Relationships, Note, References, Upstream Relationships

### Community 784 - "Community 784"
Cohesion: 0.29
Nodes (6): ApplicationDbContext.OnModelCreating(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 785 - "Community 785"
Cohesion: 0.29
Nodes (6): ApplicationDbContextModelSnapshot.BuildModel(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 786 - "Community 786"
Cohesion: 0.29
Nodes (6): ApplicationDbContextModelSnapshot.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 787 - "Community 787"
Cohesion: 0.29
Nodes (6): ApplicationDefaultSettings.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 788 - "Community 788"
Cohesion: 0.29
Nodes (6): ApplicationMessages.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 789 - "Community 789"
Cohesion: 0.29
Nodes (6): ApplicationPropertyPersianName.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 790 - "Community 790"
Cohesion: 0.29
Nodes (6): ApplySummariesOperationFilter.Apply(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 791 - "Community 791"
Cohesion: 0.29
Nodes (6): ApplySummariesOperationFilter.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 792 - "Community 792"
Cohesion: 0.29
Nodes (6): AppTelemetry.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 793 - "Community 793"
Cohesion: 0.29
Nodes (6): AppTelemetry.Dispose(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 794 - "Community 794"
Cohesion: 0.29
Nodes (6): AssemblyHelper.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 795 - "Community 795"
Cohesion: 0.29
Nodes (6): Assert.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 796 - "Community 796"
Cohesion: 0.29
Nodes (6): Assert.NotEmpty(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 797 - "Community 797"
Cohesion: 0.29
Nodes (6): Assert.NotNull(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 798 - "Community 798"
Cohesion: 0.29
Nodes (6): Attribute, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 799 - "Community 799"
Cohesion: 0.29
Nodes (6): auth.md, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 800 - "Community 800"
Cohesion: 0.29
Nodes (6): Authentication Change Checklist, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 801 - "Community 801"
Cohesion: 0.29
Nodes (6): AuthorizationDataInitializer.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 802 - "Community 802"
Cohesion: 0.29
Nodes (6): AutofacModule.Load(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 803 - "Community 803"
Cohesion: 0.29
Nodes (6): AutoMapperConfiguration.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 804 - "Community 804"
Cohesion: 0.29
Nodes (6): BadRequestException.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 805 - "Community 805"
Cohesion: 0.29
Nodes (6): BankInfo.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 806 - "Community 806"
Cohesion: 0.29
Nodes (6): BankInfo, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 807 - "Community 807"
Cohesion: 0.29
Nodes (6): BaseDocumentEntity (2), Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 808 - "Community 808"
Cohesion: 0.29
Nodes (6): BaseDto.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 809 - "Community 809"
Cohesion: 0.29
Nodes (6): BaseDTO.FromEntity(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 810 - "Community 810"
Cohesion: 0.29
Nodes (6): BaseEntity, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 811 - "Community 811"
Cohesion: 0.29
Nodes (6): BaseValidationResultDTO.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 812 - "Community 812"
Cohesion: 0.29
Nodes (6): BaseValidationResultDTO, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 813 - "Community 813"
Cohesion: 0.29
Nodes (6): bool, Dependencies, Downstream Relationships, Note, References, Upstream Relationships

### Community 814 - "Community 814"
Cohesion: 0.29
Nodes (6): BoolResultDto, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 815 - "Community 815"
Cohesion: 0.29
Nodes (6): Business Requirements Source, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 816 - "Community 816"
Cohesion: 0.29
Nodes (6): Business Rules (2), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 817 - "Community 817"
Cohesion: 0.29
Nodes (6): Business Rules (3), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 818 - "Community 818"
Cohesion: 0.29
Nodes (6): Business Rules (4), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 819 - "Community 819"
Cohesion: 0.29
Nodes (6): Business Rules (5), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 820 - "Community 820"
Cohesion: 0.29
Nodes (6): Business Rules (6), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 821 - "Community 821"
Cohesion: 0.29
Nodes (6): Business Rules (7), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 822 - "Community 822"
Cohesion: 0.29
Nodes (6): Business Rules (8), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 823 - "Community 823"
Cohesion: 0.29
Nodes (6): Business Rules, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 824 - "Community 824"
Cohesion: 0.29
Nodes (6): byte, Dependencies, Downstream Relationships, Note, References, Upstream Relationships

### Community 825 - "Community 825"
Cohesion: 0.29
Nodes (6): CachingSettings, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 826 - "Community 826"
Cohesion: 0.29
Nodes (6): Calculator.CalculateExtraFundPercent(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 827 - "Community 827"
Cohesion: 0.29
Nodes (6): Calculator.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 828 - "Community 828"
Cohesion: 0.29
Nodes (6): CaptchaHelper.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 829 - "Community 829"
Cohesion: 0.29
Nodes (6): CaptchaHelper.GoogleCaptchaValidation(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 830 - "Community 830"
Cohesion: 0.29
Nodes (6): Change Safety Rules, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 831 - "Community 831"
Cohesion: 0.29
Nodes (6): Check-WarningBaseline.ps1, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 832 - "Community 832"
Cohesion: 0.29
Nodes (6): code powershell (powershell -ExecutionPolicy Bypass -File .-tools-Check-Warni) (2), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 833 - "Community 833"
Cohesion: 0.29
Nodes (6): code powershell (powershell -ExecutionPolicy Bypass -File .-tools-Check-Warni) (3), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 834 - "Community 834"
Cohesion: 0.29
Nodes (6): code powershell (powershell -ExecutionPolicy Bypass -File .-tools-Check-Warni), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 835 - "Community 835"
Cohesion: 0.29
Nodes (6): CodeGenerator.CreateCodeVerifier(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 836 - "Community 836"
Cohesion: 0.29
Nodes (6): CodeGenerator.GenerateCodeChallenge(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 837 - "Community 837"
Cohesion: 0.29
Nodes (6): CodeGenerator.GenerateRandomNumber(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 838 - "Community 838"
Cohesion: 0.29
Nodes (6): CodeGenerator.GenerateRandomString(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 839 - "Community 839"
Cohesion: 0.29
Nodes (6): Codex Customization (Applied), Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 840 - "Community 840"
Cohesion: 0.29
Nodes (6): ConcurrentDictionary, Dependencies, Downstream Relationships, Note, References, Upstream Relationships

### Community 841 - "Community 841"
Cohesion: 0.29
Nodes (6): ControllerBase, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 842 - "Community 842"
Cohesion: 0.29
Nodes (6): Contains, CookieManager.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 843 - "Community 843"
Cohesion: 0.29
Nodes (6): CookieManager.Get(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 844 - "Community 844"
Cohesion: 0.29
Nodes (6): CookieManager.Set(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 845 - "Community 845"
Cohesion: 0.29
Nodes (6): Contains, Core Use Cases (2), Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 846 - "Community 846"
Cohesion: 0.29
Nodes (6): Contains, Core Use Cases (3), Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 847 - "Community 847"
Cohesion: 0.29
Nodes (6): Contains, Core Use Cases (4), Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 848 - "Community 848"
Cohesion: 0.29
Nodes (6): Contains, Core Use Cases (5), Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 849 - "Community 849"
Cohesion: 0.29
Nodes (6): Contains, Core Use Cases (6), Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 850 - "Community 850"
Cohesion: 0.29
Nodes (6): Contains, Core Use Cases (7), Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 851 - "Community 851"
Cohesion: 0.29
Nodes (6): Contains, Core Use Cases (8), Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 852 - "Community 852"
Cohesion: 0.29
Nodes (6): Contains, Core Use Cases, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 853 - "Community 853"
Cohesion: 0.29
Nodes (6): Contains, CreateAdminRequest.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 854 - "Community 854"
Cohesion: 0.29
Nodes (6): Contains, CreateAdminRequest, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 855 - "Community 855"
Cohesion: 0.29
Nodes (6): Contains, crm.md, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 856 - "Community 856"
Cohesion: 0.29
Nodes (6): CrudController.Create(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 857 - "Community 857"
Cohesion: 0.29
Nodes (6): CrudController.Select(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 858 - "Community 858"
Cohesion: 0.29
Nodes (6): CrudController.Update(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 859 - "Community 859"
Cohesion: 0.29
Nodes (6): CrudController.UpdateCustomProperties(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 860 - "Community 860"
Cohesion: 0.29
Nodes (6): CrudController.UpdateProperty(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 861 - "Community 861"
Cohesion: 0.29
Nodes (6): Contains, CursorPagingDTO.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 862 - "Community 862"
Cohesion: 0.29
Nodes (6): Contains, CursorPagingDTO, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 863 - "Community 863"
Cohesion: 0.29
Nodes (6): CursorPagingRequest (2), Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 864 - "Community 864"
Cohesion: 0.29
Nodes (6): Contains, CursorPagingRequest.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 865 - "Community 865"
Cohesion: 0.29
Nodes (6): Contains, CursorPagingRequest, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 866 - "Community 866"
Cohesion: 0.29
Nodes (6): Contains, CustomExceptionHandlerMiddleware.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 867 - "Community 867"
Cohesion: 0.29
Nodes (6): CustomExceptionHandlerMiddleware.Invoke(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 868 - "Community 868"
Cohesion: 0.29
Nodes (6): Contains, CustomMappingProfile.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 869 - "Community 869"
Cohesion: 0.29
Nodes (6): Contains, Data Contracts (2), Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 870 - "Community 870"
Cohesion: 0.29
Nodes (6): Contains, Data Contracts (3), Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 871 - "Community 871"
Cohesion: 0.29
Nodes (6): Contains, Data Contracts (4), Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 872 - "Community 872"
Cohesion: 0.29
Nodes (6): Contains, Data Contracts (5), Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 873 - "Community 873"
Cohesion: 0.29
Nodes (6): Contains, Data Contracts (6), Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 874 - "Community 874"
Cohesion: 0.29
Nodes (6): Contains, Data Contracts (7), Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 875 - "Community 875"
Cohesion: 0.29
Nodes (6): Contains, Data Contracts (8), Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 876 - "Community 876"
Cohesion: 0.29
Nodes (6): Contains, Data Contracts, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 877 - "Community 877"
Cohesion: 0.29
Nodes (6): Contains, Data, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 878 - "Community 878"
Cohesion: 0.29
Nodes (6): Contains, Data.Migrations (2), Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 879 - "Community 879"
Cohesion: 0.29
Nodes (6): Contains, Data.Migrations (3), Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 880 - "Community 880"
Cohesion: 0.29
Nodes (6): Contains, Data.Migrations, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 881 - "Community 881"
Cohesion: 0.29
Nodes (6): Contains, DatabaseSqlObjectsInstaller.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 882 - "Community 882"
Cohesion: 0.29
Nodes (6): Contains, DataInitilizaerDefaultValues.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 883 - "Community 883"
Cohesion: 0.29
Nodes (6): DateTime, Dependencies, Downstream Relationships, Note, References, Upstream Relationships

### Community 884 - "Community 884"
Cohesion: 0.29
Nodes (6): Contains, DateTimeExtensions.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 885 - "Community 885"
Cohesion: 0.29
Nodes (6): DateTimeExtensions.GregorianToShamsiDatePickerValue(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 886 - "Community 886"
Cohesion: 0.29
Nodes (6): DateTimeHelper.ConvertDatetimeToUnixTimeStamp(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 887 - "Community 887"
Cohesion: 0.29
Nodes (6): Contains, DateTimeHelper.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 888 - "Community 888"
Cohesion: 0.29
Nodes (6): DateTimeHelper.DiffDate(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 889 - "Community 889"
Cohesion: 0.29
Nodes (6): DateTimeHelper.GetMonths(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 890 - "Community 890"
Cohesion: 0.29
Nodes (6): DateTimeHelper.GetShamsiMonth(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 891 - "Community 891"
Cohesion: 0.29
Nodes (6): DateTimeHelper.GetWeekOfYear(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 892 - "Community 892"
Cohesion: 0.29
Nodes (6): DateTimeHelper.GoToFirstDayOfShamsiMonth(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 893 - "Community 893"
Cohesion: 0.29
Nodes (6): DateTimeHelper.GoToLastDayOfShamsiMonth(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 894 - "Community 894"
Cohesion: 0.29
Nodes (6): DateTimeHelper.GregorianToShamsi(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 895 - "Community 895"
Cohesion: 0.29
Nodes (6): DateTimeHelper.ShamsiToGregorian(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 896 - "Community 896"
Cohesion: 0.29
Nodes (6): DateTimeHelper.UnixTimeStampToDateTime(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 897 - "Community 897"
Cohesion: 0.29
Nodes (6): DbCommandInterceptor, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 898 - "Community 898"
Cohesion: 0.29
Nodes (6): DefaultContractResolver, Dependencies, Downstream Relationships, Inherits, Note, Upstream Relationships

### Community 899 - "Community 899"
Cohesion: 0.29
Nodes (6): Contains, Definition of Done, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 900 - "Community 900"
Cohesion: 0.29
Nodes (6): Dependencies, Dictionary, Downstream Relationships, Note, References, Upstream Relationships

### Community 901 - "Community 901"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, digital-menu.md, Downstream Relationships, Note, Upstream Relationships

### Community 902 - "Community 902"
Cohesion: 0.29
Nodes (6): Dependencies, DistributedCacheHealthCheck.CheckHealthAsync(), Downstream Relationships, Method, Note, Upstream Relationships

### Community 903 - "Community 903"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, DistributedCacheHealthCheck.cs, Downstream Relationships, Note, Upstream Relationships

### Community 904 - "Community 904"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Domains, Downstream Relationships, Note, Upstream Relationships

### Community 905 - "Community 905"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Done Criteria (Definition of Done), Downstream Relationships, Note, Upstream Relationships

### Community 906 - "Community 906"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, EditAdminDTO.cs, Note, Upstream Relationships

### Community 907 - "Community 907"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, EditAdminDTO, Note, Upstream Relationships

### Community 908 - "Community 908"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Editing Rules, Note, Upstream Relationships

### Community 909 - "Community 909"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, EmailReceiverDTO.CreateMappings(), Method, Note, Upstream Relationships

### Community 910 - "Community 910"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, EmailReceiverDTO.cs, Note, Upstream Relationships

### Community 911 - "Community 911"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, EmailReceiverWithIdDTO.cs, Note, Upstream Relationships

### Community 912 - "Community 912"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, EntityAdminService.GetManagedEntities(), Method, Note, Upstream Relationships

### Community 913 - "Community 913"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, EnumExcentions.cs, Note, Upstream Relationships

### Community 914 - "Community 914"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, EnumExtensions.GetEnumFlags(), Method, Note, Upstream Relationships

### Community 915 - "Community 915"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, EnumExtensions.GetEnumValues(), Method, Note, Upstream Relationships

### Community 916 - "Community 916"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, EnumExtensions.ParseEnum(), Method, Note, Upstream Relationships

### Community 917 - "Community 917"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, EnumExtensions.ToEnum(), Method, Note, Upstream Relationships

### Community 918 - "Community 918"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Error.cshtml.cs, Note, Upstream Relationships

### Community 919 - "Community 919"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, ErrorModel.OnGet(), Method, Note, Upstream Relationships

### Community 920 - "Community 920"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Events and Side Effects (2), Note, Upstream Relationships

### Community 921 - "Community 921"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Events and Side Effects (3), Note, Upstream Relationships

### Community 922 - "Community 922"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Events and Side Effects (4), Note, Upstream Relationships

### Community 923 - "Community 923"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Events and Side Effects (5), Note, Upstream Relationships

### Community 924 - "Community 924"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Events and Side Effects (6), Note, Upstream Relationships

### Community 925 - "Community 925"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Events and Side Effects (7), Note, Upstream Relationships

### Community 926 - "Community 926"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Events and Side Effects (8), Note, Upstream Relationships

### Community 927 - "Community 927"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Events and Side Effects, Note, Upstream Relationships

### Community 928 - "Community 928"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Example A Add a new use-case (feature), Note, Upstream Relationships

### Community 929 - "Community 929"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Example B Add a new entity field persisted in DB, Note, Upstream Relationships

### Community 930 - "Community 930"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Example C Authentication-Authorization changes, Note, Upstream Relationships

### Community 931 - "Community 931"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Exception, Inherits, Note, Upstream Relationships

### Community 932 - "Community 932"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Failed(), Note, Upstream Relationships

### Community 933 - "Community 933"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, File Ownership By Concern, Note, Upstream Relationships

### Community 934 - "Community 934"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, FilterUserCursorRequest.cs, Note, Upstream Relationships

### Community 935 - "Community 935"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, FilterUserRequest.cs, Note, Upstream Relationships

### Community 936 - "Community 936"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, FluentErrorResponseBuilder.BuildValidationErrors(), Method, Note, Upstream Relationships

### Community 937 - "Community 937"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, FluentErrorResponseBuilder.cs, Note, Upstream Relationships

### Community 938 - "Community 938"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, FluentValidationExtensions.cs, Note, Upstream Relationships

### Community 939 - "Community 939"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, FluentValidationExtensions.GetRequestType(), Method, Note, Upstream Relationships

### Community 940 - "Community 940"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, FluentValidationExtensions.MustExistAsync(), Method, Note, Upstream Relationships

### Community 941 - "Community 941"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, FluentValidationExtensions.MustNotExistAsync(), Method, Note, Upstream Relationships

### Community 942 - "Community 942"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, FluentValidationExtensions.NotNullAndEmpty(), Method, Note, Upstream Relationships

### Community 943 - "Community 943"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, FluentValidationExtensions.ValidateNationalCode(), Method, Note, Upstream Relationships

### Community 944 - "Community 944"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, FluentValidationExtensions.ValidatePassword(), Method, Note, Upstream Relationships

### Community 945 - "Community 945"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, FluentValidationExtensions.ValidatePhoneNumber(), Method, Note, Upstream Relationships

### Community 946 - "Community 946"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, FluentValidationFilterAttribute.cs, Note, Upstream Relationships

### Community 947 - "Community 947"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, FluentValidationFilterAttribute.OnActionExecutionAsync(), Method, Note, Upstream Relationships

### Community 948 - "Community 948"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Func, Note, References, Upstream Relationships

### Community 949 - "Community 949"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, GetAdminsPaginatedRequest.cs, Note, Upstream Relationships

### Community 950 - "Community 950"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, GetObjectReply.cs, Note, Upstream Relationships

### Community 951 - "Community 951"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, GoogleRecaptchaResponse, Note, Upstream Relationships

### Community 952 - "Community 952"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Hierarchical Instruction Files, Note, Upstream Relationships

### Community 953 - "Community 953"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, How To Use, Note, Upstream Relationships

### Community 954 - "Community 954"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, HttpRequestExtensions.cs, Note, Upstream Relationships

### Community 955 - "Community 955"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, HttpRequestExtensions.IsAjax(), Method, Note, Upstream Relationships

### Community 956 - "Community 956"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAdminPanelAuthService.SignInWithEmailPasswordAsync(), Method, Note, Upstream Relationships

### Community 957 - "Community 957"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAdminPanelAuthService.SignOutAsync(), Method, Note, Upstream Relationships

### Community 958 - "Community 958"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAppCacheService.GetAsync(), Method, Note, Upstream Relationships

### Community 959 - "Community 959"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAppCacheService.RemoveAsync(), Method, Note, Upstream Relationships

### Community 960 - "Community 960"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAppCacheService.SetAsync(), Method, Note, Upstream Relationships

### Community 961 - "Community 961"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAsyncActionFilter, Inherits, Note, Upstream Relationships

### Community 962 - "Community 962"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.CompletePhoneRegistrationAsync(), Method, Note, Upstream Relationships

### Community 963 - "Community 963"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.ConfirmEmailRegistrationAsync(), Method, Note, Upstream Relationships

### Community 964 - "Community 964"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.EmailPasswordLoginAsync(), Method, Note, Upstream Relationships

### Community 965 - "Community 965"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.ForgotPasswordAsync(), Method, Note, Upstream Relationships

### Community 966 - "Community 966"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.GetMfaStatusAsync(), Method, Note, Upstream Relationships

### Community 967 - "Community 967"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.GetMyAuthorizationAsync(), Method, Note, Upstream Relationships

### Community 968 - "Community 968"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.GetMySessionsAsync(), Method, Note, Upstream Relationships

### Community 969 - "Community 969"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.OtpLoginAsync(), Method, Note, Upstream Relationships

### Community 970 - "Community 970"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.PasswordLoginAsync(), Method, Note, Upstream Relationships

### Community 971 - "Community 971"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.PhoneOtpLoginAsync(), Method, Note, Upstream Relationships

### Community 972 - "Community 972"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.RegisterWithEmailAsync(), Method, Note, Upstream Relationships

### Community 973 - "Community 973"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.ResendEmailActivationLinkAsync(), Method, Note, Upstream Relationships

### Community 974 - "Community 974"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.ResendPhoneRegistrationOtpAsync(), Method, Note, Upstream Relationships

### Community 975 - "Community 975"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.ResetPasswordWithOtpAsync(), Method, Note, Upstream Relationships

### Community 976 - "Community 976"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.RevokeOtherSessionsAsync(), Method, Note, Upstream Relationships

### Community 977 - "Community 977"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.RevokeSessionAsync(), Method, Note, Upstream Relationships

### Community 978 - "Community 978"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.SendOtpAsync(), Method, Note, Upstream Relationships

### Community 979 - "Community 979"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.SendPhoneLoginOtpAsync(), Method, Note, Upstream Relationships

### Community 980 - "Community 980"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.SetMfaStatusAsync(), Method, Note, Upstream Relationships

### Community 981 - "Community 981"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.SignOutAsync(), Method, Note, Upstream Relationships

### Community 982 - "Community 982"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.StartPhoneRegistrationAsync(), Method, Note, Upstream Relationships

### Community 983 - "Community 983"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService.UpdatePasswordAsync(), Method, Note, Upstream Relationships

### Community 984 - "Community 984"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthorizationManagementService.UpsertPlanPermissionsAsync(), Method, Note, Upstream Relationships

### Community 985 - "Community 985"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthorizationManagementService.UpsertRolePermissionsAsync(), Method, Note, Upstream Relationships

### Community 986 - "Community 986"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthorizationManagementService.UpsertUserPermissionGrantAsync(), Method, Note, Upstream Relationships

### Community 987 - "Community 987"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthorizationManagementService.UpsertUserPermissionRevokeAsync(), Method, Note, Upstream Relationships

### Community 988 - "Community 988"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthorizationManagementService.UpsertUserPlanSubscriptionAsync(), Method, Note, Upstream Relationships

### Community 989 - "Community 989"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IBaseDTO (2), Inherits, Note, Upstream Relationships

### Community 990 - "Community 990"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IBaseDTO.cs, Note, Upstream Relationships

### Community 991 - "Community 991"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IBaseDTO, Note, Upstream Relationships

### Community 992 - "Community 992"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IDataInitializer.cs, Note, Upstream Relationships

### Community 993 - "Community 993"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IDataInitializer.InitializeData(), Method, Note, Upstream Relationships

### Community 994 - "Community 994"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IDataInitializer, Inherits, Note, Upstream Relationships

### Community 995 - "Community 995"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IdentityConfigurationExtensions.AddCustomIdentity(), Method, Note, Upstream Relationships

### Community 996 - "Community 996"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IdentityConfigurationExtensions.cs, Note, Upstream Relationships

### Community 997 - "Community 997"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IdentityDbContext, Inherits, Note, Upstream Relationships

### Community 998 - "Community 998"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IdentityExtensions.cs, Note, Upstream Relationships

### Community 999 - "Community 999"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IdentityExtensions.GetRoles(), Method, Note, Upstream Relationships

### Community 1000 - "Community 1000"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IdentityRole, Inherits, Note, Upstream Relationships

### Community 1001 - "Community 1001"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IdentitySettings, Note, Upstream Relationships

### Community 1002 - "Community 1002"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IdentityUser, Inherits, Note, Upstream Relationships

### Community 1003 - "Community 1003"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IdentityUserRole, Inherits, Note, Upstream Relationships

### Community 1004 - "Community 1004"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IDisposable, Inherits, Note, Upstream Relationships

### Community 1005 - "Community 1005"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IDocumentFilter, Inherits, Note, Upstream Relationships

### Community 1006 - "Community 1006"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IEffectiveAuthorizationService.GetEffectiveAuthorizationAsync(), Method, Note, Upstream Relationships

### Community 1007 - "Community 1007"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IEffectiveAuthorizationService.InvalidateUserAuthorizationAsync(), Method, Note, Upstream Relationships

### Community 1008 - "Community 1008"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IEntity, Inherits, Note, Upstream Relationships

### Community 1009 - "Community 1009"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IEntityAdminService.BuildKeyToken(), Method, Note, Upstream Relationships

### Community 1010 - "Community 1010"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IEntityAdminService.Count(), Method, Note, Upstream Relationships

### Community 1011 - "Community 1011"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IEntityAdminService.DeleteAsync(), Method, Note, Upstream Relationships

### Community 1012 - "Community 1012"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IEntityAdminService.FindByKey(), Method, Note, Upstream Relationships

### Community 1013 - "Community 1013"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IEntityAdminService.GetForeignKeyOptionsAsync(), Method, Note, Upstream Relationships

### Community 1014 - "Community 1014"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IEntityAdminService.GetManagedEntities(), Method, Note, Upstream Relationships

### Community 1015 - "Community 1015"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IEntityAdminService.GetManagedEntity(), Method, Note, Upstream Relationships

### Community 1016 - "Community 1016"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IEntityAdminService.GetPage(), Method, Note, Upstream Relationships

### Community 1017 - "Community 1017"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IEntityAdminService.SaveAsync(), Method, Note, Upstream Relationships

### Community 1018 - "Community 1018"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IEntityTypeConfiguration, Inherits, Note, Upstream Relationships

### Community 1019 - "Community 1019"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IHaveCustomMapping (2), Inherits, Note, Upstream Relationships

### Community 1020 - "Community 1020"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IHaveCustomMapping.CreateMappings(), Method, Note, Upstream Relationships

### Community 1021 - "Community 1021"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IHaveCustomMapping.cs, Note, Upstream Relationships

### Community 1022 - "Community 1022"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IHealthCheck, Inherits, Note, Upstream Relationships

### Community 1023 - "Community 1023"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IJwtService.GenerateAsync(), Method, Note, Upstream Relationships

### Community 1024 - "Community 1024"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IJwtService.Validate(), Method, Note, Upstream Relationships

### Community 1025 - "Community 1025"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, ILogger, Note, References, Upstream Relationships

### Community 1026 - "Community 1026"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IMapper, Note, References, Upstream Relationships

### Community 1027 - "Community 1027"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Implementation Rules, Note, Upstream Relationships

### Community 1028 - "Community 1028"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Index.cshtml.cs (3), Note, Upstream Relationships

### Community 1029 - "Community 1029"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Index.cshtml.cs, Note, Upstream Relationships

### Community 1030 - "Community 1030"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IndexModel.OnGet() (3), Method, Note, Upstream Relationships

### Community 1031 - "Community 1031"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IndexModel.OnGet(), Method, Note, Upstream Relationships

### Community 1032 - "Community 1032"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Init.BuildTargetModel(), Method, Note, Upstream Relationships

### Community 1033 - "Community 1033"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Init.Down(), Method, Note, Upstream Relationships

### Community 1034 - "Community 1034"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Init.Up(), Method, Note, Upstream Relationships

### Community 1035 - "Community 1035"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, int, Note, References, Upstream Relationships

### Community 1036 - "Community 1036"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Invoke.cshtml.cs, Note, Upstream Relationships

### Community 1037 - "Community 1037"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, InvokeModel.GetValue(), Method, Note, Upstream Relationships

### Community 1038 - "Community 1038"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, InvokeModel.OnGet(), Method, Note, Upstream Relationships

### Community 1039 - "Community 1039"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, InvokeModel.OnPostAsync(), Method, Note, Upstream Relationships

### Community 1040 - "Community 1040"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IOperationCatalogService.GetOperation(), Method, Note, Upstream Relationships

### Community 1041 - "Community 1041"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IOperationCatalogService.GetOperations(), Method, Note, Upstream Relationships

### Community 1042 - "Community 1042"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IOperationCatalogService.InvokeAsync(), Method, Note, Upstream Relationships

### Community 1043 - "Community 1043"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Inherits, IOperationFilter, Note, Upstream Relationships

### Community 1044 - "Community 1044"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IranianBankInfo.cs, Note, Upstream Relationships

### Community 1045 - "Community 1045"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IranianBankInfo, Note, Upstream Relationships

### Community 1046 - "Community 1046"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IReadOnlyList, Note, References, Upstream Relationships

### Community 1047 - "Community 1047"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRoleService.AddRolesAsync(), Method, Note, Upstream Relationships

### Community 1048 - "Community 1048"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Inherits, IScopedDependency (2), Note, Upstream Relationships

### Community 1049 - "Community 1049"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IScopedDependency.cs, Note, Upstream Relationships

### Community 1050 - "Community 1050"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IScopedDependency, Note, Upstream Relationships

### Community 1051 - "Community 1051"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, ISenderService.SendEmailAsync(), Method, Note, Upstream Relationships

### Community 1052 - "Community 1052"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, ISenderService.SendEmailWithAttachmentAsync(), Method, Note, Upstream Relationships

### Community 1053 - "Community 1053"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, ISenderService.SendGeneralSmsAsync(), Method, Note, Upstream Relationships

### Community 1054 - "Community 1054"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, ISenderService.SendOtpSmsAsync(), Method, Note, Upstream Relationships

### Community 1055 - "Community 1055"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, ISenderService.SendTestSmsAsync(), Method, Note, Upstream Relationships

### Community 1056 - "Community 1056"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, ISingletonDependency, Note, Upstream Relationships

### Community 1057 - "Community 1057"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, ITransientDependency, Note, Upstream Relationships

### Community 1058 - "Community 1058"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IUserContext.cs, Note, Upstream Relationships

### Community 1059 - "Community 1059"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IUserContext, Note, Upstream Relationships

### Community 1060 - "Community 1060"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IUserService.All(), Method, Note, Upstream Relationships

### Community 1061 - "Community 1061"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IUserService.AllByCursor(), Method, Note, Upstream Relationships

### Community 1062 - "Community 1062"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IWebHostEnvironment, Note, References, Upstream Relationships

### Community 1063 - "Community 1063"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, JsonSerializerOptions, Note, References, Upstream Relationships

### Community 1064 - "Community 1064"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, JwtSettings, Note, Upstream Relationships

### Community 1065 - "Community 1065"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Language Versions, Note, Upstream Relationships

### Community 1066 - "Community 1066"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, licenses.md, Note, Upstream Relationships

### Community 1067 - "Community 1067"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, List, Note, References, Upstream Relationships

### Community 1068 - "Community 1068"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Local Goals, Note, Upstream Relationships

### Community 1069 - "Community 1069"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Local Rules, Note, Upstream Relationships

### Community 1070 - "Community 1070"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, LogicException.cs, Note, Upstream Relationships

### Community 1071 - "Community 1071"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Login.cshtml.cs, Note, Upstream Relationships

### Community 1072 - "Community 1072"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, LoginInput, Note, Upstream Relationships

### Community 1073 - "Community 1073"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, LoginModel.OnGet(), Method, Note, Upstream Relationships

### Community 1074 - "Community 1074"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, LoginModel.OnPostAsync(), Method, Note, Upstream Relationships

### Community 1075 - "Community 1075"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Logout.cshtml.cs, Note, Upstream Relationships

### Community 1076 - "Community 1076"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, LogoutModel.OnPostAsync(), Method, Note, Upstream Relationships

### Community 1077 - "Community 1077"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, long, Note, References, Upstream Relationships

### Community 1078 - "Community 1078"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, LowercaseContractResolver.ResolvePropertyName(), Method, Note, Upstream Relationships

### Community 1079 - "Community 1079"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, MailRequestDTO.cs, Note, Upstream Relationships

### Community 1080 - "Community 1080"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, MailRequestDTO, Note, Upstream Relationships

### Community 1081 - "Community 1081"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, MailSettings, Note, Upstream Relationships

### Community 1082 - "Community 1082"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Mandatory HTML Documentation Versioning Rules, Note, Upstream Relationships

### Community 1083 - "Community 1083"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, menu.md, Note, Upstream Relationships

### Community 1084 - "Community 1084"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, MessageBuilder.CreateArgumentGraterThanAcceptedRangeErrorMessage(), Method, Note, Upstream Relationships

### Community 1085 - "Community 1085"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, MessageBuilder.CreateArgumentLessThanAcceptedRangeErrorMessage(), Method, Note, Upstream Relationships

### Community 1086 - "Community 1086"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, MessageBuilder.CreateDuplicateErrorMessage(), Method, Note, Upstream Relationships

### Community 1087 - "Community 1087"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, MessageBuilder.CreateExpiredErrorMessage(), Method, Note, Upstream Relationships

### Community 1088 - "Community 1088"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, MessageBuilder.CreateInvalidErrorMessage(), Method, Note, Upstream Relationships

### Community 1089 - "Community 1089"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, MessageBuilder.CreateInvalidLengthErrorMessage(), Method, Note, Upstream Relationships

### Community 1090 - "Community 1090"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, MessageBuilder.CreateNotEmptyErrorMessage(), Method, Note, Upstream Relationships

### Community 1091 - "Community 1091"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, MessageBuilder.CreateNotFoundErrorMessage(), Method, Note, Upstream Relationships

### Community 1092 - "Community 1092"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, MessageBuilder.CreateNotNullErrorMessage(), Method, Note, Upstream Relationships

### Community 1093 - "Community 1093"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, MessageBuilder.CreateNotSameErrorMessage(), Method, Note, Upstream Relationships

### Community 1094 - "Community 1094"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, MessageBuilder.cs, Note, Upstream Relationships

### Community 1095 - "Community 1095"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, MetaModel, Note, Upstream Relationships

### Community 1096 - "Community 1096"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, MethodInfo, Note, References, Upstream Relationships

### Community 1097 - "Community 1097"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Inherits, Migration, Note, Upstream Relationships

### Community 1098 - "Community 1098"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, ModelBuilderExtensions.AddCascadeDeleteBehaviorConvention(), Note, Upstream Relationships

### Community 1099 - "Community 1099"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, ModelBuilderExtensions.AddPluralizingTableNameConvention(), Note, Upstream Relationships

### Community 1100 - "Community 1100"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, ModelBuilderExtensions.AddRestrictDeleteBehaviorConvention(), Note, Upstream Relationships

### Community 1101 - "Community 1101"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, ModelBuilderExtensions.AddSingularizingTableNameConvention(), Note, Upstream Relationships

### Community 1102 - "Community 1102"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, ModelBuilderExtensions.cs, Note, Upstream Relationships

### Community 1103 - "Community 1103"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, ModelBuilderExtensions.RegisterAllEntities(), Note, Upstream Relationships

### Community 1104 - "Community 1104"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, ModelBuilderExtensions.RegisterEntityTypeConfiguration(), Note, Upstream Relationships

### Community 1105 - "Community 1105"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Inherits, ModelSnapshot, Note, Upstream Relationships

### Community 1106 - "Community 1106"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, NationalCodeValidator.cs, Note, Upstream Relationships

### Community 1107 - "Community 1107"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, NationalCodeValidator.GetDefaultMessageTemplate(), Note, Upstream Relationships

### Community 1108 - "Community 1108"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, NationalCodeValidator.IsValid(), Note, Upstream Relationships

### Community 1109 - "Community 1109"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, NewtonsoftHelper.cs, Note, Upstream Relationships

### Community 1110 - "Community 1110"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Non-Functional Requirements (2), Note, Upstream Relationships

### Community 1111 - "Community 1111"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Non-Functional Requirements (3), Note, Upstream Relationships

### Community 1112 - "Community 1112"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Non-Functional Requirements (4), Note, Upstream Relationships

### Community 1113 - "Community 1113"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Non-Functional Requirements (5), Note, Upstream Relationships

### Community 1114 - "Community 1114"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Non-Functional Requirements (6), Note, Upstream Relationships

### Community 1115 - "Community 1115"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Non-Functional Requirements (7), Note, Upstream Relationships

### Community 1116 - "Community 1116"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Non-Functional Requirements (8), Note, Upstream Relationships

### Community 1117 - "Community 1117"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Non-Functional Requirements, Note, Upstream Relationships

### Community 1118 - "Community 1118"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Non-Negotiable Layer Dependency Rules, Note, Upstream Relationships

### Community 1119 - "Community 1119"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, NoOpAppCacheService.GetAsync(), Note, Upstream Relationships

### Community 1120 - "Community 1120"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, NoOpAppCacheService.RemoveAsync(), Note, Upstream Relationships

### Community 1121 - "Community 1121"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, NoOpAppCacheService.SetAsync(), Note, Upstream Relationships

### Community 1122 - "Community 1122"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Normalize-WarningLine(), Note, Upstream Relationships

### Community 1123 - "Community 1123"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Notes, Upstream Relationships

### Community 1124 - "Community 1124"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, NotFoundException.cs, Upstream Relationships

### Community 1125 - "Community 1125"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Nullability-Quality-Staged-Plan.md, Upstream Relationships

### Community 1126 - "Community 1126"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, ObservabilitySettings, Upstream Relationships

### Community 1127 - "Community 1127"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Open Questions (2), Upstream Relationships

### Community 1128 - "Community 1128"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Open Questions (3), Upstream Relationships

### Community 1129 - "Community 1129"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Open Questions (4), Upstream Relationships

### Community 1130 - "Community 1130"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Open Questions (5), Upstream Relationships

### Community 1131 - "Community 1131"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Open Questions (6), Upstream Relationships

### Community 1132 - "Community 1132"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Open Questions (7), Upstream Relationships

### Community 1133 - "Community 1133"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Open Questions (8), Upstream Relationships

### Community 1134 - "Community 1134"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Open Questions, Upstream Relationships

### Community 1135 - "Community 1135"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Note, OperationCatalogSecuritySettings (2), References, Upstream Relationships

### Community 1136 - "Community 1136"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, OperationCatalogSecuritySettings, Upstream Relationships

### Community 1137 - "Community 1137"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, OperationCatalogService.GetOperations(), Upstream Relationships

### Community 1138 - "Community 1138"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, OperationCatalogService.IsOperationAllowedByPolicy(), Upstream Relationships

### Community 1139 - "Community 1139"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, OperationCatalogService.IsServiceResolvable(), Upstream Relationships

### Community 1140 - "Community 1140"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, OperationResult.cs, Upstream Relationships

### Community 1141 - "Community 1141"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, OperationResult.SetProperties(), Upstream Relationships

### Community 1142 - "Community 1142"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Output Notes, Upstream Relationships

### Community 1143 - "Community 1143"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Inherits, Note, PageModel, Upstream Relationships

### Community 1144 - "Community 1144"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, PagingDTO.cs, Upstream Relationships

### Community 1145 - "Community 1145"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, PagingRequest (2), Upstream Relationships

### Community 1146 - "Community 1146"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, PagingRequest.cs, Upstream Relationships

### Community 1147 - "Community 1147"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Inherits, Note, PagingRequest, Upstream Relationships

### Community 1148 - "Community 1148"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, PagingWithoutData (2), Upstream Relationships

### Community 1149 - "Community 1149"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Inherits, Note, PagingWithoutData, Upstream Relationships

### Community 1150 - "Community 1150"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, PasswordValidator.cs, Upstream Relationships

### Community 1151 - "Community 1151"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, PasswordValidator.GetDefaultMessageTemplate(), Upstream Relationships

### Community 1152 - "Community 1152"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, PasswordValidator.IsValid(), Upstream Relationships

### Community 1153 - "Community 1153"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, payments.md, Upstream Relationships

### Community 1154 - "Community 1154"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, PerformanceSettings.cs, Upstream Relationships

### Community 1155 - "Community 1155"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, PerformanceSettings, Upstream Relationships

### Community 1156 - "Community 1156"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Phased Delivery, Upstream Relationships

### Community 1157 - "Community 1157"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, PhoneNumberDTO.cs, Upstream Relationships

### Community 1158 - "Community 1158"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, PhoneNumberDTO, Upstream Relationships

### Community 1159 - "Community 1159"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, PhoneNumberValidator.cs, Upstream Relationships

### Community 1160 - "Community 1160"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, PhoneNumberValidator.GetDefaultMessageTemplate(), Upstream Relationships

### Community 1161 - "Community 1161"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, PhoneNumberValidator.IsValid(), Upstream Relationships

### Community 1162 - "Community 1162"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, PostalServerMailRequest.cs, Upstream Relationships

### Community 1163 - "Community 1163"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, PostalServerMailRequest, Upstream Relationships

### Community 1164 - "Community 1164"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Privacy.cshtml.cs, Upstream Relationships

### Community 1165 - "Community 1165"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, PrivacyModel.OnGet(), Upstream Relationships

### Community 1166 - "Community 1166"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Inherits, Note, Profile, Upstream Relationships

### Community 1167 - "Community 1167"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Project Snapshot, Upstream Relationships

### Community 1168 - "Community 1168"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Inherits, Note, PropertyValidator, Upstream Relationships

### Community 1169 - "Community 1169"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, PropertyValueDTO.cs, Upstream Relationships

### Community 1170 - "Community 1170"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, PropertyValueDTO, Upstream Relationships

### Community 1171 - "Community 1171"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, PublicEnumAttribute.cs, Upstream Relationships

### Community 1172 - "Community 1172"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Purpose, Upstream Relationships

### Community 1173 - "Community 1173"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, PutObjectRequest, Upstream Relationships

### Community 1174 - "Community 1174"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, QueryableExtensions.CreatePage(), Upstream Relationships

### Community 1175 - "Community 1175"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, QueryableExtensions.cs, Upstream Relationships

### Community 1176 - "Community 1176"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Note, QueryTuningSettings (2), References, Upstream Relationships

### Community 1177 - "Community 1177"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, QueryTuningSettings, Upstream Relationships

### Community 1178 - "Community 1178"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Quick check in VS Code, Upstream Relationships

### Community 1179 - "Community 1179"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Note, Random, References, Upstream Relationships

### Community 1180 - "Community 1180"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, RateLimitingSettings, Upstream Relationships

### Community 1181 - "Community 1181"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, README.md (2), Upstream Relationships

### Community 1182 - "Community 1182"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, README.md, Upstream Relationships

### Community 1183 - "Community 1183"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, ReflectionExtensions.cs, Upstream Relationships

### Community 1184 - "Community 1184"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, ReflectionHelper.BaseTypeIsGeneric(), Upstream Relationships

### Community 1185 - "Community 1185"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, ReflectionHelper.GetBaseTypesAndInterfaces(), Upstream Relationships

### Community 1186 - "Community 1186"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, ReflectionHelper.IsEnumerable(), Upstream Relationships

### Community 1187 - "Community 1187"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, RegexHelper.cs, Upstream Relationships

### Community 1188 - "Community 1188"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, RemoveVersionParameters.Apply(), Upstream Relationships

### Community 1189 - "Community 1189"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, RemoveVersionParameters.cs, Upstream Relationships

### Community 1190 - "Community 1190"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Repo Skills, Upstream Relationships

### Community 1191 - "Community 1191"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, RequestContextEnrichmentMiddleware.cs, Upstream Relationships

### Community 1192 - "Community 1192"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, RequestContextEnrichmentMiddleware.InvokeAsync(), Upstream Relationships

### Community 1193 - "Community 1193"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Note, References, RequestDelegate, Upstream Relationships

### Community 1194 - "Community 1194"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, RequestTelemetryMiddleware.cs, Upstream Relationships

### Community 1195 - "Community 1195"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, RoleDataInitializer.cs, Upstream Relationships

### Community 1196 - "Community 1196"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, RoleDataInitializer.InitializeData(), Upstream Relationships

### Community 1197 - "Community 1197"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Note, References, RoleManager, Upstream Relationships

### Community 1198 - "Community 1198"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, RoleViewDTO.CreateMappings(), Upstream Relationships

### Community 1199 - "Community 1199"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, RoleViewDTO.cs, Upstream Relationships

### Community 1200 - "Community 1200"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Safety Rules, Upstream Relationships

### Community 1201 - "Community 1201"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Scope (2), Upstream Relationships

### Community 1202 - "Community 1202"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Scope (3), Upstream Relationships

### Community 1203 - "Community 1203"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Scope (4), Upstream Relationships

### Community 1204 - "Community 1204"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Scope (5), Upstream Relationships

### Community 1205 - "Community 1205"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Scope (6), Upstream Relationships

### Community 1206 - "Community 1206"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Scope (7), Upstream Relationships

### Community 1207 - "Community 1207"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Scope (8), Upstream Relationships

### Community 1208 - "Community 1208"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Scope, Upstream Relationships

### Community 1209 - "Community 1209"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, SecurityHelper.cs, Upstream Relationships

### Community 1210 - "Community 1210"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, SecurityHelper.GetSha256Hash(), Upstream Relationships

### Community 1211 - "Community 1211"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, SendEmailRequest.cs, Upstream Relationships

### Community 1212 - "Community 1212"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, SendEmailRequest, Upstream Relationships

### Community 1213 - "Community 1213"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, SentEmailDTO.CreateMappings(), Upstream Relationships

### Community 1214 - "Community 1214"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, SentEmailDTO.cs, Upstream Relationships

### Community 1215 - "Community 1215"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, SeqSettings, Upstream Relationships

### Community 1216 - "Community 1216"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, SerilogConfiguration.cs, Upstream Relationships

### Community 1217 - "Community 1217"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, SerilogConfiguration.UseCustomSerilogRequestLogging(), Upstream Relationships

### Community 1218 - "Community 1218"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, SerilogConfiguration.UseSerilog(), Upstream Relationships

### Community 1219 - "Community 1219"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, ServiceCollectionExtensions.AddControllersConfig(), Upstream Relationships

### Community 1220 - "Community 1220"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, ServiceCollectionExtensions.AddCorsPolicy(), Upstream Relationships

### Community 1221 - "Community 1221"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, ServiceCollectionExtensions.AddDataInitializers(), Upstream Relationships

### Community 1222 - "Community 1222"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, ServiceCollectionExtensions.AddDbContext(), Upstream Relationships

### Community 1223 - "Community 1223"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, ServiceCollectionExtensions.AddFluentValidation(), Upstream Relationships

### Community 1224 - "Community 1224"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, ServiceCollectionExtensions.AddJwtAuthentication(), Upstream Relationships

### Community 1225 - "Community 1225"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, ServiceCollectionExtensions.AddSessionStorage(), Upstream Relationships

### Community 1226 - "Community 1226"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, ServiceCollectionExtensions.cs, Upstream Relationships

### Community 1227 - "Community 1227"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Services.DTOs.Users, Upstream Relationships

### Community 1228 - "Community 1228"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, SessionManager.cs, Upstream Relationships

### Community 1229 - "Community 1229"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, SessionManager.Get(), Upstream Relationships

### Community 1230 - "Community 1230"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, SessionManager.Remove(), Upstream Relationships

### Community 1231 - "Community 1231"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, SessionManager.Set(), Upstream Relationships

### Community 1232 - "Community 1232"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, SetRolesAdminDTO.cs, Upstream Relationships

### Community 1233 - "Community 1233"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, SetRolesAdminDTO, Upstream Relationships

### Community 1234 - "Community 1234"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, SetVersionInPaths.Apply(), Upstream Relationships

### Community 1235 - "Community 1235"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, SetVersionInPaths.cs, Upstream Relationships

### Community 1236 - "Community 1236"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, SimpleRateLimitMiddleware.cs, Upstream Relationships

### Community 1237 - "Community 1237"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, SiteSettings.cs, Upstream Relationships

### Community 1238 - "Community 1238"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, SiteSettings, Upstream Relationships

### Community 1239 - "Community 1239"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Skillهای ریپویی, Upstream Relationships

### Community 1240 - "Community 1240"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, SlowQueryLoggingInterceptor.CommandFailed(), Upstream Relationships

### Community 1241 - "Community 1241"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, SlowQueryLoggingInterceptor.cs, Upstream Relationships

### Community 1242 - "Community 1242"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, StartAuthenticationRequest.cs, Upstream Relationships

### Community 1243 - "Community 1243"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, StartAuthenticationRequest, Upstream Relationships

### Community 1244 - "Community 1244"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Note, References, string, Upstream Relationships

### Community 1245 - "Community 1245"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, StringExtensions.CheckValidCardNumber(), Upstream Relationships

### Community 1246 - "Community 1246"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, StringExtensions.cs, Upstream Relationships

### Community 1247 - "Community 1247"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, StringExtensions.En2Fa(), Upstream Relationships

### Community 1248 - "Community 1248"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, StringExtensions.GetEnumDisplayName(), Upstream Relationships

### Community 1249 - "Community 1249"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, StringExtensions.HasValue(), Upstream Relationships

### Community 1250 - "Community 1250"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, StringExtensions.NormalizeMobile(), Upstream Relationships

### Community 1251 - "Community 1251"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, StringExtensions.ReverseDateTime(), Upstream Relationships

### Community 1252 - "Community 1252"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, StringExtensions.ToBase64(), Upstream Relationships

### Community 1253 - "Community 1253"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, StringExtensions.ToCurrency(), Upstream Relationships

### Community 1254 - "Community 1254"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, StringExtensions.ToDecimal(), Upstream Relationships

### Community 1255 - "Community 1255"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, StringExtensions.ToInt(), Upstream Relationships

### Community 1256 - "Community 1256"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, StringExtensions.ToNumeric(), Upstream Relationships

### Community 1257 - "Community 1257"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Success(), Upstream Relationships

### Community 1258 - "Community 1258"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Suggested cleanup order, Upstream Relationships

### Community 1259 - "Community 1259"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Suggested Local Verification Commands, Upstream Relationships

### Community 1260 - "Community 1260"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, support.md, Upstream Relationships

### Community 1261 - "Community 1261"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, SwaggerConfigurationExtensions.AddSwagger(), Upstream Relationships

### Community 1262 - "Community 1262"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, SwaggerConfigurationExtensions.cs, Upstream Relationships

### Community 1263 - "Community 1263"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, SwaggerConfigurationExtensions.UseSwaggerAndUI(), Upstream Relationships

### Community 1264 - "Community 1264"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Task Execution Protocol (Agent Workflow), Upstream Relationships

### Community 1265 - "Community 1265"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, TelemetrySettings, Upstream Relationships

### Community 1266 - "Community 1266"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Template, Upstream Relationships

### Community 1267 - "Community 1267"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, TokenDTO.cs, Upstream Relationships

### Community 1268 - "Community 1268"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, TokenDTO, Upstream Relationships

### Community 1269 - "Community 1269"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, TotalCountPage.cs, Upstream Relationships

### Community 1270 - "Community 1270"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, TotalCountPage, Upstream Relationships

### Community 1271 - "Community 1271"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Note, References, Type, Upstream Relationships

### Community 1272 - "Community 1272"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, TypeExtensions.cs, Upstream Relationships

### Community 1273 - "Community 1273"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, TypeExtensions.GetPropertyDisplayNamesDictionary(), Upstream Relationships

### Community 1274 - "Community 1274"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, UnauthorizedResponsesOperationFilter.Apply(), Upstream Relationships

### Community 1275 - "Community 1275"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, UnauthorizedResponsesOperationFilter.cs, Upstream Relationships

### Community 1276 - "Community 1276"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, UploadRequest, Upstream Relationships

### Community 1277 - "Community 1277"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, UserDataInitializer.cs

### Community 1278 - "Community 1278"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UserDataInitializer.InitializeData()

### Community 1279 - "Community 1279"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, UserIdDTO.cs

### Community 1280 - "Community 1280"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, UserIdDTO

### Community 1281 - "Community 1281"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, UserIdDTOValidator.cs

### Community 1282 - "Community 1282"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, UserListDTO.CreateMappings()

### Community 1283 - "Community 1283"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, UserListDTO.cs

### Community 1284 - "Community 1284"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, UtilConvertor.cs

### Community 1285 - "Community 1285"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, Validation Rules (2)

### Community 1286 - "Community 1286"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, Validation Rules (3)

### Community 1287 - "Community 1287"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, Validation Rules (4)

### Community 1288 - "Community 1288"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, Validation Rules (5)

### Community 1289 - "Community 1289"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, Validation Rules (6)

### Community 1290 - "Community 1290"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, Validation Rules (7)

### Community 1291 - "Community 1291"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, Validation Rules (8)

### Community 1292 - "Community 1292"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, Validation Rules

### Community 1293 - "Community 1293"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, ValidationError

### Community 1294 - "Community 1294"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, ValidationHelper.cs

### Community 1295 - "Community 1295"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, ValidationHelper.IsLegalNationalCode()

### Community 1296 - "Community 1296"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, ValidationHelper.IsNaturalNationalCode()

### Community 1297 - "Community 1297"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, Verification (2)

### Community 1298 - "Community 1298"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, Verification

### Community 1299 - "Community 1299"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, Warning & Quality Gate Policy

### Community 1300 - "Community 1300"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, Warning Gate (Staged Quality)

### Community 1301 - "Community 1301"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, WebApplicationBuilderExtensions.AddAutofac()

### Community 1302 - "Community 1302"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, Upstream Relationships, WebApplicationBuilderExtensions.ConfigureSection()

### Community 1303 - "Community 1303"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, WebApplicationBuilderExtensions.cs

### Community 1304 - "Community 1304"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, Why staged-

### Community 1305 - "Community 1305"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, Workflow انجام تسک

### Community 1306 - "Community 1306"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, دستورالعمل‌های سلسله‌مراتبی

### Community 1307 - "Community 1307"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, سیاست کیفیت و Warning

### Community 1308 - "Community 1308"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, فرمان‌های پیشنهادی برای بررسی محلی

### Community 1309 - "Community 1309"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, قوانین وابستگی لایه‌ها (غیرقابل مذاکره)

### Community 1310 - "Community 1310"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, قوانین پیاده‌سازی

### Community 1311 - "Community 1311"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, مالکیت فایل‌ها

### Community 1312 - "Community 1312"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, نمای کلی پروژه

### Community 1313 - "Community 1313"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, هدف

### Community 1314 - "Community 1314"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.Add(), Method, Note, Upstream Relationships

### Community 1315 - "Community 1315"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.AddAsync(), Method, Note, Upstream Relationships

### Community 1316 - "Community 1316"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.AddRange(), Method, Note, Upstream Relationships

### Community 1317 - "Community 1317"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.AddRangeAsync(), Method, Note, Upstream Relationships

### Community 1318 - "Community 1318"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.Attach(), Method, Note, Upstream Relationships

### Community 1319 - "Community 1319"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.CountByAsync(), Method, Note, Upstream Relationships

### Community 1320 - "Community 1320"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.CreatePage(), Method, Note, Upstream Relationships

### Community 1321 - "Community 1321"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IRepository.cs, Note, Upstream Relationships

### Community 1322 - "Community 1322"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.Delete(), Method, Note, Upstream Relationships

### Community 1323 - "Community 1323"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.DeleteAsync(), Method, Note, Upstream Relationships

### Community 1324 - "Community 1324"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.DeleteById(), Method, Note, Upstream Relationships

### Community 1325 - "Community 1325"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.DeleteByIdAsync(), Method, Note, Upstream Relationships

### Community 1326 - "Community 1326"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.DeleteRange(), Method, Note, Upstream Relationships

### Community 1327 - "Community 1327"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.DeleteRangeAsync(), Method, Note, Upstream Relationships

### Community 1328 - "Community 1328"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.Detach(), Method, Note, Upstream Relationships

### Community 1329 - "Community 1329"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.GetByCondition(), Method, Note, Upstream Relationships

### Community 1330 - "Community 1330"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.GetByConditionAsync(), Method, Note, Upstream Relationships

### Community 1331 - "Community 1331"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.GetById(), Method, Note, Upstream Relationships

### Community 1332 - "Community 1332"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.GetByIdAsync(), Method, Note, Upstream Relationships

### Community 1333 - "Community 1333"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.GetColumnValue(), Method, Note, Upstream Relationships

### Community 1334 - "Community 1334"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.GetColumnValueAsync(), Method, Note, Upstream Relationships

### Community 1335 - "Community 1335"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.IsExist(), Method, Note, Upstream Relationships

### Community 1336 - "Community 1336"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.IsExistAsync(), Method, Note, Upstream Relationships

### Community 1337 - "Community 1337"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.IsNotDuplicateAsync(), Method, Note, Upstream Relationships

### Community 1338 - "Community 1338"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.LoadCollection(), Method, Note, Upstream Relationships

### Community 1339 - "Community 1339"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.LoadCollectionAsync(), Method, Note, Upstream Relationships

### Community 1340 - "Community 1340"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.LoadReference(), Method, Note, Upstream Relationships

### Community 1341 - "Community 1341"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.LoadReferenceAsync(), Method, Note, Upstream Relationships

### Community 1342 - "Community 1342"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.SaveChanges(), Method, Note, Upstream Relationships

### Community 1343 - "Community 1343"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.SaveChangesAsync(), Method, Note, Upstream Relationships

### Community 1344 - "Community 1344"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.SelectBy(), Method, Note, Upstream Relationships

### Community 1345 - "Community 1345"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.SelectByAsync(), Method, Note, Upstream Relationships

### Community 1346 - "Community 1346"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.SelectByCursorAsync(), Method, Note, Upstream Relationships

### Community 1347 - "Community 1347"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.Update(), Method, Note, Upstream Relationships

### Community 1348 - "Community 1348"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.UpdateAsync(), Method, Note, Upstream Relationships

### Community 1349 - "Community 1349"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.UpdateCustomPropertiesAsync(), Method, Note, Upstream Relationships

### Community 1350 - "Community 1350"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.UpdateRange(), Method, Note, Upstream Relationships

### Community 1351 - "Community 1351"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IRepository.UpdateRangeAsync(), Method, Note, Upstream Relationships

### Community 1352 - "Community 1352"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IServiceRepository.Create(), Method, Note, Upstream Relationships

### Community 1353 - "Community 1353"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IServiceRepository.Delete(), Method, Note, Upstream Relationships

### Community 1354 - "Community 1354"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IServiceRepository.Get(), Method, Note, Upstream Relationships

### Community 1355 - "Community 1355"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IServiceRepository.Select(), Method, Note, Upstream Relationships

### Community 1356 - "Community 1356"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IServiceRepository.SelectByCursor(), Method, Note, Upstream Relationships

### Community 1357 - "Community 1357"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IServiceRepository.Update(), Method, Note, Upstream Relationships

### Community 1358 - "Community 1358"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IServiceRepository.UpdateCustomProperties(), Method, Note, Upstream Relationships

### Community 1359 - "Community 1359"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IServiceRepository.UpdateProperty(), Method, Note, Upstream Relationships

### Community 1360 - "Community 1360"
Cohesion: 0.29
Nodes (6): AdminPanelAuthService.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1361 - "Community 1361"
Cohesion: 0.29
Nodes (6): AuthenticationService.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1362 - "Community 1362"
Cohesion: 0.29
Nodes (6): AuthenticationService.GetMyAuthorizationAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 1363 - "Community 1363"
Cohesion: 0.29
Nodes (6): AuthenticationService.SignOutAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 1364 - "Community 1364"
Cohesion: 0.29
Nodes (6): AuthorizationCatalog.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1365 - "Community 1365"
Cohesion: 0.29
Nodes (6): AuthorizationManagementService.cs, Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1366 - "Community 1366"
Cohesion: 0.29
Nodes (6): AuthorizationManagementService.UpsertUserPlanSubscriptionAsync(), Dependencies, Downstream Relationships, Method, Note, Upstream Relationships

### Community 1367 - "Community 1367"
Cohesion: 0.29
Nodes (6): CachingSettings (2), Dependencies, Downstream Relationships, Note, References, Upstream Relationships

### Community 1368 - "Community 1368"
Cohesion: 0.29
Nodes (6): DbContext, Dependencies, Downstream Relationships, Note, References, Upstream Relationships

### Community 1369 - "Community 1369"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, DistributedAppCacheService.cs, Downstream Relationships, Note, Upstream Relationships

### Community 1370 - "Community 1370"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, EffectiveAuthorizationService.cs, Note, Upstream Relationships

### Community 1371 - "Community 1371"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, EntityAdminService.cs, Note, Upstream Relationships

### Community 1372 - "Community 1372"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IAppCacheService.cs, Note, Upstream Relationships

### Community 1373 - "Community 1373"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthenticationService (2), Inherits, Note, Upstream Relationships

### Community 1374 - "Community 1374"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IAuthenticationService.cs, Note, Upstream Relationships

### Community 1375 - "Community 1375"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IAuthorizationManagementService (2), Inherits, Note, Upstream Relationships

### Community 1376 - "Community 1376"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IAuthorizationManagementService.cs, Note, Upstream Relationships

### Community 1377 - "Community 1377"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IEffectiveAuthorizationService.cs, Note, Upstream Relationships

### Community 1378 - "Community 1378"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IEntityAdminService (2), Note, References, Upstream Relationships

### Community 1379 - "Community 1379"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IHttpContextAccessor, Note, References, Upstream Relationships

### Community 1380 - "Community 1380"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IJwtService.cs, Note, Upstream Relationships

### Community 1381 - "Community 1381"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IOperationCatalogService (2), Note, References, Upstream Relationships

### Community 1382 - "Community 1382"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Inherits, IRoleService (2), Note, Upstream Relationships

### Community 1383 - "Community 1383"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IRoleService.cs, Note, Upstream Relationships

### Community 1384 - "Community 1384"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, ISenderService.cs, Note, Upstream Relationships

### Community 1385 - "Community 1385"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, IServiceProvider, Note, References, Upstream Relationships

### Community 1386 - "Community 1386"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IServiceRepository.cs, Note, Upstream Relationships

### Community 1387 - "Community 1387"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Inherits, IUserService (2), Note, Upstream Relationships

### Community 1388 - "Community 1388"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, IUserService.cs, Note, Upstream Relationships

### Community 1389 - "Community 1389"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, JwtService.cs, Note, Upstream Relationships

### Community 1390 - "Community 1390"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, JwtService.Validate(), Method, Note, Upstream Relationships

### Community 1391 - "Community 1391"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, NoOpAppCacheService.cs, Note, Upstream Relationships

### Community 1392 - "Community 1392"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, OperationCatalogService.cs, Upstream Relationships

### Community 1393 - "Community 1393"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, Repository.CountByAsync(), Upstream Relationships

### Community 1394 - "Community 1394"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Repository.cs, Upstream Relationships

### Community 1395 - "Community 1395"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, Repository.Detach(), Upstream Relationships

### Community 1396 - "Community 1396"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, Repository.GetByCondition(), Upstream Relationships

### Community 1397 - "Community 1397"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, Repository.GetByConditionAsync(), Upstream Relationships

### Community 1398 - "Community 1398"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, Repository.GetColumnValue(), Upstream Relationships

### Community 1399 - "Community 1399"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, Repository.GetColumnValueAsync(), Upstream Relationships

### Community 1400 - "Community 1400"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, Repository.IsExist(), Upstream Relationships

### Community 1401 - "Community 1401"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, Repository.IsExistAsync(), Upstream Relationships

### Community 1402 - "Community 1402"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, Repository.IsNotDuplicateAsync(), Upstream Relationships

### Community 1403 - "Community 1403"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, RoleService.AddRolesAsync(), Upstream Relationships

### Community 1404 - "Community 1404"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, RoleService.cs, Upstream Relationships

### Community 1405 - "Community 1405"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, RoleService.GetAllAsync(), Upstream Relationships

### Community 1406 - "Community 1406"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, RoleService.GetRolesByUserAsync(), Upstream Relationships

### Community 1407 - "Community 1407"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, RoleService.SetRolesToAdminAsync(), Upstream Relationships

### Community 1408 - "Community 1408"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, SenderService.cs, Upstream Relationships

### Community 1409 - "Community 1409"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, SenderService.SendGeneralSmsAsync(), Upstream Relationships

### Community 1410 - "Community 1410"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, SenderService.SendOtpSmsAsync(), Upstream Relationships

### Community 1411 - "Community 1411"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, SenderService.SendTestSmsAsync(), Upstream Relationships

### Community 1412 - "Community 1412"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, ServiceRepository.cs, Upstream Relationships

### Community 1413 - "Community 1413"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, ServiceRepository.Delete(), Upstream Relationships

### Community 1414 - "Community 1414"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, ServiceRepository.Select(), Upstream Relationships

### Community 1415 - "Community 1415"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Method, Note, ServiceRepository.SelectByCursor(), Upstream Relationships

### Community 1416 - "Community 1416"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Note, References, SignInManager, Upstream Relationships

### Community 1417 - "Community 1417"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Note, References, SiteSettings (2), Upstream Relationships

### Community 1418 - "Community 1418"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Note, References, TimeSpan, Upstream Relationships

### Community 1419 - "Community 1419"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, UserContext.cs

### Community 1420 - "Community 1420"
Cohesion: 0.29
Nodes (6): Dependencies, Downstream Relationships, Note, References, Upstream Relationships, UserManager

### Community 1421 - "Community 1421"
Cohesion: 0.29
Nodes (6): Contains, Dependencies, Downstream Relationships, Note, Upstream Relationships, UserService.cs

### Community 1422 - "Community 1422"
Cohesion: 0.29
Nodes (6): Adapterهای مناسب در وضعیت فعلی, Adapterهایی که فعلا نباید اضافه شوند, MCP Adapter Playbook, اصل طراحی, اولویت فعلی, هدف

### Community 1424 - "Community 1424"
Cohesion: 0.33
Nodes (5): IdentitySettings, JwtSettings, MailSettings, SeqSettings, SiteSettings

### Community 1429 - "Community 1429"
Cohesion: 0.33
Nodes (5): Current Recommendation, Direct PostgreSQL MCP, MCP Notes, Recommended Adapters, Why These First

### Community 1430 - "Community 1430"
Cohesion: 0.33
Nodes (5): Episodic Memory, Graph Memory, Memory Model, Semantic Memory, رابطه بین سه حافظه

### Community 1431 - "Community 1431"
Cohesion: 0.33
Nodes (5): AI Agent Rules (VS Code + GPT-5.3-Codex), Codex Customization (Applied), Notes, Quick check in VS Code, Warning Gate (Staged Quality)

### Community 1432 - "Community 1432"
Cohesion: 0.33
Nodes (5): Episodic Memory Eval, سناریوهای حداقلی, سوال‌های ارزیابی, شاخص‌های retrieval, معیار پذیرش

### Community 1433 - "Community 1433"
Cohesion: 0.33
Nodes (5): AccountStatus.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1434 - "Community 1434"
Cohesion: 0.33
Nodes (5): BankAccountStatus.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1435 - "Community 1435"
Cohesion: 0.33
Nodes (5): Dependencies, DocumentStatus.cs, Downstream Relationships, Note, Upstream Relationships

### Community 1436 - "Community 1436"
Cohesion: 0.33
Nodes (5): Dependencies, DocumentType.cs, Downstream Relationships, Note, Upstream Relationships

### Community 1437 - "Community 1437"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, EmailStatus.cs, Note, Upstream Relationships

### Community 1438 - "Community 1438"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Entities.AssemblyInfo.cs, Note, Upstream Relationships

### Community 1439 - "Community 1439"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Entities.GlobalUsings.g.cs, Note, Upstream Relationships

### Community 1440 - "Community 1440"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, PaymentDocumentType.cs, Upstream Relationships

### Community 1441 - "Community 1441"
Cohesion: 0.33
Nodes (5): AccountSessionDtos.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1442 - "Community 1442"
Cohesion: 0.33
Nodes (5): AdminPanel.AssemblyInfo.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1443 - "Community 1443"
Cohesion: 0.33
Nodes (5): AdminPanel.GlobalUsings.g.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1444 - "Community 1444"
Cohesion: 0.33
Nodes (5): AdminPanel.RazorAssemblyInfo.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1445 - "Community 1445"
Cohesion: 0.33
Nodes (5): API.AssemblyInfo.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1446 - "Community 1446"
Cohesion: 0.33
Nodes (5): API.GlobalUsings.g.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1447 - "Community 1447"
Cohesion: 0.33
Nodes (5): API.MvcApplicationPartsAssemblyInfo.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1448 - "Community 1448"
Cohesion: 0.33
Nodes (5): AuthorizationManagementDtos.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1449 - "Community 1449"
Cohesion: 0.33
Nodes (5): Common.AssemblyInfo.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1450 - "Community 1450"
Cohesion: 0.33
Nodes (5): Common.GlobalUsings.g.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1451 - "Community 1451"
Cohesion: 0.33
Nodes (5): CompletePhoneRegistrationRequest.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1452 - "Community 1452"
Cohesion: 0.33
Nodes (5): CompleteUserRegistrationRequest.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1453 - "Community 1453"
Cohesion: 0.33
Nodes (5): ConfirmEmailRegistrationRequest.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1454 - "Community 1454"
Cohesion: 0.33
Nodes (5): CurrentUserAuthorizationResponse.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1455 - "Community 1455"
Cohesion: 0.33
Nodes (5): Data.AssemblyInfo.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1456 - "Community 1456"
Cohesion: 0.33
Nodes (5): Data.GlobalUsings.g.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1457 - "Community 1457"
Cohesion: 0.33
Nodes (5): DbCloneRunner.AssemblyInfo.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1458 - "Community 1458"
Cohesion: 0.33
Nodes (5): DbCloneRunner.GlobalUsings.g.cs, Dependencies, Downstream Relationships, Note, Upstream Relationships

### Community 1459 - "Community 1459"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, EmailPasswordLoginRequest.cs, Note, Upstream Relationships

### Community 1460 - "Community 1460"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, ForgotPasswordRequest.cs, Note, Upstream Relationships

### Community 1461 - "Community 1461"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, GlobalUsing.cs (2), Note, Upstream Relationships

### Community 1462 - "Community 1462"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, GlobalUsing.cs, Note, Upstream Relationships

### Community 1463 - "Community 1463"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Initialize-Database.ps1, Note, Upstream Relationships

### Community 1464 - "Community 1464"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, LICENSE.md, Note, Upstream Relationships

### Community 1465 - "Community 1465"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, LoginResponse.cs, Note, Upstream Relationships

### Community 1466 - "Community 1466"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, MfaDtos.cs, Note, Upstream Relationships

### Community 1467 - "Community 1467"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, OtpChallengeResponse.cs, Upstream Relationships

### Community 1468 - "Community 1468"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, PasswordLoginRequest.cs, Upstream Relationships

### Community 1469 - "Community 1469"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, PhoneOtpLoginRequest.cs, Upstream Relationships

### Community 1470 - "Community 1470"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, Program.cs (2), Upstream Relationships

### Community 1471 - "Community 1471"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, Program.cs (3), Upstream Relationships

### Community 1472 - "Community 1472"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, Program.cs, Upstream Relationships

### Community 1473 - "Community 1473"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, RegisterWithEmailRequest.cs, Upstream Relationships

### Community 1474 - "Community 1474"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, ResendEmailActivationLinkRequest.cs, Upstream Relationships

### Community 1475 - "Community 1475"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, ResendPhoneRegistrationOtpRequest.cs, Upstream Relationships

### Community 1476 - "Community 1476"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, ResetPasswordWithOtpRequest.cs, Upstream Relationships

### Community 1477 - "Community 1477"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, SendOtpRequest.cs, Upstream Relationships

### Community 1478 - "Community 1478"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, SendOtpResponse.cs, Upstream Relationships

### Community 1479 - "Community 1479"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, SendPhoneLoginOtpRequest.cs, Upstream Relationships

### Community 1480 - "Community 1480"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, Sentry.Attributes.cs, Upstream Relationships

### Community 1481 - "Community 1481"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, Services.AssemblyInfo.cs, Upstream Relationships

### Community 1482 - "Community 1482"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, Services.GlobalUsings.g.cs, Upstream Relationships

### Community 1483 - "Community 1483"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, site.js, Upstream Relationships

### Community 1484 - "Community 1484"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, StartAuthenticationResponse.cs, Upstream Relationships

### Community 1485 - "Community 1485"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, StartPhoneRegistrationRequest.cs, Upstream Relationships

### Community 1486 - "Community 1486"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, UpdatePasswordRequest.cs, Upstream Relationships

### Community 1487 - "Community 1487"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, UpdatePasswordResponse.cs, Upstream Relationships

### Community 1488 - "Community 1488"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, Upstream Relationships, ValidateOtpRequest.cs

### Community 1489 - "Community 1489"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, Upstream Relationships, WebFramwork.AssemblyInfo.cs

### Community 1490 - "Community 1490"
Cohesion: 0.33
Nodes (5): Dependencies, Downstream Relationships, Note, Upstream Relationships, WebFramwork.GlobalUsings.g.cs

### Community 1492 - "Community 1492"
Cohesion: 0.4
Nodes (3): Entities.Items, Item, PostConfiguration

### Community 1493 - "Community 1493"
Cohesion: 0.4
Nodes (3): Category, CategoryConfiguration, Entities.Categories

### Community 1494 - "Community 1494"
Cohesion: 0.4
Nodes (3): ApplicationDbContextModelSnapshot, Data.Migrations, ModelSnapshot

### Community 1497 - "Community 1497"
Cohesion: 0.4
Nodes (4): AgentCapabilitiesDto, AgentDatabaseStatusDto, AgentMemoryStatusDto, AgentToolDescriptorDto

### Community 1498 - "Community 1498"
Cohesion: 0.5
Nodes (3): ApiResult, MetaModel, ValidationError

### Community 1499 - "Community 1499"
Cohesion: 0.4
Nodes (4): Feature جدید, Project Notes, لایه‌بندی, هدف

### Community 1500 - "Community 1500"
Cohesion: 0.4
Nodes (4): Episodic Memory Skill, خروجی مورد انتظار, عدم استفاده, ورودی‌های مهم

### Community 1501 - "Community 1501"
Cohesion: 0.4
Nodes (4): Editing Rules, Local Goals, Output Notes, Verification

### Community 1502 - "Community 1502"
Cohesion: 0.4
Nodes (4): Business Docs Index, Domains, How To Use, Template

### Community 1503 - "Community 1503"
Cohesion: 0.4
Nodes (4): code:dataview (TABLE type, source_file, community), Dataview, Folders, Graphify Obsidian Export

### Community 1504 - "Community 1504"
Cohesion: 0.4
Nodes (4): Authentication Change Checklist, Local Rules, Safety Rules, Verification

### Community 1505 - "Community 1505"
Cohesion: 0.5
Nodes (3): IScopedDependency, ISingletonDependency, ITransientDependency

### Community 1509 - "Community 1509"
Cohesion: 0.5
Nodes (3): IdentityUser, Entities.Users, User

### Community 1514 - "Community 1514"
Cohesion: 0.5
Nodes (3): IdentityRole, Entities.Users, Role

### Community 1529 - "Community 1529"
Cohesion: 1.0
Nodes (3): BaseEntity, Entities.Common, IEntity

### Community 1532 - "Community 1532"
Cohesion: 0.5
Nodes (3): PagingDTO, PagingWithoutData, PagingWithoutData

### Community 1542 - "Community 1542"
Cohesion: 0.5
Nodes (3): 2026-05-17 - Introduce Episodic Memory inside existing Onion stack, 2026-05-17 - Prefer MCP-safe application adapters over direct PostgreSQL access, Decision Log

## Knowledge Gaps
- **6164 isolated node(s):** `AdminPanelAuthenticationSettings`, `AdminPanelAuthenticationSettings`, `OperationCatalogSecuritySettings`, `Type`, `MethodInfo` (+6159 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **109 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `int` connect `Community 85` to `Community 0`, `Community 1`, `Community 199`, `Community 13`, `Community 89`, `Community 27`?**
  _High betweenness centrality (0.001) - this node is a cross-community bridge._
- **Why does `string` connect `Community 23` to `Community 1`, `Community 37`, `Community 38`, `Community 7`, `Community 199`, `Community 45`, `Community 46`, `Community 593`, `Community 594`, `Community 1428`, `Community 24`, `Community 27`, `Community 31`?**
  _High betweenness centrality (0.001) - this node is a cross-community bridge._
- **Why does `AuthenticationService` connect `Community 1` to `Community 204`, `Community 205`, `Community 17`, `Community 85`, `Community 21`, `Community 23`, `Community 22`, `Community 26`?**
  _High betweenness centrality (0.001) - this node is a cross-community bridge._
- **What connects `AdminPanelAuthenticationSettings`, `AdminPanelAuthenticationSettings`, `OperationCatalogSecuritySettings` to the rest of the system?**
  _6164 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Community 0` be split into smaller, more focused modules?**
  _Cohesion score 0.06 - nodes in this community are weakly interconnected._
- **Should `Community 1` be split into smaller, more focused modules?**
  _Cohesion score 0.11 - nodes in this community are weakly interconnected._
- **Should `Community 2` be split into smaller, more focused modules?**
  _Cohesion score 0.05 - nodes in this community are weakly interconnected._