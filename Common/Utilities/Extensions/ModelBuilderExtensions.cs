...
public static class ModelBuilderExtensions
{
    ...
    {
        //modelBuilder.AddDefaultValueSqlConvention("Id", typeof(Guid), "gen_random_uuid()");// agar db postgress bashad bayad in khat active bashad va na khate zir
        modelBuilder.AddDefaultValueSqlConvention("Id", typeof(Guid), "NEWSEQUENTIALID()");
        modelBuilder.AddDefaultValueSqlConvention("CreatedAt", typeof(DateTimeOffset), "NOW()");
    }
            ...
            foreach (Type iface in type.GetInterfaces())
            {
                if (iface.IsConstructedGenericType && iface.GetGenericTypeDefinition() == typeof(IEntityTypeConfiguration<>))
                {
                    ...
                    if (instance is not null)
                        applyConcreteMethod.Invoke(modelBuilder, [instance]);
                }
            }
        }
    }
    ...
    public static void AddDefaultValues(this ModelBuilder modelBuilder, string DbType)
    {
        var isPostgreSQL = DbType != null && DbType.ToLower() == "p";

        modelBuilder.AddDefaultValueSqlConvention("Id", typeof(Guid), isPostgreSQL ? "gen_random_uuid()" : "NEWSEQUENTIALID()");
        modelBuilder.AddDefaultValueSqlConvention("CreatedDate", typeof(DateTime), isPostgreSQL ? "now()" : "GetDate()");
    }
}
